...
| 2026-09-24 | `#___` | §2.10 겹침 각주에 **「동시 요청도 같은 판정을 받는다」** 문단 추가 (`#1629` · `F-8` 결정). 같은 선박에 겹치는 구간을 동시에 올리면 하나만 201이고 나머지는 기존 409다 — 서버가 겹침 조회 전에 선박 행을 잠가 직렬화하며(`TECH_SPEC §16.3`) 새 오류 코드·계약 변경은 없다. 수정(`§2.11`)·CSV 가져오기(`§8.2`)도 같은 검사를 지난다. `AGENTS §4.3`상 각주 보강이라 버전은 올리지 않는다 (#1629) |
