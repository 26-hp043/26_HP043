...
| 2026-09-23 | `#___` | **§15.1 「화면의 결과를 읽는 도구」 소절에 `lookup_regulation` 한 줄** (`#1703`) — 규제 기준값 표를 `source_ref`와 함께 읽는 챗봇 도구. `§7` 조회와 같은 서비스에서 값이 오고, 표에 없는 연도는 `null`로 둔다. `AGENTS §4.3`상 항목 추가라 버전은 올리지 않는다 (#1703) |
