...
| 2026-08-17 | PR #NNN | **v1.18 — §3.6 항차 실적 입력 구현 확정 (`#440`).** 상태별 허용 표(`IN_PROGRESS`·`COMPLETED`만) · 계획값을 지우지 않는 근거(`PRD §8.4`) · CF snapshot 보존 · 오류 응답 4종을 명시했다. 종전에는 요청·응답 예시만 있어 **「어느 상태에서 받는가」와 「계획값은 어떻게 되는가」가 비어 있었다** (#440) |
