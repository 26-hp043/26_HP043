...
| `PATCH /vessels/{vessel_id}` | SCR-002 선박 관리 · 2-8 선박 상세 | 〃. **위치·운항 상태(`PATCH /vessels/{id}/position`)는 현황 입력이라 세 역할 모두** |
| `DELETE /vessels/{vessel_id}` | SCR-002 선박 관리 | 〃 |
| `POST /annual-simulations` | 2-3 연간 등급 관리 | 시뮬레이션 실행은 계획 업무. 결과 조회(`GET`)는 세 역할 모두 |
| `POST /annual-simulations/{simulation_run_id}/reproduce` | 2-3 연간 등급 관리 | 〃 |
| `POST /scenarios/{scenario_id}/adopt` | 2-2 항로 비교 | 채택은 계획 확정 행위. 비교(`compare`)는 세 역할 모두 |
...
| `POST /parameters/import` | SCR-006 파라미터 관리 | **등급 판정 기준 자체를 바꾼다** (`§7.5` · #673). 조회(`GET §7.1~§7.4`)는 세 역할 모두 |
...
>
> **⚠️ 이메일 변경 엔드포인트를 두지 않는다 (#506).** `PATCH /auth/me`는 `display_name`만 받으며 `email`을 보내면 `extra="forbid"`가 **422로 거부**한다. 이메일은 로그인 ID이자 활성 키 `email_active`의 유니크 인덱스 `uq_app_user_email_active`의 키이고(`DB_SCHEMA §2.15` · 061), 새 주소를 잘못 입력하면 **계정에 접근할 수 없다**(재설정 메일도 그 주소로 간다). 주소를 바꾸려면 탈퇴 후 재가입한다 — 탈퇴가 활성 키를 `NULL`로 비우고 유니크 인덱스는 `NULL`을 세지 않아 성립한다(`#1631`). 근거와 고지 문구는 `PRD §6.3`.
>
...
인증 경로의 요청 본문을 한자리에 모은다. **`#1523`의 대조 가드**(`tests/test_api_spec_request_fields_sync.py`)가 이 표들을 서버 스키마와 **양방향으로** 맞춰 보므로, 한쪽만 바뀌면 CI가 실패한다.

열 개 모두 `extra="forbid"`다 — **표에 없는 칸을 보내면 422**다. 오타 필드가 조용히 무시되면 사용자가 의도하지 않은 값으로 계정이 만들어진다.

⚠️ **비밀번호 칸에는 예시 값을 적지 않는다.** 길이·문자 종류 정책은 서버(`auth.password`)가 보며, 스키마는 **빈 값만** 막는다.

#### 1.2.1 `POST /auth/signup` — 가입
...
| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `email` | string | Y | 이메일 형식 · 320자 이하 | — |
| `password` | string | Y | 1자 이상 | — |

> **실패 응답은 사유를 구분하지 않는다** — 「없는 계정」과 「틀린 비밀번호」를 가르면 가입 여부를 캐낼 수 있다(`PRD §6.3` 정본 문구).

#### 1.2.3 `POST /auth/tour-login` — 둘러보기 [#1486]

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `code` | string | N | 200자 이하 (기본 `""`) | 링크에 실린 접근 코드. **이메일도 비밀번호도 받지 않는다** |

> **하한을 두지 않는다.** 공개 둘러보기에서는 화면이 빈 문자열을 보내며, 하한이 있으면 그 요청이 **스키마에서 먼저 잘려** 공개 여부 판정까지 닿지 못한다. 빈 코드가 통과하는 것이 「검사를 건너뛴다」는 뜻은 아니다 — 공개 스위치가 꺼져 있으면 fail-closed로 거절한다.

#### 1.2.4 `POST /auth/password-change` — 비밀번호 변경

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `current_password` | string | Y | 1자 이상 | 본인 확인 |
| `new_password` | string | Y | 1자 이상 | 정책 위반은 서버가 422로 가른다 |

#### 1.2.5 `PATCH /auth/me` — 내 정보 수정
...
| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `role` | string | Y | `OFFICE` · `FIELD` · `ADMIN` 중 하나 | 관리자 전용 경로(`require_admin`). 마지막 관리자는 강등할 수 없다(409) |

> 값이 셋뿐인 것은 `DB_SCHEMA §2.15`가 정한다. 모르는 값을 **여기서 422로 끝내는** 이유는, DB까지 닿으면 트리거(`trg_app_user_role_*`)가 REJECT를 내 **500이 되기** 때문이다.

#### 1.2.7 `POST /auth/verify-email/request` — 인증 메일 재요청

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `email` | string | Y | 이메일 형식 · 320자 이하 | 계정 존재 여부와 무관하게 같은 응답이다 |

#### 1.2.8 `POST /auth/verify-email/confirm` — 인증 확인

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `token` | string | Y | 1~256자 | 메일 링크의 토큰 |

#### 1.2.9 `POST /auth/password-reset/request` — 재설정 메일 요청

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `email` | string | Y | 이메일 형식 · 320자 이하 | 계정 존재 여부와 무관하게 같은 응답이다 |

#### 1.2.10 `POST /auth/password-reset/confirm` — 재설정 확인

| 필드 | 타입 | 필수 | 검증 | 설명 |
|---|---|---|---|---|
| `token` | string | Y | 1~256자 | 메일 링크의 토큰 |
| `password` | string | Y | 1자 이상 | 새 비밀번호 |

> **토큰 실패는 사유를 구분하지 않는다** — 「없음」·「만료」·「이미 사용됨」이 모두 같은 응답이다(위 `§1.2` 각주).
...
### 1.11 요청 본문의 시각 — 시간대 필수 · UTC 정규화 (#1627)

**JSON 본문으로 들어오는 모든 시각은 시간대를 달고 와야 한다**(`Z` 또는 `±HH:MM`). 시간대가 없으면 `422` · `VALIDATION_ERROR`이며, 어느 칸이 문제인지 그 칸 이름으로 알린다.

| 절 | 필드 |
|---|---|
| `§3.3` 항차 생성 · `§3.4` 항차 수정 | `planned_departure_at` · `planned_arrival_at` |
| `§3.6` 항차 실적 입력 | `actual_departure_at` · `actual_arrival_at` |
| `§5.2` 시나리오 채택 | `planned_departure_at` |
| `§6.1` 연간 시뮬레이션 | `as_of` |
| `§2.10`~`§2.13` not under way 구간 | `started_at` · `ended_at` (`#1333` — 이 규칙을 먼저 세운 자리다) |

> **왜 거부하는가.** 시간대 없는 `2026-06-01T09:00`은 **읽는 쪽이 어디냐에 따라 다른 순간**이다. 그대로 받으면 배포 호스트의 시간대 설정이 항차 순서 · 연간 귀속 · `as_of` 경계를 바꾼다 — 같은 요청이 호스트에 따라 다른 답을 내므로 **재현성(`§1.10` 계약 ⑵·⑶)이 서버 설정에 달리게 된다**. 사용자가 고칠 수 있는 입력이므로 500이 아니라 422다(`#1333`과 같은 판단).
>
> **이 규칙은 원래 입구마다 달랐다.** CSV 경로(`§8.2`)는 처음부터 시간대를 요구했고(`#906`), not under way의 JSON 경로는 `#1333`이 맞췄다. 항차·시나리오·연간 시뮬레이션 세 경로만 남아 있었다 — **같은 값을 넣는 두 문이 다른 규칙을 쓰면** 어느 문으로 들어왔는지에 따라 저장된 순간이 달라진다.
>
> **받은 뒤 UTC로 맞춘다.** `+09:00`으로 온 값은 같은 순간의 UTC 표기로 바뀌어 저장·비교·해시에 들어간다(`astimezone(UTC)` — 순간은 바뀌지 않는다). 그래야 `input_hash`와 로그에서 **같은 순간이 언제나 같은 문자열**이 된다.
>
> **응답에는 적용되지 않는다** — 서버가 내보내는 시각은 처음부터 UTC다(`§1.7`).
...
#### `course_deg` — 목적항 방향 (#1804)

| 필드 | 타입 | 설명 |
|---|---|---|
| `course_deg` | string \| null | **현재 위치 → 진행 중 항차 도착항의 초기 방위각**. 0~360(북=0 · 시계 방향), 소수 1자리 |

지도가 배 마커를 돌리는 값이다. ⚠️ **실제 침로가 아니다** — AIS 침로(`cog_deg`)는 수집이 돌지 않고 `PRD §5.2`가 AIS 자동 수집을 범위 밖에 두었다. 화면 표기는 「목적항 방향」이라는 뜻을 드러낸다(`DESIGN_SYSTEM §11` 실측·추정 구분).

> **항로 비교와 같은 함수로 만든다** — `§5.1` 항로 비교의 기상 보정이 같은 두 점으로 입사각을 낸다(`calc/distance.initial_bearing_deg`). 화면이 따로 계산하면 같은 식이 두 곳에 생겨 지도와 항로 비교가 다른 방향을 말할 수 있다. **쿼리는 늘지 않는다** — 현재 위치와 진행 중 항차(`route`와 같은 조회)를 이미 들고 있다.
>
> **`null`인 경우** — 현재 위치(`current_lat/lon`)나 도착항 좌표가 비었거나, 진행 중 항차가 없다. 없는 방향을 지어내지 않는다. 도착항 좌표만 있고 출발항 좌표가 없으면 `route`는 `null`이어도 `course_deg`는 실린다 — 방향에는 출발항이 필요 없다.
...
>
> **[#1629] 동시 요청도 같은 판정을 받는다.** 같은 선박에 겹치는 구간을 **동시에** 올리면 하나만 201이고 나머지는 위 409다 — 서버가 겹침 조회 전에 선박 행을 잠가 직렬화한다(`TECH_SPEC §16.3` · `F-8` 결정). 두 번째 요청은 첫 요청이 끝날 때까지 기다린 뒤 응답을 받으며, 새 오류 코드는 없다. 수정(`§2.11`)·CSV 가져오기(`§8.2`)도 같은 검사를 지난다. 잠금 없이는 둘 다 저장돼 같은 정박의 연료가 두 번 세어졌다(`#1796` 실측).
...
```json
{
  "data": {
    ...
    "year_end_projection": {
      ...
      "drivers": [
        { "key": "CURRENT_VOYAGE", "delta_cii": "…" },
        { "key": "REMAINING_PLAN", "delta_cii": "…" }
      ]
    },
...
「연말 예상이 D」라는 결론만으로는 사용자가 할 수 있는 일이 없다. `drivers[]`는 ⑴ 올해 누적에서 ⑶ 연말 예상까지를 **시간 순으로 하나씩 더한** 단계별 변화다(`PRD §12.3`). 각 원소는 `{ key, delta_cii }`이고 `delta_cii`는 6자리 문자열(`§1.7`)이며 **음수일 수 있다.**

| `key` | 뜻 | 언제 싣는가 |
|---|---|---|
| `BASIS_DIFFERENCE` | ⑶의 조립(확정 항차 + 올해 이미 쓴 정박 몫 + 진행 중 항차의 **경과분**)으로 ⑴을 다시 만든 값과 ⑴의 차이 | 0이 아닐 때만. 두 값이 같은 저장소 함수·같은 절단으로 확정분을 읽으므로 실측에서는 0이다 — 갈리는 날 다른 요인에 녹이지 않고 여기 드러낸다 |
| `CURRENT_VOYAGE` | 진행 중 항차를 ⑴의 **경과분**에서 ⑶이 세는 **계획 전량**으로 바꿨을 때의 변화 — 「이 항해를 마치면」 | 진행 중 항차를 ⑴이나 ⑶이 세고 있을 때. **어느 쪽도 세지 않는 진행 항차**(`EXCLUDE` · 조회 연도 밖)는 제외이고, 확정 거리가 0인데 ⑶이 그 항차를 세지 않으면(계획 연료 없음 · `#812`) 이 단계의 값이 정의되지 않아 생략한다(아래). 경과분이 계획 거리 상한에 닿은 뒤에는 거리는 같고 연료만 다르므로(시계 연료 vs 계획 연료) 음수도 양수도 가능하다 |
| `REMAINING_PLAN` | 남은 계획 항차를 더했을 때의 변화 — 「남은 계획까지 하면」 | 항상. 잔여 계획이 0건이면 `"0.000000"`이고 `PROJECTION_NO_REMAINING_PLAN`이 그 뜻을 말한다 |

- **합은 정확히 `attained_cii − ytd.attained_cii`다** — 문자열 그대로 뺀 값과 같다. 서버가 각 단계의 **누적값을 먼저 6자리로 절사한 뒤** 빼기 때문이다. 차이를 따로 절사하면 단계마다 최대 1 ulp가 버려져 합이 어긋난다. 화면은 차이를 다시 계산하지 않는다(`TECH_SPEC §5.4.1` ⑸).
- **화면이 3자리로 따로 반올림하면 부분 합은 끝자리에서 어긋날 수 있다.** 동치가 성립하는 자릿수는 응답 자릿수(6자리)다. 총량은 서버가 준 두 값(`attained_cii` · `ytd.attained_cii`)을 그대로 보이고, 부분값을 더해 총량을 만들지 않는다.
- 순서는 위 표의 순서로 **고정**이다. CII는 비율이라 요인을 더하는 순서에 따라 값이 달라진다 — 그래서 시간 순서 하나로 못박았다.
- **`CURRENT_VOYAGE`를 만들 수 없는 상태**가 하나 있다 — 확정 거리가 0이고 ⑶이 진행 중 항차를 세지 않을 때(`SIMULATION_PLAN_NO_FUEL`). 「확정분 + 진행 항차 계획 전량」이 거리 0이라 CII가 정의되지 않는다. 그때 `CURRENT_VOYAGE`를 생략하고 `REMAINING_PLAN`이 그 앞 단계(⑶의 조립으로 ⑴을 다시 만든 값)부터 ⑶까지를 잇는다 — 사슬이 끊기지 않아 합은 그대로 `attained_cii − ytd.attained_cii`다. `[]`로 비우지 않는 것은 ⑴·⑶이 둘 다 있는데 「분해할 것이 없다」로 읽히기 때문이다.
- **`ytd.data_available`가 거짓이면 `drivers`는 `[]`다.** 출발점이 없으면 「합 = ⑶ − ⑴」이 성립할 자리가 없다(확정 실적 없이 계획만 있는 선박 — 그때 ⑶ 전체가 계획이다). ⑶ 자체를 못 내면(`data_available: false`) `attained_cii`와 마찬가지로 키가 없다.
- 새 가정이 들어가지 않는다 — 각 단계는 `§6.1`과 같은 엔진을 **읽기 전용으로** 한 번 더 부른 값이다. 스냅샷·`input_hash`는 그대로다.
- 시드 벌크선 50k(`as_of = 적재 기준일 + 3일`): ⑴ `8.213830` → `CURRENT_VOYAGE` `+0.760151` → `REMAINING_PLAN` `−0.008088` → ⑶ `8.965893`. 합 `0.752063`.
...
>
> **[#1026] 이 값을 거짓으로 만드는 것은 연료다.** 근거는 진행 중 항차의 연료·거리가 시계 산출이라는 사실이다(`TECH_SPEC §5.4.1` `VoyageProgress.is_simulated`). 위치가 실측이 되어도(AIS · `#764`) 이 값은 그대로 참이며, 연료가 실측으로 들어오는 경로 — 엔진 센서 연동 — 가 생길 때만 거짓이 될 수 있다. 그 연동은 하지 않는다(`PRD §21` `[#1026]` 각주).
...
| 종류 | 시각 | 값 |
|---|---|---|
| `ACTUAL` | 확정 항차(`INCLUDE_AS_ACTUAL`)의 **도착 시각** `COALESCE(actual_arrival_at, planned_arrival_at)` — 저장소 절단과 같은 식. 둘 다 없으면 누적에는 들지만 자기 점이 없다 · 정박 구간의 **`started_at`** — 저장소 절단 술어(`started_at <= as_of`)와 같은 시각이라 정박 연료가 누적에 들어가는 순간에 점이 찍힌다(종료·진행 중 구간 둘 다) · `as_of` 자신 | 그 시각까지의 누적 |
| `IN_PROGRESS` | 진행 중 항차의 경과분(시계가 만든 **모델값** · `PRD §3.3.8` COR-5)이 든 점 — 보통 `as_of` 점 하나지만, 진행 항차 출항 뒤의 경계(같은 해의 다른 항차 도착 · 정박 시작)에도 붙을 수 있다 | 확정분 + 진행 중 항차의 그 시각까지 경과분. 경과분이 들어 있으면 이 종류다 |
| `PLAN` | **진행 중 항차가 먼저**(도착 예정과 무관), 나머지 잔여 항차(`INCLUDE_AS_PLAN`)는 **도착 예정** 오름차순. 예정이 `as_of`보다 이르면 `as_of`에 붙이고, 예정이 없거나 직전 점보다 이르면 직전 점의 시각을 잇는다 | ⑶과 같은 식 — 확정분 + 앞에서 k건까지의 계획 전량 |
...
>
> **첫 `PLAN` 점 = 진행 중 항차를 계획 전량으로 더한 값**(진행 항차가 있을 때). `§2.14` `year_end_projection.drivers[]`(`#1673`)가 ⑴ → `CURRENT_VOYAGE`(진행 항차의 남은 몫) → `REMAINING_PLAN` 순으로 설명하므로, 첫 `PLAN` 점은 ⑴ + `CURRENT_VOYAGE`와 같다 — 같은 화면의 두 설명이 어긋나지 않는다. 진행 항차 점의 시각은 `max(그 항차 도착 예정, as_of)`이고, 그보다 이른 도착 예정의 잔여 계획은 그 시각을 잇는다.
>
> **예정일이 지난 잔여 항차**(`#1323`)는 `as_of` 점에 붙는다. 곡선이 뒤로 가는 점을 만들지 않는다.
>
> **과거 연도**(`year`가 `as_of`의 해보다 앞)의 `as_of` 점은 `at`을 **그 해 끝**(다음 해 1월 1일 00:00 · 열린 경계)으로 자른다 — 값은 `as_of`로 계산한 그 해 전체 누적이다. 그 해 밖에 점이 놓이면 화면의 축을 벗어난다. `PLAN`·`IN_PROGRESS` 점은 없다(`#815`).
>
...
>
> **`annual_inclusion_policy` 필터는 동작한다** — `#1332`(PR `#1402`)이 라우트에 선언과 값 검증을 더했다. 세 값(`EXCLUDE` · `INCLUDE_AS_PLAN` · `INCLUDE_AS_ACTUAL`) 중 하나로 **정확히 일치하는 항차만** 돌려주고, 생략하면 거르지 않는다. `tests/test_voyage_input_validation_db.py`의 `test_the_inclusion_policy_filter_actually_filters`가 두 값으로 실제 목록이 갈리는지 본다.
>
> **[#1667] 종전 이 자리는 「표에만 있고 구현되지 않았다」로 적혀 있었다.** `#1332`를 고치기 **전** 상태를 적은 문장이 고친 뒤에도 남아, 구현된 필터를 미구현으로 읽히게 했다. 당시 결함은 라우트가 선언하지 않은 쿼리를 FastAPI가 오류 없이 버려, 문서대로 보낸 호출자가 **필터가 걸린 줄 알고 전체 목록**을 받은 것이었다.
>
> **없는 선박·삭제된 선박은 404다** — 종전에는 목록이 돌아와 `GET /vessels/{id}`(404)와 어긋났다.

#### 정렬 — 출항 시각 최신순 [#1806]

1. **출항 시각이 있는 항차가 먼저**, 그 안에서 **최신순**이다. 출항 시각은 **실제 출항, 없으면 계획 출항**이다.
2. **출항 시각이 없는 항차는 맨 아래**다 — 날짜가 없는 초안이라, 맨 위에서 최근 항차를 가리지 않게 한다.
3. 같은 출항 시각끼리는 `created_at` → `id` 내림차순으로 고정한다. 커서가 이 키를 그대로 담아 「더 보기」로 이어 받아도 행이 겹치거나 빠지지 않는다.

> **등록 시각이 아니라 출항 시각인 이유** — 사용자가 이 목록에서 찾는 것은 **가장 최근에 떠난 항차**다. 종전 순서(`created_at` 오름차순)는 오래된 항차부터 보여 진행 중 항차가 「더 보기」 뒤로 밀렸고, `created_at` 내림차순으로만 뒤집어도 **CSV 가져오기로 지난 항차를 몰아 넣거나 지난달 항차를 나중에 등록하면** 항해 순서와 갈린다. 네 시각 칸이 모두 NULL을 허용하므로(`DB_SCHEMA §2.2`) 2항이 필요하다.
>
> **정렬이 바뀌기 전에 받은 커서는 422다**(`VALIDATION_ERROR` · 「커서 형식이 올바르지 않습니다」). 다른 정렬의 위치라 이어 받으면 겹치거나 빠지므로, 첫 페이지부터 다시 받는다. `§8.1` CSV 내보내기도 같은 순서다.
...
> **[#1626] 같은 항차에 동시에 들어온 쓰기는 항차 행 잠금으로 줄을 선다.** 전환 · `§3.4` PATCH · `§3.6` 실적 입력 · `§3.7` 삭제 · `§5.2` 채택이 같은 잠금을 쓴다(`TECH_SPEC §16.3`). 뒤 요청은 앞 요청이 커밋할 때까지 **기다렸다가 바뀐 상태로 판정**된다 — 앞 요청이 항차를 종결하면 뒤의 전환은 이 표의 규칙대로 `422 STATE_TRANSITION_ERROR`를 받는다. **새 오류 코드·재시도 계약은 없다.** 대기 상한이 없어(`lock_timeout=-1` — 서버 기본값 전제이며, 이 전제를 잠그는 가드는 `#1631`(`test_db_isolation_premise_db.py`)에 있다) 잠금 대기가 오류로 나오지 않으며, 클라이언트는 422를 「항차를 다시 읽으라」로 처리하면 된다(`§1.4`). 종전에는 두 요청이 같은 옛 상태를 읽고 둘 다 통과해 **마지막 쓰기가 이겼다**(`#1796` ⑹ 실측).
...
### 3.11 공개 해상 경로망 위의 바닷길 (#1300)

```http
GET /api/v1/ports/sea-route?from_lat=35.1&from_lon=129.0333&to_lat=1.2833&to_lon=103.85
GET /api/v1/ports/sea-route?from_lat=35.1&from_lon=129.0333&via_lat=21.3&via_lon=-157.87&to_lat=33.7&to_lon=-118.2
```

두 점 사이의 **바닷길**을 좌표 목록으로 준다 — `PRD §5.2` 「지도 기반 항로 렌더링」(2026-09-22 `#1300` E-6). 선대 대시보드와 항로 비교 지도가 종전 대권선 대신 이 선을 그린다. 경로망은 서버의 `searoute` 패키지(Apache-2.0)가 번들한 **Eurostat SeaRoute 해상 경로망**(EUPL-1.2 · ORNL *Global Shipping Lane Network 2000* 기반)이며 **오프라인·결정론**이다 — 외부 호출이 없고 같은 입력은 같은 선이다.

| 쿼리 | 설명 |
|---|---|
| `from_lat` · `from_lon` · `to_lat` · `to_lon` | 필수. 범위 밖이면 422 `VALIDATION_ERROR`(VAL-007) |
| `via_lat` · `via_lon` | 선택. **함께**만 유효하다 — 한쪽만 주면 422(`§5.1` 좌표 쌍과 같은 규칙). 있으면 「출발 → 경유지 → 도착」을 잇는다(항로 비교의 우회 경유지 · `§5.1` `detour_waypoint_*`) |

**경로망이 두 점을 잇지 못하면 404 `NOT_FOUND`**(`§3.10`과 같은 갈래 · 통과 제한으로 막힌 바다 등). 라이브러리는 그때 경고와 함께 **두 점을 잇는 직선**을 돌려주는데 그 직선은 바닷길이 아니라 서버가 내보내지 않는다.

```json
{
  "data": {
    "coordinates": [[129.0333, 35.1], [129.2, 35.0], [128.916321, 34.761923], "…", [103.85, 1.2833]],
    "length_nm": 2552.14,
    "legs": 1,
...
| 필드 | 설명 |
|---|---|
| `coordinates` | GeoJSON 순서 `[경도, 위도]`. **양 끝은 입력 좌표 그대로**다(마커와 선 끝이 떨어지지 않게). 날짜변경선을 넘는 구간은 경도를 `±180` 밖으로 이어 적는다(지도가 허용한다 — 되돌아가는 선을 막는다) |
| `length_nm` | 선의 길이(해리, 소수 2자리). **표시용 참고값**이지 계산 거리가 아니다 — `§3.9`처럼 JSON 숫자다 |
| `legs` | 이은 구간 수. 경유지가 없으면 1, 있으면 2 |
| `source` | 출처 표기 문자열 `searoute/marnet` |

> ⚠️ **표시용이다 — CII 계산 거리는 바꾸지 않는다.** 계산 거리는 사용자 입력 또는 `§3.9` 대권거리다(`PRD §15.2` · 재현성 계약 `TECH_SPEC §5.4`). 이 선의 길이가 표의 거리와 다른 것은 정상이며, 화면은 이 값을 거리로 쓰지 않는다. 그래서 재현성 해시 재료도 아니다.
>
...
>
> **[#1626] 같은 항차에 동시에 들어온 두 채택은 항차 행 잠금으로 줄을 서며 나중 채택이 남는다** — 단일 요청에서 다시 채택할 때와 같다. 채택 행은 항차당 하나다(`DB_SCHEMA §2.4`). 잠금 없이는 두 요청이 모두 「이전 채택 없음」을 보고 **두 행이 남았다**(`#1796` ⑺ 실측). 대기·오류 계약은 `§3.5` 각주와 같다.
...
> **`type=voyages`의 행 순서는 `§3.1` 목록과 같다**(출항 시각 최신순 · 출항 시각 없는 항차는 맨 아래 · `#1806`) — 화면 순서와 파일 순서가 갈리지 않는다.
...
> 이 엔드포인트는 내부 디버깅용이다. **조회(`GET /weather/snapshot`)는 로그인한 세 역할 모두 호출할 수 있고**(역할 가드가 걸려 있지 않다 — 아래 `#767` 판정), 수동 갱신 경로는 열지 않았다. 「내부용」은 **화면에서 링크하지 않는다**는 뜻이다.

> ⚠️ **[#1663] 아래 판정의 「역할 구분이 없다」는 2026-09-12 당시 사실이다 (2026-09-23).** 사흘 뒤 `#672`가 역할 2종을, `#1301`이 관리자를 도입해 **지금은 역할 구분이 있다**(`§1.2` 두 표). 다만 **판정 자체는 그대로 유효하다** — `GET /weather/snapshot`에는 역할 가드가 없어 세 역할 모두 조회하며, 근거는 「값이 선박·항차와 달리 특정 역할의 업무 산출물이 아니다」로 바뀐다.
>
...
> **`commit`** — 지금 떠 있는 이미지를 빌드한 커밋(12자리) (`#789`). 배포 빌드가 `--build-arg GIT_SHA`로 넘기고, 배포 워크플로의 「헬스 체크」가 **이 값이 배포한 커밋과 같을 때만** 배포를 성공으로 친다 — 이전 컨테이너가 아직 응답하는 동안은 기다린다. **로컬 개발처럼 값이 없으면 `null`**(알 수 없음)이며 빈 문자열을 내지 않는다 — 빈 값끼리 「같다」가 되지 않게.
...
>
> **[#1483] 프록시 뒤의 「클라이언트 IP」는 서명 헤더로 식별한다.** 클라우드에서는 요청이 Pages Function 프록시와 터널을 거쳐 들어와 소켓 상대가 전원 같았다 — `auth` 10/분을 현장 전원이 나눠 썼다. 이제 프록시가 원 IP를 `X-BlueLog-Client-IP`에 싣고 비밀 값(`X-BlueLog-Proxy-Secret`)을 붙이며, 서버는 **비밀 값이 맞을 때만** 그 IP로 센다. 없거나 틀리면 종전대로 소켓 상대 IP다. 두 헤더는 **프록시와 서버 사이의 내부 헤더**이며 클라이언트가 보내는 요청 계약이 아니다 — 브라우저가 보낸 같은 이름은 프록시가 뗀다. 한도 단위가 IP라는 점과 표의 수치는 그대로다 — 다만 **IPv6는 `/64` 대역을 한 IP로** 센다(한 가입자가 대역 안에서 주소를 바꿔 한도를 피하지 못하게). 구성은 `OPERATIONS §1.2.1`.
...
| `calculation_run_id` | UUID | — | 화면이 **방금 낸 계산 결과**의 실행 id (#1533). 항차 CII 추정(§2.10)·시나리오 비교(§5.1)·연간 시뮬레이션(§6.1) 응답의 `calculation_run_id`를 그대로 넣는다. 도구 `explain_screen_result`가 이 실행의 **저장된 결과를 읽는다** — 다시 계산하지 않는다. **외부 모델로는 나가지 않는다** |
...
`calculation_run_id`가 오면 모델은 도구 `explain_screen_result`로 그 실행의 **저장된 결과**를 읽는다. 다른 계산 도구는 그 자리에서 **새로 계산**하므로 입력이 조금만 달라도 화면과 다른 수가 나오고, 확률(몬테카를로)은 다시 돌려서는 화면과 같아질 수 없다. 응답의 면책 문구 「화면의 계산 결과를 풀어 쓴 것」(`PRD §6.3`)이 참이 되는 경로가 이것이다.

| 실행 종류 | 모델에게 가는 값 (`PRD §16.3.1`) |
|---|---|
| `VOYAGE_ESTIMATE` | `attained_cii` · `required_cii` · `rating` · `risk_level` · `next_boundary_gap` |
| `SCENARIO` | 시나리오마다 위와 같은 묶음 — **저장 순서 그대로**(순위로 정렬하지 않는다) |
| `ANNUAL_MONTE_CARLO` | 연말 예상 `attained_cii` · `rating`, `risk_level`, `target_rating`, `target_success_probability`, `rating_probabilities` |

- **상단 선박과 실행의 선박이 다르면 읽지 않는다** — 오류 봉투「화면의 계산 결과와 상단에서 고른 선박이 다릅니다」. 맞는 수로 틀린 배를 설명하게 되기 때문이다. 상단 선박이 없으면 따지지 않는다
- 없는 실행이면 오류 봉투(고정 문구)다. **요청은 422가 아니다** — 결과를 못 읽어도 질문 자체는 답할 수 있다
- 확률·비율은 화면 표기(소수 1자리 백분율)를 덧붙여 보낸다 — `"0.8123 (81.2%)"`. 모델이 화면과 같은 표기로 말해도 수치 검증(`§15.2`)에 걸리지 않게 한다
- 도구 `lookup_regulation`은 **규제 기준값 표**(기준선 · 등급 경계 · 감축률 · 연료 CF)를 각 행의 `source_ref`와 함께 읽는다 (#1703). 값은 `§7` 조회와 같은 서비스에서 오며 계산하지 않는다. 선종을 말하지 않으면 이 대화의 선박 선종을 쓰고, 모르는 선종·연료는 고정 문구 오류 봉투다. 표에 없는 연도의 감축률은 `null`이다 — 가까운 해의 값을 대신 주지 않는다
- 도구 `project_year_end`는 연말 예상(`year_end_projection`)과 함께 **올해 누적(`ytd`)** 을 낸다 — 실시간 CII 화면이 크게 보여 주는 값이다. 결정론 계산이라 다시 불러도 화면과 같다
...
| **만료된 대화** `session_id`(생성 + 90일 경과 · 청소 전) | 404 | `NOT_FOUND` (`#1632`). 남의 대화와 **같은 코드**다 — 다른 코드를 두면 「있었지만 만료됐다」가 새어 존재 여부를 알린다. **외부 모델 호출과 질문 저장이 모두 없다.** 경계(`expires_at` = 지금)는 만료다 — 청소(`purge_expired`)의 조건과 같다. 대화 삭제(`§15.6`)는 만료 대화에도 그대로 동작한다(지우는 요청을 막을 이유가 없다) |
...
| 2026-09-22 | `#1582` | **§1.2 「같은 소요시간」 보증을 `POST /auth/login` 하나로 좁혔다** (`#1405`). 종전 문장은 `password-reset/request`에도 같은 소요시간을 적었으나 **코드에 균등화가 없어 거짓이었다** — 없는 계정은 곧바로 200, 있는 계정은 토큰 발급·SMTP 발송을 거친다. `verify-email/request`도 같다. 맞추려면 발송을 응답 뒤로 미뤄야 하고 그러면 `#1326`의 발송 실패 고지(500)가 사라진다. 각주 `[#1405]`에 기각한 안(백그라운드 발송 · 응답 시간 바닥)과 이유를 남겼고, 로그인 균등화는 `test_auth_api.py`가 호출 단언으로 잠근다. `§4.3`상 정정이라 버전은 올리지 않는다 |
| 2026-09-22 | `#1585` | **§2.14 `meta.simulated`에 「이 값을 거짓으로 만드는 것은 연료다」 한 문단** (`#1026`). 위치가 실측이 되어도(AIS · `#764`) 값은 참으로 남고, 연료가 실측으로 들어오는 경로(엔진 센서 — `PRD §21` `[#1026]`, 하지 않음)가 생길 때만 거짓이 될 수 있다. `§4.3`상 각주 보강이라 버전은 올리지 않는다 |
| 2026-09-22 | `#1590` | **§1.7 각주 `[#1349]`** — CII 문자열의 마지막 자리는 절사값이다(`TECH_SPEC §1.2.1` 「응답 직렬화의 절사」). 클라이언트는 받은 문자열을 그대로 표시 자릿수로 반올림한다. 전송=표시 필드와 Layer 2 4자리는 `ROUND_HALF_UP` 그대로. `§4.3`상 각주 보강이라 버전은 올리지 않는다 |
...
| 2026-09-23 | `#1743` | **v1.43 — §1.2.1~§1.2.10 인증 요청 표 10종 신설.** `#1523`이 만든 요청 필드 대조 가드(`tests/test_api_spec_request_fields_sync.py`)가 **인증만 대조 밖**에 두고 있었다 — 표가 아예 없어 `NO_TABLE_YET`에 담겨 있었고, 그 자리는 **화면이 가장 자주 부르는 경로**(가입·로그인·재설정)다. 열 개 모두 필드·타입·필수·검증·설명 표로 적어 `TABLES`로 옮기고 **`NO_TABLE_YET`을 비웠다.** 표를 만들면서 스키마·정본에 흩어져 있던 규칙을 같은 자리에 모았다 — `extra="forbid"`(표에 없는 칸은 422) · 둘러보기 코드에 **하한을 두지 않는 이유**(공개 둘러보기가 빈 문자열을 보내므로 하한이 있으면 판정까지 닿지 못한다) · `PATCH /auth/me`가 **`email`을 받지 않는 이유**(로그인 ID이자 재설정 메일 주소라 잘못 넣으면 계정에 닿을 수 없다) · 역할 값을 **여기서 422로 끝내는 이유**(DB까지 닿으면 트리거 REJECT로 500이 된다). 비밀번호 칸에는 예시 값을 적지 않았다. 돌연변이 2종(필드명 변경 · 필수 표기 뒤집기) **2/2 검출**. `AGENTS §4.3`상 절 신설이라 버전을 올린다 (#1602) |
...
| 2026-09-23 | `#1830` | **v1.45 — §2.18 「올해 누적 CII 추이 조회」 신설** · §12 요약표 행. 실시간 CII 화면이 지도 아래에 누적 CII 추이(연초~오늘 실적 · 오늘~연말 계획 · 등급 밴드)를 그리는데 `§2.14`는 한 시점의 값만 줘 그릴 데이터가 없었다(재설계안 R-D1). 점 하나 = 항차 경계 하나(확정 항차 도착 · 정박 구간 종료 · `as_of` · 잔여 항차 도착 예정). 계산식을 새로 만들지 않는다 — 실적 점은 `§2.14` ⑴과 같은 조립(`resolve_ytd_at`)을 시각만 바꿔 부르고, 계획 점은 ⑶과 같은 입력·엔진에 잔여 항차를 도착 예정 순으로 한 건씩 더한다. 그래서 실적 마지막 점 = `ytd`, 계획 마지막 점 = `year_end_projection`(데모 벌크선 실측 8.213830 · 8.965893). 곡선은 페이지 진입 때 한 번, 60초 틱은 `/cii/current`만. 절 신설이라 `AGENTS §4.3`에 따라 버전을 올린다 (#1671) |
| 2026-09-24 | `#1852` | §2.10 겹침 각주에 **「동시 요청도 같은 판정을 받는다」** 문단 추가 (`#1629` · `F-8` 결정). 같은 선박에 겹치는 구간을 동시에 올리면 하나만 201이고 나머지는 기존 409다 — 서버가 겹침 조회 전에 선박 행을 잠가 직렬화하며(`TECH_SPEC §16.3`) 새 오류 코드·계약 변경은 없다. 수정(`§2.11`)·CSV 가져오기(`§8.2`)도 같은 검사를 지난다. `AGENTS §4.3`상 각주 보강이라 버전은 올리지 않는다 (#1629) |
...
| 2026-09-26 | `#___` | §2.16 데이터 점검에 **다섯째 심각도 `PUBLIC_RECORD`(공적 기록과 다름)** — `summary.public_record_count` · `issues[].public_record`(출처 · 받은 시각 · 어긋남 목록) · 코드 `PUBLIC_RECORD:<칸>` 넷. 판정은 `PRD §17.4.4`(6시간 초과 · 48시간 안의 가장 가까운 기항 · 완결성 제외). 기존 필드는 그대로이고 다른 행의 `public_record`는 `null`이다. `§4.3`상 행·필드 추가라 버전은 올리지 않는다 (#1197) |
