...
| 2026-08-17 | PR #NNN | **v1.16 — §2.8 `unavailable_reason` 신설 (`#419`).** `data_available=false`인 선박이 **왜** 그런지 4종(`NO_DATA` · `MISSING_SPEC` · `NO_PARAMETERS` · `CALCULATION_ERROR`)으로 구분한다. 사용자가 할 일이 서로 다르므로(항차 등록 · 제원 입력 · 운영자 문의) 한 사유로 뭉치면 화면이 안내할 수 없다. 한 척의 계산 실패가 요청 전체를 500으로 만들지 않는다는 계약과, **연도 파라미터 부재는 여전히 409**라는 경계를 함께 명시 (#419) |
