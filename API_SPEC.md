...
| 둘러보기 | **[#1486] 링크에 실린 코드로 여는 관리자 열람 세션 — 인터뷰·시연용.** 설정 `TOUR_ACCESS_CODE` 하나로 잠긴다. **fail-closed — 비어 있으면 항상 거절한다.** 위 「가입 제한」의 판정(둘 다 비면 열림)과 **반대 방향**이다 — 그 패턴을 쓰면 변수 미설정 배포에서 **누구나 관리자가 된다.** 세션은 고정 스텁 계정(`ADMIN`)으로 발급되며 일반 세션처럼 **7일** 산다 — 코드를 비워도 **이미 발급된 세션은 살아 있다**(`user_session` 행이 별개). 둘러보기 세션에 별도 권한 가드는 두지 않는다(`#1486` 2026-09-21 결정 — `PRD §5.2`가 역할 세분화를 막는다). 거절은 `422 VALIDATION_ERROR` · `PRD §6.3` 「둘러보기 — 거절」 문구 — **꺼져 있음과 불일치를 말하지 않는다**(가입 제한과 같은 원칙) |
...
| `POST /auth/tour-login` | 인증 플로우 자체 — 링크에 실린 코드가 자격 증명이다 (`#1486`). **환경과 무관하게 항상 등록**되므로 `dev-login`과 달리 공개 경로 목록의 **기본 집합**(`auth/dependencies.py` `_BASE_PUBLIC_PATHS`)에 든다 |
...
| `POST` | `/auth/tour-login` | 불필요 | **둘러보기** (`#1486`) — 본문 `{"code": "<링크에 실린 코드>"}`를 설정 `TOUR_ACCESS_CODE`와 상수 시간 비교(`hmac.compare_digest` · 초대 코드와 같은 패턴) → 고정 스텁 계정(**관리자 `ADMIN`** · 고정 UUID·고정 이메일 · Argon2 형식이 아닌 해시라 `POST /auth/login`으로는 열리지 않는다)으로 세션 발급. **성공 응답은 `POST /auth/login`과 같은 형태**(200 · `data` 사용자 객체 · `sid`·`csrf` 쿠키). 거절은 **`422 VALIDATION_ERROR` 한 가지** — 코드 미설정(fail-closed)과 불일치를 **가르지 않는다**, 문구는 `PRD §6.3` 「둘러보기 — 거절」. 요청 한도는 인증 API 버킷(`§13.2` 분당 10회) |
...
>
> ⚠️ **인증 메일 재발송(`/auth/verify-email/request`)의 소요시간은 아직 같지 않다.** 계정이 없거나 이미 인증된 경우 곧바로 돌아오고, 그 밖에는 토큰 발급 + 메일 발송을 거친다. 이 항목은 `#1326`에서 **분리**했다 — 발송을 백그라운드로 옮기면 **발송 실패를 사용자에게 알릴 수 없어** 위 상태 코드 정리가 무의미해지기 때문이다. 자세한 사유와 후속은 `#1405`에 적혀 있다 — ⚠️ **계정 존재 자체는 `POST /auth/signup`이 이미 알린다**(`§2`의 409 · `PRD §6.3` **비대칭 규정**)는 점도 함께 따졌다.
>
...
| POST | `/api/v1/auth/tour-login` | 둘러보기 로그인 — 링크 코드로 여는 관리자 세션 (**항상 등록 · `TOUR_ACCESS_CODE`로 잠김** · #1486) | §1.2 |
