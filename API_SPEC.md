...
| 2026-09-25 | `#___` | §13.2 「/ 사용자」 각주에 **`[#1483]` 프록시 뒤 클라이언트 IP 식별** — Pages Function이 원 IP를 `X-BlueLog-Client-IP`에 싣고 비밀 값을 붙이며, 서버는 비밀 값이 맞을 때만 그 IP로 센다. 내부 헤더이며 클라이언트 요청 계약이 아니다. `§4.3`상 각주 보강이라 버전은 올리지 않는다 (#1483) |
