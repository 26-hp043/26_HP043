...
| 2026-09-23 | `#___` | §2.8 `vessels[]`에 **`course_deg`(목적항 방향)** 필드와 절. 현재 위치 → 진행 중 항차 도착항의 초기 방위각을 항로 비교와 같은 함수(`calc/distance.initial_bearing_deg`)로 서버가 준다 — 화면이 따로 계산하면 지도와 항로 비교가 다른 방향을 말할 수 있다. **실제 침로(AIS `cog_deg`)가 아니다.** 쿼리는 늘지 않는다. 디자인 요청 R-D4(`#1672`). `AGENTS §4.3`상 필드 추가라 버전은 올리지 않는다 (#1804) |
