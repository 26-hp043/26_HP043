...
| 2026-09-25 | `#___` | §2.8 선대 요약 `route`에 **`departure_port_name` · `arrival_port_name`** 추가 (`#1882`). 지도 마커 표(`DESIGN_SYSTEM §9.5`)의 **항구 핀**이 두 끝에 그려지며 이름을 읽어 준다 — 좌표와 같은 항차에서 오므로 화면이 항구 표를 새로 두지 않는다. 필드 추가라 기존 소비자는 그대로다. `§4.3`상 행 추가라 버전은 올리지 않는다 (#1882) |
