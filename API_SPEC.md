...
| 401 Unauthorized | `INVALID_CREDENTIALS` | 자격 증명 불일치 — 로그인 실패(없는 계정·비밀번호 오류), 비밀번호 변경 시 현재 비밀번호 오류. 세션 문제(`UNAUTHORIZED`)와 **같은 401, 다른 code**로 가른다. 클라이언트가 문구 대조 없이 사유를 판별하게 한다 (#902). 계정 존재 비노출 규칙(§1.2)은 문구·응답 시간으로 지키므로 이 코드는 존재 여부를 말하지 않는다 |
...
>
> **[#776 정정] 이 플래그를 표시하는 화면은 없다.** 종전 문장은 *"화면은 이 플래그로 재계산 권고를 표시한다"* 였으나 그런 화면이 존재한 적이 없다 — 이 엔드포인트는 화면에 연결하지 않는 것으로 판정됐다(`#556` · `#776` 재판정, 근거는 `routes/calculations.py` docstring). `PRD §8.4`의 「재계산 필요 표시」는 이 플래그를 세우는 처리이며 화면 표시를 규정하지 않는다. 조회 용도는 재현성 검증·감사다.
...
> **[#756] 거리 두 행이 기준값(`5.02`)과 같은 것은 오기가 아니다.** 거리 ±5%는 연료를 같은 비율로 함께 움직여 CII가 거의 변하지 않는다(`PRD §12.6` 각주). 종전 예시는 `4.96`·`5.08`로 **구현이 낼 수 없는 변화**를 싣고 있었다.
>
...
> ## ⏸ 이 절의 2종은 **아직 구현하지 않았다** (`#591` 판정 · 2026-08-23)
>
> **명세를 남긴다.** 세 안(구현한다 · 스펙에서 뺀다 · 미구현을 명시하고 남긴다) 중 셋째다.
>
> | 왜 지우지 않는가 | |
> |---|---|
> | **기능은 이미 있다** | 빠진 것은 **HTTP 노출뿐**이다. 조회 어댑터(`weather/open_meteo.py`)·저장·fallback 체인(`services/weather.py`)이 다 있고 시나리오 비교(`§5.1`)가 실제로 쓴다 |
> | **지우면 의도까지 사라진다** | 「기상 fallback을 바깥에서 들여다볼 수단을 두려 했다」는 판단이 문서에서 없어진다. 나중에 필요해지면 **없었던 것**이 된다 |
> | **사용자 기능이 아니다** | 이 절이 스스로 「내부」로 성격을 밝히고 있어 MVP 화면 어디에도 소비처가 없다 |
>
> **구현하려면 인가 설계가 선행한다.** 내부 디버깅용이므로 누가 부를 수 있는지를 먼저 정해야 하고, 그것은 어드민 범위(`#359`)에 걸린다. `#359`는 **1차 시연 범위 밖**으로 판정됐다(`PRD` O-14 각주).
>
> 이 판정이 낡지 않게 **`§12` 요약표와 실제 라우트를 대조하는 가드**(`tests/test_api_spec_endpoints_sync.py`)가 CI에서 돈다 — 누군가 구현하면 그 테스트가 깨지고, 이 표시를 지우게 한다.
...
| GET | `/api/v1/weather/snapshot` | 기상 스냅샷 (내부) (**미구현 — `§9` 판정 `#591`**) | §15.3 |
| POST | `/api/v1/weather/refresh` | 기상 수동 갱신 (내부) (**미구현 — `§9` 판정 `#591`**) | §15.3 |
...
**세 버킷은 서로 독립이다.** 카운터가 `(버킷, 클라이언트)`별로 따로 돌므로, 인증 한도를 다 써도 계산·CRUD는 막히지 않는다. 한 클라이언트의 분당 상한은 세 값의 합이며 어느 한 행의 값이 아니다.
