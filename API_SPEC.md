...
| 2026-09-25 | `#___` | §10 헬스 응답에 **`commit`**(빌드 커밋 12자리 · 없으면 `null`) 추가 (`#789` · `#1177`에서 옮겨 온 잔여 항목). 운영이 어느 커밋으로 떠 있는지 밖에서 알 수 없어 `#1177` 판정도 동작으로 거꾸로 추정했다. 배포 확인이 이 값과 배포한 커밋을 대조한다. 필드 추가라 기존 소비자는 그대로다. `§4.3`상 행 추가라 버전은 올리지 않는다 (#789) |
