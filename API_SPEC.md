...
| 우선순위 | 출처 | 비고 |
|---|---|---|
| 1 | **요청 `vessel_id`** | 화면이 「지금 보고 있는 선박」 — 항상 가장 최신이다. 넘어오면 **세션 귀속도 이 값으로 맞춘다** |
| 2 | **`chat_session.vessel_id`(세션 귀속)** | `search_vessel`의 **고유 일치**(1척)가 저장한다. 둘 이상이 걸리면 저장하지 않는다 — 애매한 것을 몰래 고르면 사용자가 고른 것이 아닌 배로 답한다 |
| — | 없음 | 계산 도구는 「어느 선박인지 먼저 정해야 합니다」 오류 봉투를 낸다 |

- 세션 귀속이 요청값을 **덮어쓰지 않는다**(우선순위 1이 이기고, 검색은 요청값이 있는 턴에 저장하지 않는다)
