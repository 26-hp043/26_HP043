...
| 2026-08-06 | `#165` | v1.4: §7 이슈 연결에 `Closes`/`Refs`/`Part of` 구분 신설, 「1 PR = 하나의 주 작업 이슈」로 정확화, PR 본문 템플릿을 최소 요건 4절 + 선택 절로 규정, §7.1 정본 변경 PR·§7.2 승인 시점 신설. **종전 §7.1(팀원 기여 기록)은 §7.3으로 이동** (#165) |
