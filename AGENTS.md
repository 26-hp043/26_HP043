...
**디자인 토큰은 `DESIGN_SYSTEM.md`가 최종 권위이며, 위 순위와 교차하지 않는다.** 토큰의 이름·의미·제약을 다루고 계산·데이터 구조를 정하지 않으므로 위 사슬과 충돌할 지점이 없다. 토큰 **값**(hex·radius·그림자 수치)의 소유는 Figma이며, `DESIGN_SYSTEM.md` §0.2의 제약을 위반할 수 없다.
