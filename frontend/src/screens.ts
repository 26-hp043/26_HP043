...
export interface ScreenMeta {
  ...
  implemented: boolean
  /**
   * 사무직 전용 화면인가 (`UIFLOW §2.2` 역할 열 · `API_SPEC §1.2` · `#672` · `#1301`).
   *
   ...
   *
   * **관리자도 통과한다** — `isOffice()`가 ADMIN을 참으로 보므로(`#1301`) 이 뱃지·가드는
   * 실질적으로 「현장직 전용 제외」다. 이름은 「사무직 전용」 그대로 둔다 — 계정 관리를
   * 뺀 업무 13경로 전체가 여전히 사무직의 몫이고, 관리자는 그 위에 얹힌 것뿐이다.
   */
  officeOnly?: boolean
}
 ...
 */
/**
 * 사이드바 순서 — `UIFLOW §2.2.1`이 소유한다.
 *
 * **계층 순서가 아니라 작업 흐름 순서다.** 종전에는 `§2.2` 계층 매핑 표의 행
 * 순서(선대 → 선박 → 항차 → 산출물)를 그대로 썼는데, 그 표에는 순서를 정한
 * 문장이 없었다 — 한 표가 「어느 계층인가」와 「어떤 차례로 놓는가」를 겸하고
 * 있었고, **둘은 같은 것이 아니다.** `#712`가 `§2.2.1`을 신설해 갈랐다.
 *
 * **선박 관리가 둘째인 것이 이 순서의 근거다.** 제원이 비어 있으면 CII 계산도
 * 항로 비교도 실행되지 않는다(`PRD §6.1`) — 사실상 시작점인데 종전에는
 * 여섯째였다.
 *
 * 순서를 바꾸려면 `§2.2.1`을 먼저 고친다. 여기서만 바꾸면 정본과 갈린다.
 */
export const NAV_ORDER = [
  'MAINBOARD',        // 1. 선대 전체를 먼저 본다
  'VESSEL_MANAGEMENT',// 2. 제원이 없으면 뒤가 전부 안 돈다 (PRD §6.1 SCR-002)
