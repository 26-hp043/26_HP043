...
export const WARNING_MESSAGE: Readonly<Record<string, string>> = {
  ...
  // `#798` — 「예측이 없다」가 아니라 「더할 계획이 없다」를 말한다. 값은 나오는데
  // ⑴과 같으므로, 이유를 말하지 않으면 종전 결함(항상 ⑴과 같음)과 구분되지 않는다.
  ...
  // `#363` — 보정계수를 켰는데 표본이 모자라 **곱하지 않았다.** 조용히 넘기면 켠 대로
  // 계산된 줄 안다.
  FEEDBACK_FACTOR_UNAVAILABLE:
    '실적 보정계수를 켰지만 확정 항차가 모자라 적용하지 않았습니다. 이번 결과는 계획 연료 그대로 계산했습니다.',
  // `#513` — 함대 감축 계획에서 제원이 없어 감속을 적용하지 못한 항차가 있다.
