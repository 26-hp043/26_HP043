...
export interface FixedParameters {
  ...
  year: number
  /**
   * 고정값 — 계산하지 않는다. **`#45` 정본 픽스처의 30자리 값 그대로다.**
   * 응답에는 6자리로 형식화해 싣는다(`#132` 계약의 `"5.045066"`).
   */
  requiredCii: string
  /**
   * 고정값 — 계산하지 않는다. **`#45` 정본 픽스처(`tests/fixtures/cii/
   * bulk_50000_hfo_2026.json`)의 30자리 값을 그대로 옮겼다.**
   *
   *   A/B superior = required × 0.86     B/C lower    = required × 0.94
   *   C/D upper    = required × 1.06     D/E inferior = required × 1.18
   *
   * `upper`는 `#132` 계약의 `next_worse_boundary_margin = 0.365370`과 교차 검증된다
   * (`5.347770312… − 4.9824 = 0.365370312…` → 6자리 `0.365370`).
   *
   * ## 종전 값이 왜 바뀌었나
   *
   * 처음에는 `PRD §13.1`의 소수 10자리 표기를 전사했는데, **그 표기 자체가
   * 원값과 10번째 자리에서 달랐다** — `superior`가 원값 `4.3387570459…`인데
   * 표기는 `4.3387570460`이었다. `#166`(PR #180)이 `§13.1`을 소수 9자리로 통일해
   * 이 문제를 정리했고, 이제 두 표기가 모두 `4.338757046`이다.
   *
   * **응답은 6자리로 형식화하므로 화면 숫자는 바뀌지 않는다.** 다만 고정표가 알려진
   * 오류값을 들고 있을 이유가 없어 정본 30자리로 교체했다.
   *
   * ## `TEST_PLAN`과의 불일치는 해소됐다
   *
   * 종전 주석은 *"`TEST_PLAN §1.2`가 다른 값을 쓰며 `#38`이 다룰 범위"* 라고 적고
   * 있었다. **`#166`이 그 불일치를 해소했고 `TEST_PLAN`은 이제 `PRD`와 같다.**
   * 위 값들이 곧 `TEST_PLAN §1.2`의 값이다.
   */
...
export const FIXED_PARAMETERS: readonly FixedParameters[] = [
  {
    ...
    year: 2026,
    requiredCii: '5.04506633249618206053073653978',
    boundaries: {
