...
/**
 * 두 폼 상태가 같은 입력인가 — 결과가 아직 유효한지 판단한다 (`#727`).
 *
 * 계산 후 사용자가 입력을 고쳐도 결과 카드는 그대로 남는다. 그 상태에서 화면은
 * **옛 입력으로 낸 숫자를 현재 조건의 답처럼** 보여 준다. 시연에서 값만 만지고
 * 넘어가면 그 숫자를 그대로 읽게 되므로, 어긋난 상태를 화면이 말해야 한다.
 *
 * **속력이 결과를 바꾸지 않는다는 것을 알면서도 비교에 넣는다.** 빼면 「속력만
 * 고쳤을 때는 다시 계산할 필요가 없다」가 참이 되지만, 사용자는 그 예외를 모른 채
 * 「입력을 고쳤는데 아무 말이 없다」로 읽는다. 다시 계산하라는 안내는 불필요할 때
 * 나와도 틀리지 않지만, 필요할 때 빠지면 틀린 숫자를 읽게 된다.
 *
