...
export function PositionChart({ vessels, minSpan = MIN_SPAN }: PositionChartProps) {
   ...
   * ## 위도와 경도를 **같은 배율**로 놓는다
   *
   * 종전에는 두 축을 각각 늘려 상자를 채웠다. 시연 화면이 위도 4.5° · 경도 96.7°
   * 범위였는데 같은 폭에 펴 놓으니 **위도가 21배 부풀려져** 있었다 — 「서로 얼마나
   * 떨어져 있나」를 보여 준다고 적어 놓고 그 거리를 왜곡하고 있었다.
   *
   * 육지를 그리려면 어차피 균등 배율이어야 한다(형태가 뭉개진다). 두 문제가 한
   * 수정으로 닫힌다.
   *
   * **긴 축에 맞추고 짧은 축은 가운데 정렬**한다. 배들이 한쪽에 몰려 보일 수 있으나
   * 그것이 사실이고, 남는 자리는 육지가 채운다.
   */
  const inner = { w: VIEW_W - PADDING * 2, h: VIEW_H - PADDING * 2 }
  const scale = Math.min(inner.w / lonSpan, inner.h / latSpan)
  const originX = PADDING + (inner.w - lonSpan * scale) / 2
  const originY = PADDING + (inner.h - latSpan * scale) / 2
  const midLat = (minLat + maxLat) / 2
  const midLon = (minLon + maxLon) / 2

  /** 경도 → x, 위도 → y. **위도는 위쪽이 크므로 뒤집는다.** */
  const at = (lon: number, lat: number) => ({
    x: originX + (lon - midLon + lonSpan / 2) * scale,
    y: originY + (midLat + latSpan / 2 - lat) * scale,
  })
  ...
  return (
    ...
    >
      ...
      */}
      {/*
        육지 — `landOutline.ts`. 뷰박스를 넘는 부분은 SVG가 알아서 잘라 내므로
        범위를 미리 거르지 않는다. 44개 링이라 전부 그려도 부담이 없고, 거르는
        코드를 두면 **경계에 걸친 대륙이 잘려 보이는** 다른 문제가 생긴다.

        축선보다 **먼저** 그린다 — 배경이므로 축과 마커가 그 위에 얹혀야 한다.
      */}
      <g className="position-chart__land" aria-hidden="true">
        {LAND_RINGS.map((ring, i) => {
          let d = ''
          for (let k = 0; k < ring.length; k += 2) {
            const { x, y } = at(ring[k], ring[k + 1])
            d += `${k === 0 ? 'M' : 'L'}${x.toFixed(1)} ${y.toFixed(1)}`
          }
          return <path key={i} d={`${d}Z`} />
        })}
      </g>
      <line
    ...
    */}
    {/*
      지도(`FleetMap`)의 캡션과 **같은 것을 말한다** (`#1052` ⓥ). 자산이 있느냐에 따라
