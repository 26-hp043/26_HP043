 ...
 * 피해 간다. waypoint가 생기면(`#1205`) 그것으로 대체된다.
 *
 * **대륙간 구간에서는 육지를 가로지른다.** 상하이→로테르담의 대권은 시베리아 위를
 * 지나며 `4,822 nm`인데, 수에즈를 도는 실제 항로는 약 `10,500 nm`다 — **실제의 46%**다.
 * 「조금 돌아간다」가 아니라 절반이므로, 이 선을 그리는 화면은 그 사실을 캡션으로
 * 밝혀야 한다(`FleetMap`의 읽는 법).
 */
