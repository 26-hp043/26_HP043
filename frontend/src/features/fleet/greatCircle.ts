 ...
 *
 * ⚠️ **이것은 「예상 항로」가 아니라 「최단 경로」다.** 구면 최단 경로이므로 **육지를
 * 그대로 통과한다** — 대시보드에서 암스테르담행 항차가 유라시아를 가로지르는 선으로
 * 보인 것이 그것이다(`#1275`). 실제 항해는 해협·운하·수심·기상을 돌아가므로 거리도
 * 크게 늘어난다. **항차 모델에 경유점을 두지 않으므로**(`PRD §5.2` · `#1622`) 이 선은
 * 대권선으로 남는다 — 항로 비교의 우회 경유지 표시는 `#1300`이 맡는다.
 *
 * **그래서 이 값을 「예상 항로」로 읽히게 두면 안 된다.** 이 경로를 그리는 화면은
 * 캡션으로 그 사실을 받는다(`DESIGN_SYSTEM §9.5`).
 */
