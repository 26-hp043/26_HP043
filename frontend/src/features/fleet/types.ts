...
export type DaysReason =
  ...
  | 'NO_DATA'
  /**
   * 최근 창(30일)에 항해가 없어 소비율을 낼 근거가 없다 (`#431`).
   *
   * **`NO_DATA`와 다르다.** 실적은 있는데 **최근 구간**이 비어 기울기를 못 낸 것이다.
   * 종전에는 이 값을 몰라 폴백이 「실적 없음」을 붙였다 (`#1091`).
   */
  | 'NO_RECENT_DATA'
  /**
   ...
   * A~C 선박 대부분이 이 사유라 가장 흔한 상태인데, 종전에는 「실적 없음」으로 보였다.
   */
  | 'NOT_WORSENING'
...
export interface FleetVessel {
  ...
  /**
   * 목적항 방향 — 현재 위치 → 진행 중 항차 도착항의 초기 방위(0~360 · 북=0) (`#1804`).
   * **실제 침로가 아니다**(AIS 미수집). 서버가 항로 비교와 같은 함수로 만든다 — 화면이
   * 다시 계산하지 않는다. 위치·도착항이 없으면 `null`이고 마커는 방향 없이 그린다.
   */
