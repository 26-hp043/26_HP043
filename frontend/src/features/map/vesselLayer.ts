...
import { ratingColorToken, vesselHullVertices, VESSEL_SCREEN_LENGTH_PX } from './vesselGeometry'
...
export function createGlobeVesselLayer(
  ...
  const materialFor = (rating: string | null | undefined): THREE.MeshStandardMaterial => {
    const token = ratingColorToken(rating ?? null)
    const key = token ?? 'none'
    const found = materials.get(key)
    if (found) return found
    // 토큰을 읽지 못하는 환경(테스트·서버)에서는 중립색으로 떨어진다 — 색을 지어내지 않는다.
    const made = new THREE.MeshStandardMaterial({
      color: 0x2f5d78, roughness: 0.55, metalness: 0.08,
      // 지도의 깊이 버퍼에 가려지지 않는다 — 배는 언제나 지도 위다(마커와 같은 규칙).
      depthTest: false, depthWrite: false,
      // 손으로 적은 정점이라 **감기 방향이 면마다 다르다** — 한쪽만 그리면 사라진다.
  ...
  const sync = () => {
    ...
    for (const vessel of model.vessels) {
      ...
      if (!mesh) {
        mesh = new THREE.Mesh(geometry, materialFor(vessel.rating))
        meshes.set(vessel.id, mesh)
        scene.add(mesh)
      } else {
        mesh.material = materialFor(vessel.rating)
      }
  ...
  const start = () => {
    ...
    scene.add(sun)
    // +Y가 선수 방향인 저폴리 선체. 외부 asset·texture가 없어 재배포 출처가 코드 자체로 닫힌다.
    geometry = new THREE.BufferGeometry()
    geometry.setAttribute('position', new THREE.BufferAttribute(vesselHullVertices(model.mode), 3))
