 ...
 *
 * 잠그는 것은 두 가지다(종전 셋째 「demo 모드의 고정표」는 `#542`가 데모 모드를 폐기해 없어졌다 · `#1660`).
 *
 * * **서버 목록**이 온다 — 네 화면이 고정표를 읽던 것이 이번 결함이다
 * * 성공·실패·재사용 경로가 모두 테스트로 잠긴다
 *
 * 판정에 쓰이는 부분은 전부 순수 함수로 빼 두었다 — `isKnownFuel`이 그것이다. 화면 배선
 * (`useFuelOptions`)은 이 파일에서 렌더하지 않는다(종전 사유 「DOM 환경 없음」은 `#557` 이후
 * 사실이 아니다 · `#1660`).
 */
