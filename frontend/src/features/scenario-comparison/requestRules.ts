 ...
 * 초기 조건.
 *
 * 선박과 연도를 비운다 — 나머지는 종전 `DEMO_REQUEST`가 쓰던 값을 그대로 옮겼다.
 ...
 *
 * **연도에도 넣지 않는다** (`#1093` ⑷). 종전에는 `'2026'`이 박혀 있었다. 연도
 * 목록이 실패·빈 목록이면 화면은 셀렉트 대신 주석 한 줄을 그려 **사용자가 연도를
 * 고를 수 없는데**, 이 값이 검증을 통과해 **고른 적 없는 2026년 기준 결과**가
