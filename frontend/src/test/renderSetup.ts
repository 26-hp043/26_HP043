 ...
 *
 * ⚠️ **테스트가 끝난 뒤 비동기로 찍히는 호출은 보지 않는다.** 대기하지 않은 Promise·타이머가
 * `afterEach` 뒤에 경고를 내면, 다음 테스트의 감시기에 잡혀 **엉뚱한 테스트가 실패**하거나 두
 * 테스트 사이에 원본으로 흘러 아무도 잡지 못한다. 지금 스위트에는 그런 호출이 없다(PR #1840
 * 리뷰). 어떤 테스트가 이유 없이 이 메시지로 실패하면 **바로 앞 테스트**의 미대기 비동기부터 본다.
 */
