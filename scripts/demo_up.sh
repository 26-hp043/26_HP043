...
#   bash scripts/demo_up.sh --check   기동하지 않고 현재 상태만 점검 — **아무것도 쓰지 않는다**
#                                     (계산 경로 확인은 세션·계산 이력을 만들어 건너뛴다 · #1639)
...
# --- `--check`는 이 단계를 건너뛴다 (#1639) -------------------------------------------
#
# 이 단계는 **쓰기를 한다.** `dev-login`이 `user_session` 행과 감사 로그를 만들고,
# 계산 호출이 `calculation_run` 행과 감사 로그를 더한다(`persist=True`가 기본이다).
#
# 「점검만 했는데 DB가 바뀐다」는 `--check`의 계약(「기동하지 않고 상태만 본다」)과
# 어긋난다. 시연 직전 점검이 계산 이력을 남기면, 이력 화면·감사 로그에 사람이 하지
# 않은 실행이 섞인다.
#
# **읽기 전용 대체를 두지 않는 이유** — 계산 경로를 인증 없이 확인할 방법이 없다.
# 조회 API도 세션을 요구하므로 `dev-login`부터 쓰기가 된다. 그래서 여기서는 **확인하지
# 않았다는 사실을 말하고** 넘어간다. 「됐다고 착각하게 두지 않는다」가 `#616` 이후의 규율이다.
if [ "$CHECK_ONLY" = "--check" ]; then
  info "점검(--check)은 계산 경로를 호출하지 않습니다 — 세션·감사 로그·계산 이력을 만들기 때문입니다 (#1639)."
  info "계산 경로는 실제 기동(인자 없이 실행)에서 확인합니다."
else
