...
LAYERS: tuple[tuple[str, str], ...] = (
    ...
    ("layer:ui", "LD 화면 규격"),
    ("layer:platform", "LP 플랫폼"),
    ("layer:assurance", "LV 검증"),
    #
    # ── 별도 트랙 ─────────────────────────────────────────────────────────
    #
    # 2.5D 시각화 묶음(#1430~#1449 · #1456~#1473)은 연간 시뮬 결과 화면에 붙지만
    # **L2 선박에 섞지 않는다** (#1548). 38건이 L2에 들어가면 선박 계층의 열린
    # 이슈 대부분이 2.5D가 되어 선박이 밀려 보이는 착시가 생긴다. 담당 미정 ·
    # 결정요청 제외인 별도 트랙이라 자기 행에서 따로 센다.
    #
    ("layer:2.5d", "L2.5 2.5D 시각화"),
    #
    # ── 폐지 ─────────────────────────────────────────────────────────────
    #
    # `layer:cross`는 **비웠지만 지우지 않았다** (#794). 닫힌 이슈 59건이 이
    # 라벨을 달고 있어 삭제하면 그 이력이 사라진다.
    #
    # ⚠️ **맨 뒤에 남기는 것이 핵심이다.** `#777`은 폐지한 `layer:backlog`를 이
    # 목록에서 아예 빼 버렸는데, 그러면 그 라벨만 달린 open 이슈가 들어왔을 때
    # `(미부착)`으로 떨어진다 — `#93`이 08-22에 겪은 「표에서 사라져 없는 것이
    # 되는」 상태다. 0건 행이 폐지 사실 자체를 보이게 한다.
    #
    ("layer:cross", "LX 횡단 (폐지)"),
    # `layer:backlog`도 같은 이유로 **맨 뒤에 남긴다** (`#1350`). 바로 위 주석이
    # *「`#777`이 폐지한 `layer:backlog`를 목록에서 아예 빼 버렸다」*고 적으면서도
    # 정작 그 항목을 되돌려 놓지 않았다 — 설명과 목록이 어긋나 있었다.
    ("layer:backlog", "LB 보류 (폐지)"),
)
...
def fetch(state: str) -> list[dict]:
    ...
    result = subprocess.run(
        [
            ...
            "--state",
            state,
            # ⚠️ **전수여야 한다** (`#1350`). 200으로 박아 두어 `--state all`(실측 612건)이
            # 조용히 잘렸다 — 표의 수치가 「전체」가 아니라 「앞의 200건」이었고, 그 사실이
            # 어디에도 나타나지 않았다. `gh`의 상한은 없으므로 넉넉히 둔다.
            "--limit",
            "5000",
            "--json",
            "number,title,labels",
