...
>
> **무엇을 담았나 (실측 · 2026-09-12).**
>
> | 층 | 범위 | 용량 | 읽히는 것 |
> |---|---|---|---|
> | z0–z6 | 전 세계 | **44.9 MB** | 해역과 국가. 대한해협이 구분된다 |
> | z7–z10 | 항만 43곳 ±0.25°(약 55 km) | **38.0 MB** | **항만 구역과 방파제 안쪽** |
> | 글리프 | 2종 × 256 range | **12.0 MB** | 라벨. CDN을 쓰지 않는다 |
>
> 합계 **약 95 MB**. `scripts/fetch_basemap.sh`가 배포 볼륨으로 내려받는다 — 저장소에 넣지 않는다(이미지가 이미 699 MB다).
>
> **항로(대양)에는 깊은 층을 주지 않았다** — 대양 한가운데는 확대해도 새로 보일 것이 없다(해안선도 섬도 없다). 값이 생기는 자리는 해안과 항만 주변이다. 깊은 층이 없는 지역을 확대해도 마지막 층을 늘려 보여 주므로(overzoom) **화면이 비지 않는다.**
>
> ⚠️ **z10을 상한으로 둔 것은 용량이 아니라 데이터 때문이다.** 지금 위치는 사람이 찍은 한 점이다. z12까지 열면 **한 달 전에 손으로 찍은 점이 부두 하나를 정확히 가리키는 것처럼** 보인다. AIS(`#764`)가 붙어 관측이 쌓이면 그때 올린다.
>
