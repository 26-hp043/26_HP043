...
| **샘플 선박 선택** | OUT OF SCOPE | **데모 시드가 그 역할을 대신한다** (`#826` M-2 판정 · 2026-09-09). 아래 각주 참조 |
...
> ## ⏸ 「샘플 선박 선택」의 범위 판정 (`#826` M-2 · 2026-09-09)
>
> `§5.1` 「선박 관리」 행에 있던 **샘플 선박 선택**을 제외 범위로 옮긴다. 같은 행의 **선박 등록·제원 수동 입력은 MUST로 남는다.**
>
> **다른 미구현 MUST 넷과 성질이 다르다.** `#826`이 함께 지목한 나머지(CSV 내보내기 · 기능① 액션 3종 · 기능② 입력 7종 · 기상 보정)는 **서버가 완비돼 있고 화면 배선만 없었다** — 그래서 각각 `#890`·`#891`·`#892`로 갈라 착수한다. 샘플 선박만 저장소 전체에서 **재료가 하나도 없다.**
>
> ```
> is_sample류 컬럼      0건
> UIFLOW의 「샘플」      0건
> vessel-registration/  0건
> ```
>
> **데모 시드가 이미 그 자리를 채우고 있다.** `demo_seed`가 선박 4척(합성 2·실존 2)과 항차 11건을 넣고 시연이 그것을 쓴다. 「샘플 선박 선택」이 원래 답하려던 질문은 **「빈 계정으로 시작한 사용자가 무엇을 볼 것인가」**인데, 그 질문은 `#808`(가입 정책·데이터 격리)이 시연 계정 처리 방식을 정하면서 **그쪽에서 답해진다** — 사용자별 데이터 격리가 이미 이 표에서 OUT OF SCOPE이므로 로그인 사용자는 같은 선박을 공유한다.
>
> **되돌리는 조건** — `#808`이 계정별 격리를 도입하는 방향으로 정해지면 「빈 계정의 첫 화면」이 다시 실제 문제가 된다. 그때 이 행을 지우고 `§5.1`로 옮긴다. 필요한 재료는 세 갈래 중 하나를 고르는 것이다: `vessel.is_sample` 컬럼 · 고정 목록 · 데모 시드 재사용.
>
> **곁가지** — `§11`의 연료 산정 우선순위 3단계(*「샘플 선박 기본값이 있는 경우 → 샘플 기본값 사용」*)도 구현이 없다(`services/scenario_compare.py:448-450`은 요청값 → `vessel.reference_daily_foc_ton` 2단계뿐이다). 이 판정과 같은 이유이며, 되돌릴 때 함께 본다.
