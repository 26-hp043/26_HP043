...
| 2026-08-17 | PR #NNN | **v4.4 — 헤더 정체성 정정 (`#460`).** 제품명을 **BlueLog**로 명시하고 종전 표기(「중소선사를 위한 CII 예측 및 운항 의사결정 보조 플랫폼」)를 부제로 내렸다 — 화면(`AppShell.tsx`)과 `MAIL_FROM`은 2026-08-04부터 이 이름을 쓰는데 **문서 9종 어디에도 없었다.** 「문서 목적」의 «PRD v3.0을 상세화»도 정정했다(현재 v4.4). 본문 변경 없음 (#460) |
