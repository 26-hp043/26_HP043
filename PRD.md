...
> **[EXT-P0-3]** 본 PRD의 fixture 값은 **설명용 표시값**(소수 9자리)이며, 자동 테스트의 기준값은 `tests/fixtures/cii/*.json` 하나뿐이다. CII 계산은 **중간에 자르거나 반올림하지 않고 끝까지 이어서 수행**하며, 반올림은 최종 출력 직전 1회(`ROUND_HALF_UP`)만 한다. **등급 판정은 반올림 전 원값으로 한다.** 계산 규칙의 정본은 `TECH_SPEC §1.2.1`이다.
>
...
| 2026-08-05 | `#PR` | §13.1 Fixture 1 기대값을 소수 10자리 → 9자리로 통일하고 `[EXT-P0-3]`을 계산 규칙 문안으로 교체 — 단계별 표시값 곱셈으로 얻은 값이었음 (#166) |
