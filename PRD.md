...
| AI 연료 예측 | 실험 기능 | MAY |
| LLM 챗봇 (O-12) | 핵심 경로 제외, 실험 기능으로만 허용 — §20 O-12 · §16.3 참조 | MAY |
| 사용자 인증 | 구글 OIDC 로그인, 세션 관리 | MUST |
...
| 자체 회원가입·비밀번호 관리 | OUT OF SCOPE | 인증은 구글 OIDC에 위임한다. 비밀번호를 저장하지 않는다 |
...
> **[#280] 이 표는 기능 요구사항의 화면 정의 목록이며, 화면 구조의 정본은 아니다.** 화면 목록·계층·흐름·진입 조건은 `UIFLOW.md` §1·§2가 소유한다(`AGENTS §3.2.1`). 두 문서의 대응은 다음과 같다: SCR-001 ↔ UIFLOW 1-3(메인보드), SCR-002 ↔ 1-2(선박 등록), SCR-003 ↔ 2-1(CII 예측), SCR-004 ↔ 2-2(항로 비교), SCR-005 ↔ 2-3(연간 등급 관리), SCR-006 ↔ 파라미터 조회(별도 화면 정의는 `UIFLOW`에 없음 — `TECH_SPEC §16.2` 참조), SCR-007 ↔ 2-5(보고서, 내부 보고용 한정). **UIFLOW §2의 2-4(선대 모니터링)·2-6(설정)은 MVP 범위 밖**이다(`§5.2` 제외·`§5.1` 미포함, `AGENTS §3.2.3` 범위 판정행).
...
인증 주체를 나타낸다. 인증은 구글 OIDC에 위임하며, 제품은 비밀번호를 저장하지 않는다.

| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `id` | UUID | Y | 내부 사용자 ID |
| `google_sub` | string | Y | 구글 OIDC `sub` 클레임. 사용자 식별의 단일 기준 |
| `email` | string | Y | 표시·연락용. **식별자로 사용하지 않는다** |
| `display_name` | string | N | 구글 프로필 이름 |
| `last_login_at` | datetime | N | 마지막 로그인 시각 |
| `is_deleted` | boolean | Y | Soft delete 플래그 |

> **식별자는 `email`이 아니라 `google_sub`이다.** 구글 계정의 이메일은 변경될 수 있고 회수·재할당될 수 있다. `sub`는 해당 계정에 대해 변하지 않는다. `email`을 키로 쓰면 이메일 변경 시 동일인이 다른 사용자로 갈리고, 재할당 시 서로 다른 사람이 같은 사용자로 합쳐진다.
>
...
| O-13 | 사용자 인증 범위 | 구글 OIDC 인증만 도입. 사용자별 데이터 격리·RBAC는 제외 | CLOSED |
