...
>
> **곁가지** — `§11.4` 연료 산정 3단계(*「샘플 선박 기본값이 있는 경우 → 샘플 기본값 사용」*)는 같은 재료를 쓰지만 **계산 결과가 바뀌는 변경**이라 별도 이슈로 이어서 처리한다(`services/scenario_compare.py`는 요청값 → `vessel.reference_daily_foc_ton` 2단계뿐이다).
