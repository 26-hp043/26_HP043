...
| 6. 기타 — 어드민 계정 | §20 O-13(인증 범위) → **O-14** | **판정 완료 — 1차 시연 범위 밖** (`#359` · 재개는 `#672`) |
...
| AIS·IoT 자동 수집 | MVP 일정·선박별 장비 차이. **선박의 현재 위치·운항 상태는 사용자 입력 또는 시뮬레이션 시계로 확보한다**(COR-5). **IoT는 AIS와 성격이 달라 따로 다룬다** — 엔진 명명 체계가 표준으로 단일화되지 않아 사실상 선주별 개별 연동이다(`§21` `[#764]` 각주) |
...
| AIS 자동 위치 수집 | OUT OF SCOPE | **위치·운항 상태 데이터 자체는 MVP에 포함된다.** 자동 수집만 제외하며, 확보 방식은 사용자 입력 또는 시뮬레이션 시계다. **수집 경로(테이블·제공자 계약)는 열려 있다** — 붙일 AIS 출처가 아직 없어 자동 수집만 남았다(`§21` `[#764]` 각주) |
...
| **함대 감축 계획 화면** (`UIFLOW 2-10`) | OUT OF SCOPE | **1차 심사 범위 밖** (`#513` 판정 · 2026-08-23). 아래 각주 참조 |
| **데이터 점검 화면** (`UIFLOW 2-11`) | OUT OF SCOPE | **1차 심사 범위 밖** (`#513` 판정 · 2026-08-23). 아래 각주 참조 |

> ## ⏸ 선대 계층 신설 화면 2종의 범위 판정 (`#513` · 2026-08-23)
>
...
> |---|---|---|
> | `2-10` | ~~감축 계획 산식의 범위~~ **→ 2026-09-10 해소**(`§12.3.1` 신설 · `#433`) · **선박별 일일 용선료 단가의 입력 경로** | **비용 요약(용선료 손실·순손익)을 계산할 수 없다** — 스키마에 단가 컬럼이 0개다 |
> | `2-11` | 이상치 판정 임계값 · `CII 영향` 산출 방법 · 「데이터 완결성」 지표의 정의 | **심각도 4종 중 1종과 목록의 한 열이 빈다.** 임계값을 구현이 정하면 CII 신뢰도 기준을 근거 없이 만드는 것이 된다 |
>
...
| 에러 | 영역 실패 — 처리 | `{동작}에 실패했습니다` | 계산 · 비교 · 시뮬레이션 · 재현 |
...
| **O-14** | **사용자 인증 방식** | **자체 이메일·비밀번호 인증을 도입한다.** 가입 시 이메일 인증, 비밀번호 재설정을 제공한다. 구글 OIDC는 완전히 제거한다. 사용자별 데이터 격리·RBAC는 계속 제외한다(O-13에서 승계). **가입은 허용 도메인 또는 초대 코드로 제한한다 — 사내 도구(`#808` · 2026-09-11)** | **CLOSED** |

> **[#359] 신방향 명세 6의 「어드민 계정」은 1차 시연 범위 밖이다 — O-14의 RBAC 제외는 그대로 유효하다.** (2026-08-23 결정)
>
...
| **제출 전 자체 점검** [#770] | 대체 계산 건수(연료·거리) · 실적 미입력(진행 중) 건수와 그 뜻. **용도 고지를 함께 싣는다** | `§2.14` `ytd.substitutions`(#449) |
...
>
> **용도 고지가 절에 실린다** — *「제출용 문서가 아닙니다. … 이 표의 「이상 없음」은 규제 적합 판정이 아닙니다.」* `§0.3`의 기본 원칙과 `§25.1`의 「하지 않는다」는 그대로다.
>
