...
| 6. 기타 — 어드민 계정 | §20 O-13(인증 범위) → **O-14** | **역할 2종 도입 — 사무직·현장직** (`#672` · 2026-09-15 사용자 결정. `#359`의 「1차 시연 범위 밖」 판정을 재개해 닫았다) |
...
| MVP 접근 | 역할은 **사무직·현장직 2종**만 둔다(§20 O-14 · `#672`). 두 역할이 **같은 화면·같은 데이터**를 보며, 사무직 전용 조작(제원·시뮬레이션 실행·채택·보고서·감축 계획·역할 지정)만 갈린다. 조직·세부 권한 설정은 두지 않는다 |
...
| 권한 관리 고도화 | **부분 포함** (2026-09-15 개정 · `#672`) | **사무직(`OFFICE`)·현장직(`FIELD`) 역할 2종은 범위 안**이다 — 현장직은 「넣고 본다」, 사무직은 「정하고 낸다」(§20 O-14 · `API_SPEC §1.2` 역할 표 · `UIFLOW §2.2` 역할 열). **셋 이상의 역할 · 화면별 세부 권한 · 조직 설정은 두지 않는다** — 필요해지면 이 행을 먼저 개정한다 |
...
| 역할 거부 (`403 FORBIDDEN_ROLE`) | `이 작업은 사무직 계정만 할 수 있습니다.` |
| 마지막 사무직 (`409`) | `마지막 사무직 계정은 탈퇴하거나 현장직으로 바꿀 수 없습니다. 다른 계정을 먼저 사무직으로 지정해 주세요.` |
| 역할 설명 (설정 화면) | `현장직은 항차 실적을 넣고 현황을 봅니다. 사무직은 선박 제원·연간 시뮬레이션·보고서·함대 감축 계획과 계정 역할을 다룹니다.` |
...
| `role` | string | Y | **사무직(`OFFICE`) · 현장직(`FIELD`)** (`#672` · §20 O-14). 새 계정은 현장직, 기존 계정은 전부 사무직으로 시작했다. **행위 권한이지 소유권이 아니다** |
...
>
> **[#672] `role`은 「무엇을 할 수 있나」이지 「누구 것인가」가 아니다.** 역할을 두어도 위 문장은 그대로다 — 현장직이 넣은 항차를 사무직이 고치고, 사무직이 등록한 선박에 현장직이 실적을 넣는다. `#316`(소유권 검사 부재)의 판정은 이 문장에 기대 있으므로 그대로 유효하다. 최초 사무직·마지막 사무직 규칙은 `API_SPEC §1.2`가 정한다.
...
| **O-14** | **사용자 인증 방식** | **자체 이메일·비밀번호 인증을 도입한다.** 가입 시 이메일 인증, 비밀번호 재설정을 제공한다. 구글 OIDC는 완전히 제거한다. 사용자별 데이터 격리는 계속 제외한다(O-13에서 승계). **역할은 사무직·현장직 2종을 둔다(`#672` · 2026-09-15 — 종전 「RBAC 계속 제외」를 대체. §5.2 · §7.10 · `API_SPEC §1.2`).** **가입은 허용 도메인 또는 초대 코드로 제한한다 — 사내 도구(`#808` · 2026-09-11)** | **CLOSED** |
