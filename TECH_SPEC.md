...
> 참조로 바뀐다(#159).
>
> `PRD §6.1 · §6.2`(화면 7개 SCR-001~007)와 `UIFLOW §2`는 구조적으로 병렬 정의다.
> **범위 충돌은 해소됐다**(#280): UIFLOW 2-4(선대 모니터링)·2-6(설정)은 MVP 밖,
> 2-5(보고서)는 내부 보고용 한정으로 각주 처리됐고, `PRD §6.2`에 SCR ID ↔ UIFLOW 절
> 대응 각주가 있다. 화면 구조의 정본은 `AGENTS §3.2.1`대로 `UIFLOW`다. 남은 별도
> 작업은 `API_SPEC`의 엔드포인트↔화면 매핑 표(`§6.2 SCR-002~007` 참조 17행) 정리뿐이다.
...
| 2026-08-14 | `#___` | §2.5.2에 lock 파일 미도입 결정 각주 추가(범위 핀 원칙·전환 신호) · stale 참조 정정(requirements.txt → pyproject.toml) (#235) |
| 2026-08-14 | `#___` | §16.2 불일치 각주 갱신 — UIFLOW 범위 충돌 해소 표시(2-4·2-6 MVP 밖, 2-5 내부 보고용), PRD §6.2 대응 각주 참조 (#280) |
| 2026-08-14 | `#___` | §13.1에 인증 주체·로그인 이벤트 문단 추가 — user_id 귀속·자격 증명 미기록 원칙 (#277) |
