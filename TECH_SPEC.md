...
> ⚠️ **화면 구조에 한해 `UIFLOW.md`가 `PRD.md`보다 앞선다.** 근거는 두 문서가 서로를
> 그렇게 가리킨다는 점이다 — `DESIGN_SYSTEM §0.3`이 "화면 구조·플로우 → UIFLOW §2"로
> 넘기고, `UIFLOW` 헤더는 후속 문서로 "프론트엔드 구현(#133 · #136)"을 지정한다.
>
> **이는 `AGENTS.md §3`의 예외가 아니라 공백이다.** §3 우선순위 목록(IMO 원문 → PRD →
> TECH_SPEC → API_SPEC → DB_SCHEMA → TEST_PLAN)에 `UIFLOW.md`는 **아예 없다.**
> `DESIGN_SYSTEM.md`에 대해서는 "디자인 토큰은 최종 권위이며 위 순위와 교차하지 않는다"는
> 카브아웃이 있으나 그 범위가 **토큰의 이름·의미·제약**에 한정되어, 위 표의 레이아웃·표기
> 규칙(§7.1 폭 정책 · §11 추정값 표기 등)은 그 범위를 넘는다.
> §3에 `UIFLOW.md`를 편입하고 `DESIGN_SYSTEM.md` 카브아웃 범위를 넓히는 것은 별도
> 이슈로 처리한다. **그때까지 본 절이 근거다.**
>
> 그 결과 `PRD §6.1 · §6.2`(화면 7개 SCR-001~007)와 어긋난 상태다. PRD를 UIFLOW
> 기준으로 정정하는 것도 별도 이슈에서 처리하며, `API_SPEC`의 엔드포인트↔화면 매핑
> 표(`§6.2 SCR-002~007` 참조 17행)도 같은 범위에 포함한다.
...
| 2026-08-01 | `#158` | §16.2에 프론트엔드 디렉터리 구조(`frontend/`) 추가 — 백엔드/프론트엔드 블록 분리, 영역별 기준 문서 표 신설(화면 구조는 UIFLOW 우선 — AGENTS §3 공백 보완), 토큰 단일화·provider 경계 규칙 명시 (#133) |
