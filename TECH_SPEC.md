...
> 이는 `AGENTS.md §3` 문서 우선순위의 **예외**이므로 §3에 조항을 추가하는 별도 이슈로
> 상환한다. 그때까지 이 절이 근거다.
>
> 그 결과 `PRD §6.1 · §6.2`(화면 7개 SCR-001~007)와 어긋난 상태다. PRD를 UIFLOW
> 기준으로 정정하는 것도 별도 이슈에서 처리한다.
...
| 2026-08-01 | `#158` | §16.2에 프론트엔드 디렉터리 구조(`frontend/`) 추가 — 백엔드/프론트엔드 블록 분리, 영역별 기준 문서 표 신설(화면 구조는 UIFLOW 우선, AGENTS §3 예외), 토큰 단일화·provider 경계 규칙 명시 (#133) |
