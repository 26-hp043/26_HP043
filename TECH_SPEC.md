...
| 2026-08-07 | `#PR` | §1.2.1에 「작업 정밀도의 적용 지점」·「공표 시점의 확정」 소절 신설 — 전역 `getcontext()` 금지·thread-local 근거·`rounding` 예외·**파생값도 적용 지점 안에서 계산**(실측 대조 포함), 확정 대상을 값의 역할로 구분. 코드블록을 `localcontext` 기반으로 교체하고 컨텍스트 표에 「적용 범위」 행 추가. ⚠️ 구현 미충족 주석을 정합화 완료 기록으로 교체 (#179) |
