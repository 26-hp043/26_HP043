...
Layer 1 상수 체인(`cii_ref` → `required_cii` → 등급 경계값)은 **전정밀도로 연속 계산**하며, 반올림은 각 정본값을 **공표하는 시점에 한 번만**(`ROUND_HALF_UP`) 적용한다. **반올림된 값을 다음 단계 입력으로 사용하지 않는다.** 등급 판정도 반올림 전 원값으로 한다(`PRD §13.1 [EXT-P0-3]`).
...
`CII_ref = 4745 × 50,000^(-0.622)`를 작업 정밀도만 바꿔 계산한 결과다.
...
`PRD §9.3`의 내부 정밀도·화면 표시 규정은 **계산·응답·출력 계층**에 적용되며 **픽스처 파일의 표기 자릿수를 규정하지 않는다.** 픽스처는 **수학적으로 정확한 최소 표기**로 적고, 값의 동등성은 **수치 비교**로 판단한다(표기 자릿수는 비교 결과에 영향을 주지 않는다).

> ⚠️ **현재 구현은 본 조항을 충족하지 못한다.** `calc/precision.py`의 `LAYER1_PRECISION = 30`이 작업 정밀도로 쓰이고 있어, `cii_engine._decimal_power`의 `ln`/`exp`가 30자리에서 반올림된다. 그 결과 `cii_ref`가 정본과 30번째 자리에서 어긋나고(`…25808` vs `…25818`), 체인 단계마다의 반올림도 남아 있다. **본 절은 규칙을 세우고, 구현 정합화는 별도 이슈에서 처리한다.**
...
> 참조로 바뀐다(#159).
>
> `PRD §6.1 · §6.2`(화면 7개 SCR-001~007)와 `UIFLOW §2`는 아직 어긋나 있다. 다만 어느
> 쪽을 따를지는 `AGENTS §3.2.1`이 정했으므로 **구현 판단은 막히지 않는다.** `PRD` 본문
> 정정과 `API_SPEC`의 엔드포인트↔화면 매핑 표(`§6.2 SCR-002~007` 참조 17행)는 별도로
> 처리한다.
...
| 2026-08-05 | `#PR` | §1.2.1에 Layer 1 계산 규칙 신설 — 중간 단계 처리·여유 정밀도(guard digits)·불변성 검사·「N자리」 표기 규약·픽스처 표기와 비교. §1.2.3 검증 수식을 절단 없는 체인으로 교체 (#166) |
