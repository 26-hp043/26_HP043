...
| `test_report_export_routes_api_db.py` | 9 | **§4 API · 리포트·내보내기 라우트** — 형식 분기(html·csv·pdf)와 `Content-Disposition`(미리보기는 첨부가 아니다 · 첨부는 ASCII `filename`과 UTF-8 `filename*`을 둘 다) · 내보내기 `type` 3종의 CSV 첨부와 JSON 봉투(`meta.row_count` = 실제 행 수) · `year` 필터 · 형식·종류 오류의 422 (`#911`) |
...
| `test_auth_tokens.py` | 19 | **§4.7 인증 API** — 토큰 일회성·만료·용도 분리 · 재설정 시 세션 전량 무효화 |
...
**합계 117개 파일 · 1580 함수 · 1961 수집.** (2026-09-11 실측)
