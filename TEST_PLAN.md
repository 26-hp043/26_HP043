...
| `test_chat_tools.py` | 11 | **§3.18 통합 · 도구 계층**(`#121`) — 쓰기 도구가 없는가 · 봉투에 도메인 값이 섞이지 않는가 · **API 이름(`estimated_rating`)이 정본 이름(`rating`)으로 바뀌어 나가는가** · 대응표 도착지가 전부 화이트리스트 안인가 · 모르는 도구가 예외가 아니라 봉투로 돌아오는가 · 도구 `project_year_end`의 이름·설명이 「시뮬레이션」「기능③」을 쓰지 않는다(결정론 연말 예상 · `#1534`) |
...
**합계 191개 파일 · 2572 함수 · 3200 수집.** (2026-09-22 실측 — #1534 챗봇 연말 예상 도구 2함수 추가 반영)
