...
| 최종 수정일 | 2026-08-23 |
| 상위 문서 | `PRD.md` v4.4, `TECH_SPEC.md` v1.7, `API_SPEC.md` v1.21, `DB_SCHEMA.md` v1.15 — `AGENTS §4.4` 「마지막으로 대조를 마친 판본」 |
...
| `test_reports.py` | 46 | **§3.4~§3.5 리포트 렌더링** — CSV injection 방어 · BOM · 면책 · 한글 PDF · **`DESIGN_SYSTEM §4` 표시 형식**(자릿수·천단위 구분자·선종 표기·KST 시각). 문서가 직렬화 자릿수를 그대로 내보내 화면과 갈렸다 (`#584`) · **표시 문구 동기화**(위험도·경고·사유·항차 상태를 정본/화면과 대조 — `#631`) |
| `test_reports_db.py` | 27 | **§4 API · 리포트 데이터 수집** — 진행 중 항차 제외 · 시나리오 인용 · 값 재계산 금지 · **문서 어디에도 UTC ISO가 남지 않는다**(`#646`) |
...
| `test_demo_up_script.py` | 6 | **§5 DB · 운영 스크립트** — 시연 기동 스크립트의 계약. `bash -n` 문법 · **JSON 값 추출**(파이썬 없이) · `--check`가 `.venv` 없이 도는 것 · 기동은 여전히 막히는 것. **CI가 이 스크립트를 실행하지 않아** `#616`의 `mktemp` 오류가 저장소에 들어와 있었다 (`#637`) |
...
**합계 102개 파일 · 1344 함수 · 1630 수집.** (2026-08-23 실측)
