...
| `test_suite_lock_db.py` | 3 | **§5 인프라 · 테스트 격리 · 실행 잠금** — 스위트가 도는 동안 테스트 DB 잠금을 쥔다 · **두 번째 `pytest`를 실제로 띄워** 원인을 말하며 즉시 멈추는지(종료 코드 3) · `upgrade head` 실패가 남은 리비전과 복구 절차를 알리는지 (`#894` — `#691`과 다른 층: 이쪽은 「지금 누가 쓰는가」) |
...
**합계 163개 파일 · 2210 함수 · 2703 수집.** (2026-09-20 실측 — #1207·#1305·#1294 9함수·12수집 추가 반영)
