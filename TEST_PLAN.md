...
**합계 192개 파일 · 2622 함수 · 3267 수집.** (2026-09-23 실측 — #1703 규제 기준값 표 도구 6함수 추가 반영)
...
| 2026-09-23 | `#___` | §14 `test_chat_api_db.py` 29 → **30** · `test_chat_tools_db.py` 19 → **24** · 합계 실측 갱신(2616함수·3261수집 → **2622함수·3267수집**). 도구 `lookup_regulation`이 시드 표의 값을 출처와 함께 그대로 주는지(설정 화면과 같은 조회 서비스), 선박 없이도 도는지, 표에 없는 연도를 다른 해로 채우지 않는지, 모르는 선종·연료를 고정 문구로 거절하는지, 표 값을 인용한 답이 수치 가드를 통과하는지를 잠근다 (#1703) |
