...
| 2026-09-26 | `#1944` | §14 `test_deploy_secret_shape.py` **신설(3함수)** · 합계 실측 갱신(220파일·2878함수·3589수집 → **221파일·2881함수·3592수집**). PR #1798 되돌림의 원인을 가리려고 넣은 임시 진단 단계가 시크릿 값을 찍지 않는지 실제 실행으로 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1634) |
