...
| `test_annual_simulation_read_db.py` | 32 | **§4 API · 기능③ 조회·재실행** — 조회가 다시 계산하지 않는지 · 스냅샷 항차 표현 · 재현 판정(파라미터 변경 409 / 재현 실패 500) (`#443`) · **선박 제원 스냅샷**(`#493` — 제원·capacity·선종을 고쳐도 재현이 흔들리지 않는다 · `037` 이전 실행은 사유를 밝히고 끊는다) |
...
**합계 150개 파일 · 1929 함수 · 2410 수집.** (2026-09-13 실측)
