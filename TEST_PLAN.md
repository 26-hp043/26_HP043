...
| 2026-09-25 | `#___` | §3.16 `IT-MAP-001`에 **두 항구 이름** — 선대 요약 `route`가 출발항·도착항 이름을 싣는다(`#1882` · 지도의 항구 핀이 이름을 읽어 준다). §14 인벤토리 수치 변화 없음(기존 함수의 기대값만 넓혔다). 화면 쪽 핀 검사는 `FleetMap.test.tsx`(vitest)다 (#1882) |
