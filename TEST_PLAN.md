...
| IT-STATE-009 | **동시 쓰기의 직렬화** | 두 연결이 같은 항차에 전환 × 전환·PATCH·실적·삭제·채택을 **커밋 직전에** 교차 · 정박 구간(선박 X) × 채택 `CREATE_NEW_VOYAGE`(항차 X)를 **INSERT 전에** 교차 · 정박 구간(선박 X) × 항차 X 경로 다섯(전환·PATCH·실적·삭제·채택 UPDATE)을 **`voyage` UPDATE 전에** 교차 | 뒤 요청은 앞 커밋 뒤의 상태로 판정 — 종결 상태 덮어쓰기 없음 · 채택 행 하나 (`#1626`) · 교차 잠금이 교착하지 않음 (`#1860` · `#1868`) |
...
| `test_voyage_row_lock_db.py` | 7 | **§3.1 통합 · 항차 쓰기의 직렬화**(`IT-STATE-009` · `#1626` · `F-8`) — **두 연결을 실제로 교차시킨다.** 첫 세션의 `commit`을 갈아 끼워 **잠금을 쥔 채 커밋 직전에** 머물게 하고 그 사이에 두 번째를 넣는다 — 서비스가 스스로 커밋하므로 호출이 끝난 뒤에 교차시키면 잠금이 있든 없든 같은 결과가 나온다(`#1629` 코멘트). 다섯 짝: 전환×전환(종결 뒤 `IN_PROGRESS` 422 · 새 상태를 **보고** 거절했는지 문구로 확인) · 전환×PATCH(출항 뒤 계획값 422) · 전환×실적(취소 뒤 실적 422) · 전환×삭제(`PLANNED` 뒤 삭제 422) · 채택×채택(**채택 행 하나**, 나중 것이 남는다). 돌연변이(`get_by_id`의 `for_update` 갈래 제거) **실측**(2026-09-24): 앞 넷은 **실패** — 셋(전환 × 전환 · PATCH · 실적)은 두 번째가 예외 없이 성공하고, 전환 × 삭제는 첫 세션의 UPDATE가 사라진 행을 겨눠 `StaleDataError`로 터진다 — 채택 × 채택은 **통과한다**(행 1건 · 단독 실행도 통과). 이 교차 시점에서는 항차 잠금이 없어도 두 채택이 같은 `voyage_scenario` 행을 쓰며 순서가 정해지는 것으로 보인다(정황) — 이 짝은 항차 잠금을 **검출하지 못한다**(`#1869`). `#1796` ⑺의 「채택 행 2건」은 서비스 호출 뒤 교차라 조건이 다르다. (`#1850` 당시 로컬 DB가 내려가 있어 CI로만 확인했다.) 여섯째 짝(`#1860`): **정박 구간 × 채택 `CREATE_NEW_VOYAGE`의 교차 잠금** — 첫 세션이 선박 X를 쥔 채 INSERT 전에 머무는 사이(커밋 직전이 아니다 — 교착은 두 INSERT가 서로의 부모를 기다릴 때 난다) 채택을 넣는다. 채택이 선박을 먼저 잠가 둘 다 성공(구간 1 · 새 항차 1). 돌연변이(선박 선잠금 제거 — `#1868`부터 그 자리는 `get_by_id(for_update=True)` 안이다) 기대 결과: CUBRID가 교착을 감지해 한쪽을 끊는다(`errno=-72` unilaterally aborted · 실측 확인). 일곱째(`#1868` · 파라미터 5건): **정박 구간 × 항차 X 경로 다섯**(전환 · PATCH · 실적 · 소프트 삭제 · 채택 UPDATE) — 같은 첫 세션(선박 X → INSERT 전 머묾) 사이에 항차 X를 쥐고 `voyage`를 UPDATE하는 경로를 넣는다. CUBRID 11.4는 **FK 열을 바꾸지 않는 UPDATE에도 부모 S를 요구**하므로(`#1868` ⑽-b 3/3) 잠금 순서가 항차 → 선박이면 교착이다. 저장소가 선박을 먼저 잠가 둘 다 성공(구간 1 · 항차는 기대 상태). 돌연변이(같은 선박 선잠금 제거) **실측**(2026-09-24): 5/5 실패(`errno=-968` 또는 `-72`). 세션은 `app_fresh_engine`이 갈아 끼운 것을 호출 시점에 받는다 — 모듈 수준 `get_sessionmaker` 묶음은 파일만 따로 돌리면 `_setup`이 `NOT NULL` 오류로 끝났다 |
