...
**합계 163개 파일 · 2203 함수 · 2693 수집.** (2026-09-20 실측 — #1309 2함수 추가 반영)
...
| 2026-09-20 | `#1310` | **변경 이력 표의 풀리지 않은 병합 조각 두 줄(`>>>>>>> origin/` · 그 다음 줄 `main`)을 지웠다** — PR `#1259`(`#1203`)가 충돌을 풀며 남긴 것으로, 표가 그 줄에서 끊겨 뒤의 행들이 문단으로 렌더됐는데 어떤 검사도 실패하지 않았다. 재발 방지로 §14.2 `test_tracked_files_are_text.py` 2 → **4함수** · 합계 실측 갱신(163파일·2201함수·2691수집 → **163파일·2203함수·2693수집**). 가드 둘 — ⑴ 병합 표시 줄 머리(도입 직후 `>>>>>>>` 줄을 실제로 잡았다) ⑵ **표 행 사이에 낀 표가 아닌 한 줄**(`main` 조각은 병합 표시 모양이 아니라 ⑴이 못 잡는다 — 독립 검토가 찾았다). 행 삭제·함수 추가라 `AGENTS §4.3`상 버전은 올리지 않는다 (#1309) |
