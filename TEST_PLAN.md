...
| `test_audit_client_ip.py` | 5 | **§4 API · 감사 IP 판정**(`#1889` · `#1483` 후속) — 감사용 판정이 요청 한도와 같은가(서명 헤더 맞음 → 원 IP · 틀림 → 소켓 상대) · 알 수 없으면 `unknown`이 아니라 NULL인가 · **소켓 상대 직접 참조가 판정 함수와 접근 로그 `peer` 밖에 없는가**(grep 가드) · 파일별 `_client_ip` 사본이 없는가 |
...
| 2026-09-26 | `#1926` | §14 `test_audit_client_ip.py` **신설(5함수)** · `test_audit_events_db.py` 3 → 4 · 합계 실측 갱신(218파일·2868함수·3579수집 → **219파일·2874함수·3585수집**). 감사 로그·세션 IP가 요청 한도와 같은 판정을 쓰고 소켓 상대를 직접 읽는 자리가 다시 생기지 않게 고정한다 (#1889) |
