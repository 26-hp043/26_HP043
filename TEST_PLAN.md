...
| `test_fleet_reduction_db.py` | 8 | **§4 API · 함대 감축 계획 서비스**(`#513` · `API_SPEC §2.17`) — ⚠️ **감속률 0%면 전 선박 등급이 그대로인가**(데모 선박 포함) · ⚠️ **조정 전 값이 연간 등급 관리의 결정론 예상과 같은가** · 감속 시 CII·추가 항해일·절감 톤 · 단가 누락 칸이 빈칸인가 · 모르는 선박 거부 · ⚠️ **저장본이 저장 시점 결과 그대로인가**(항차를 더한 뒤에도) · 없는 계획 404 · 실제 HTTP(봉투 · 422 · 201 · 목록 · 단건) |
...
**합계 154개 파일 · 1989 함수 · 2484 수집.** (2026-09-13 실측)
