...
| 2026-09-24 | `#___` | §14 두 행에 **돌연변이 실측**을 적는다(로컬 DB 복구 뒤 · 원복은 백업 cp와 `cmp` 동일 확인). `test_not_underway_crud_db.py`: 선박 `lock_row` 제거 시 동시성 3건 **3/3 실패**(`#1629` — PR #1852 때는 로컬 DB가 내려가 실측하지 못했다). `test_voyage_row_lock_db.py`: 항차 `for_update` 갈래 제거 시 **4/5 실패 · 채택 × 채택만 통과** — 종전 기대값 「채택 행 2건」은 이 교차 시점에서 성립하지 않았다. 이 짝이 무엇을 지키는지 다시 정하는 일은 `#1869`로 남긴다. 함수 수 변화 없음. `AGENTS §4.3`상 행 보강이라 버전은 올리지 않는다 (#1869) |
