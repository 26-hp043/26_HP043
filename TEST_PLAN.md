...
| `test_deploy_secret_shape.py` | 3 | **§5 인프라 · 시크릿 모양 진단**(`#1634`) — `preflight`의 진단 단계가 시크릿의 **끝 모양만**(끝 줄바꿈 개수 · 끝 CR · 끝 공백 — 저장소가 공개라 길이·문자 종류도 찍지 않는다) 찍고 **값은 어떤 토막도 찍지 않는가**를, 워크플로 안의 스크립트를 꺼내 가짜 값(작은따옴표·큰따옴표·`$`·역슬래시·`#`·끝 줄바꿈)으로 **실제 실행해** 본다. PR #1798을 되돌린 원인(가설: 명령 치환이 값 끝 줄바꿈을 지운다)을 확인하는 임시 단계이며, 원인을 고친 뒤 단계와 함께 지운다 |
