...
**합계 196개 파일 · 2639 함수 · 3274 수집.** (2026-09-23 실측 — #1658 정박 몫 필드 반영)
...
| 2026-09-23 | `#___` | §14 `test_deploy_sha_pinning.py` **신설(4함수)** · 합계 실측 갱신. 배포 워크플로의 원격 두 호스트가 `main` 대신 **이 실행의 커밋**을 받아 가게 하고(`#1633`), 받은 것이 다르면 배포를 멈추게 했다. 종전에는 이미지에 `GITHUB_SHA`를 붙이면서 원격은 브랜치를 받아, **빌드 뒤 머지된 코드가 이미지와 함께 돌 수 있었다.** 멈추면 이전 컨테이너가 남으므로 서비스는 살아 있다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1633) |
