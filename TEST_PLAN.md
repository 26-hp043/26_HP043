...
| 문서명 | TEST_PLAN.md |
| 버전 | v1.32 |
| 상태 | **§5.10 격리 수준 전제 가드 (#1796) + §5.6·§4.7 활성 키 유니크 인덱스·동시 등록 (#1631)** + Oracle Review + 외부 리뷰 반영 + Layer 1 픽스처 정본값 규칙 반영 (#166) + v1.4에서 §1.3 케이스 스키마 기호 표기 전환 (#46) + §4.7 인증 API 케이스 (#279) + **v1.6에서 방향 전환 반영 — 신규 서브시스템 5절 · §14 파일 인벤토리 · §11 실측 정정 (#394)** + **§3.25~3.27 신설 · §10 실CI 재작성 · §14.3~14.5 갱신 (#1104 · #1081)** + **§4.11 올해 누적 CII 추이 API (#1671)** |
| 최종 수정일 | 2026-09-26 |
...
| IT-STATE-009 | **동시 쓰기의 직렬화** | 두 연결이 같은 항차에 전환 × 전환·PATCH·실적·삭제·채택을 **커밋 직전에** 교차 · 정박 구간(선박 X) × 채택 `CREATE_NEW_VOYAGE`(항차 X)를 **INSERT 전에** 교차 · 정박 구간(선박 X) × 항차 X 경로 다섯(전환·PATCH·실적·삭제·채택 UPDATE)을 **`voyage` UPDATE 전에** 교차 | 뒤 요청은 앞 커밋 뒤의 상태로 판정 — 종결 상태 덮어쓰기 없음 · 채택 행 하나(전환 네 짝은 `#1626`의 항차 잠금이, 채택×채택은 `UPDATE_EXISTING_PLAN`의 공용 `voyage` 행 쓰기 잠금이 지킨다 — `#1869`) · 교차 잠금이 교착하지 않음 (`#1860` · `#1868`) |
...
> vessel 및 voyage의 `is_deleted` 플래그 + 활성 키 유니크 인덱스(`061` · `DB_SCHEMA §2.1`) 동작 검증.

| TC ID | 테스트 | 입력 | 기대 결과 |
|---|---|---|---|
| IT-SOFTDEL-001 | 소프트 삭제 후 조회 제외 | vessel DELETE (soft) | GET /vessels 응답에 미포함 |
| IT-SOFTDEL-002 | 삭제 후 IMO 번호 재사용 | vessel A soft-delete → 동일 IMO로 신규 등록 | 등록 성공 (삭제 행의 `imo_active`가 NULL이라 유니크 인덱스가 허용) |
...
| TC ID | 테스트 | 입력 | 기대 결과 |
|---|---|---|---|
| IT-MAP-001 | 네 좌표와 두 항구 이름이 그대로 실린다 | 좌표가 있는 진행 중 항차 | 출발·도착 4값 (6자리) + `departure_port_name`·`arrival_port_name`(`#1882` — 지도의 항구 핀과 낭독) |
...
### 3.29 공적 기록으로 채우기 (`test_public_record_fill_db.py`) [#1923]

> `PRD §17.4.4` 「이 값으로 채우기」 · `API_SPEC §3.12`. 데이터 점검의 「공적 기록과 다름」 행에서 **사용자가 누른 칸 하나만** 공적 재항 기록의 시각으로 바뀌고, 그 출처가 행(`*_source` · 마이그레이션 `064`)과 감사 로그(`VOYAGE_ACTUALS_FILL`)에 남는지를 실제 DB와 HTTP로 본다. 확정 항차의 되돌리기·채우기·감사가 **한 트랜잭션**인지가 뼈대다 — 화면이 두 요청으로 보내면 되돌려진 채 옛값이 남는다.

| ID | 테스트 | 입력 | 기대 결과 |
|---|---|---|---|
| IT-FILL-001 | 출처 열 값 트리거 (`064`) | INSERT·UPDATE에 `COORDINATE_ESTIMATE` · `public_record` · `AIS` · `""` | 트리거 REJECT. 스키마의 `ACTUAL_TIME_SOURCES` 전부와 NULL은 통과(두 목록이 갈리면 500) · 트리거 여덟이 실재 |
| IT-FILL-002 | 누르지 않으면 바뀌지 않는다 | 데이터 점검 조회(HTTP) | 시각·출처 그대로 · 감사 행 없음. 어긋남에 `voyage_status` · `call_year` · `call_seq` · `fetched_at` · `period_id`가 **HTTP 응답에** 실린다 |
| IT-FILL-003 | 완료 항차의 한 칸 채우기 | `COMPLETED` · `DEPARTURE` | 200 · 출항만 공적 기록 값 · 출처 `PUBLIC_RECORD` · **도착과 그 출처는 그대로** · 상태 그대로 · `VOYAGE_ACTUALS_FILL` 한 행(칸 · 이전 값 · 새 값 · 기항 열쇠) · 응답에 감사 재료(`_audit`)가 새지 않는다 · 다시 조회하면 그 어긋남이 사라진다 |
| IT-FILL-004 | 확정 항차는 동의 표시가 있어야 | `CONFIRMED` · `revert_confirmed` 없음 / 있음 | 없으면 422 · 아무것도 안 바뀜 · 감사 없음. 있으면 200 · `COMPLETED`로 남음 · `VOYAGE_TRANSITION`(CONFIRMED → COMPLETED)과 `VOYAGE_ACTUALS_FILL` 두 행 |
| IT-FILL-005 | 감사가 실패하면 아무것도 남지 않는다 | `CONFIRMED` + `revert_confirmed` · `insert_event` 예외 주입 | 500 · 상태 `CONFIRMED` 그대로 · 시각·출처 그대로(`IT-AUDIT-004`와 같은 성질) |
| IT-FILL-006 | 거절은 값을 바꾸지 않는다 | 없는 기록 · 다른 배의 기록 · 화면이 본 값과 다른 `recorded_at` · 항차 칸에 `period_id` | 404 · 422(`field = record`) · 409 · 422 — 네 경우 모두 시각·출처·감사 그대로 |
| IT-FILL-007 | 사람이 고치면 출처가 풀린다 | 채운 뒤 실적 입력(`§3.6`)으로 **같은 시각** 재전송 · **다른 시각** · 사람이 `PUBLIC_RECORD`를 주장 | 같은 시각이면 출처 유지 · 다른 시각이면 `null` · 주장은 422(서버만 붙인다 — 결정 A) |
| IT-FILL-008 | 정박 구간 시작 채우기 | `BERTH_START` · `period_id` 없음 / 있음 · 뒤이어 구간 수정(`§2.11`)으로 다른 시각 | 없으면 422 · 있으면 그 구간의 `started_at_source`만 `PUBLIC_RECORD` · 수정 뒤 `null` · 감사 `target = not_underway_period` |
...
| AT-AUTH-019 | **동시 중복 가입** (`#1631`) — 같은 이메일로 거의 동시에 `POST /auth/signup` 둘(첫 요청이 flush 뒤 커밋 전에 머무는 사이 두 번째가 들어온다 · 스레드 둘) | **한쪽 201 · 다른 쪽 409 `CONFLICT`** + `AT-AUTH-005`와 **같은 정본 문구**(`PRD §6.3`) · 계정 1개 · 패자는 세션 발급에 이르지 못한다. 사전 중복 확인은 남의 미커밋 행을 못 보므로(`#1796`) 막는 것은 활성 키 `email_active`의 유니크 인덱스(`061`)이고, 라우트가 그 위반을 같은 409로 바꾼다 (`test_active_key_unique_db.py`) |
...
| DB-CHK-023 | 속력 물리 상한 60kn (`vessel.reference_speed_kn` · `voyage.planned_speed_kn`·`actual_avg_speed_kn` · `voyage_scenario.speed_kn` · 트리거 `trg_chk_*speed_max*_ins/upd` · 062) [#1269] | reference_speed_kn = 60.01 · 125 · 9999.99 / voyage 속력 UPDATE 120 | 트리거 REJECT. 60 정각 · 58.1(고속선 실측값) · NULL은 통과 |
...
> 활성 키 열(`vessel.imo_active` · `app_user.email_active` — 활성이면 원본의 사본, 삭제면 NULL)의 유니크 인덱스 동작 검증(`061` · `#1631`). PostgreSQL의 partial unique index(`WHERE is_deleted = false`)를 CUBRID가 하는 형태로 옮긴 것이다. `047`의 트리거는 `NOT EXISTS`가 미커밋 행을 못 봐 동시 등록을 막지 못했다(`#1796` 실측).
>
> `DB-SOFT-003`~`005`는 `test_active_key_unique_db.py`. 동시 가입은 `§4.7 AT-AUTH-019`. `DB-SOFT-007`(마이그레이션 사전 검사)은 `test_active_key_precheck.py`(DB 없이 · 스텁) + `test_zz_active_key_precheck_db.py`(실제 CUBRID).

| TC ID | 테스트 | 입력 | 기대 결과 |
|---|---|---|---|
| DB-SOFT-001 | 소프트 삭제 후 동일 IMO 등록 | vessel A soft-delete → 동일 IMO 신규 등록 | 등록 성공 (삭제 행의 활성 키가 NULL이라 유니크 인덱스가 허용) |
| DB-SOFT-002 | 소프트 삭제 vessel이 unique 위반 유발 | 동일 IMO, 둘 다 is_deleted=false | `IntegrityError` — 위반은 061 채움 트리거 액션 안에서 나므로(errno `-528`) `cubrid_errors`가 옮긴다 · 메시지에 인덱스 이름 `uq_vessel_imo_active` |
| DB-SOFT-003 | **활성 키는 DB가 채운다** — 앱이 넣지 않은 INSERT · soft delete · 되살리기 · IMO 변경 · 엉뚱한 값을 넣은 INSERT | raw SQL (서비스를 지나지 않는다) | `imo_active` = 활성이면 `imo_number`, 삭제면 NULL — 어느 경로로 써도 같다. `app_user.email_active`도 같은 구조 |
| DB-SOFT-004 | **동시 등록 경합** — 두 세션이 같은 IMO를 교차 등록 (첫 세션이 flush 뒤 커밋을 미룬 사이 두 번째가 INSERT) | `create_vessel` × 2 (`test_auth_tokens.py`의 두 세션 교차 틀) | **정확히 하나만** 저장 · 패자는 사전 조회와 **같은 409 `CONFLICT` · 같은 문구**(`ConflictError`) — 유니크 인덱스를 빼면 행 2개, 예외 변환을 빼면 `IntegrityError`가 새어 실패한다(돌연변이) |
| DB-SOFT-005 | **NULL 다중 허용** — 삭제 행 셋 + 활성 하나 (선박·계정 각각) | 같은 키의 삭제 행 여러 개 | 전부 저장(4행) · 활성 둘째만 거부 — 「NULL을 하나만 허용하는」 인덱스로 바꾸면 여기서 드러난다 |
| DB-SOFT-006 | **중복이 아닌 무결성 위반은 409로 둔갑하지 않는다** | `insert`가 다른 인덱스의 위반을 올리게 흉내 | `IntegrityError` 그대로 (`ConflictError` 아님) — 인덱스 이름으로 가른다(`cubrid_errors.violated_unique_index`) |
| DB-SOFT-007 | **`061` 사전 검사 — 중복이면 아무것도 바꾸기 전에 멈춘다** (`#1631` G-1 「다′」) | `060`에서 같은 IMO의 활성 행 둘(`047` `_ins` 트리거를 잠시 걷고 만든다) → `upgrade head` | 0이 아닌 종료 코드 · 한국어 문구에 **표별 그룹 수**와 `OPERATIONS §3.6.4`만 있고 **IMO·이메일 값은 없다**(배포 로그는 공개 저장소에 남는다) · **DB는 그대로**: `047` 트리거 4개 유지 · `imo_active` 열 없음 · 한쪽을 소프트 삭제하면 같은 명령이 지나간다. 스텁 검사는 두 표 0이면 `047` 트리거 4개를 걷는 것까지 본다(늘 멈추는 검사가 아니다) |
...
### 5.10 격리 수준 전제 가드 (`test_db_isolation_premise_db.py`) [#1796]

> 저장소의 동시성 방어 세 갈래 — `#1079` 조건부 UPDATE · `#1630` 부모 행 `FOR UPDATE` · `#1631` 유니크 인덱스 — 가 전부 **「READ COMMITTED에서 미커밋 변경은 보이지 않고 읽기를 막지도 않는다」** 위에 서 있다. 서버 설정(`isolation_level` · `lock_timeout`) · 드라이버 인자 · CUBRID 판올림으로 전제가 바뀌어도 알려 주는 검사가 없었다. `#1796`이 실측한 다섯 현상을 **값이 아니라 동작으로** 잠근다 — 앱이 쓰는 세션 팩토리로 세션 둘을 교차시키고, B 쪽은 `SET TRANSACTION LOCK TIMEOUT 1`로 대기 여부를 시간으로 판정한다(0.5초 안이면 「대기 없음」). 표는 `probe_iso_*`이며 파일이 만들고 지운다.

| TC ID | 테스트 | 기대 결과 |
|---|---|---|
| DB-ISO-001 | A의 미커밋 INSERT를 B가 읽는다 (일반 SELECT · 같은 키 `FOR UPDATE`) | **보이지 않고 기다리지도 않는다**(결과 없음 · 0.5초 안) — `#1631` 트리거·사전 조회가 중복을 못 막은 이유 |
| DB-ISO-002 | A의 미커밋 UPDATE를 B가 읽는다 | 일반 SELECT는 **옛 값** · 대기 없음. 같은 행 `FOR UPDATE`는 잠금에 걸려 lock_timeout 뒤 `errno=-75` — 쓰기 잠금은 실재한다(`#1630`의 전제) |
| DB-ISO-003 | 범위 SELECT → INSERT 교차 (`#1629` 형태) | 잠금 없이는 둘 다 「겹침 없음」을 읽어 **2건** · 부모 행 `FOR UPDATE`면 B가 A의 커밋까지 기다린 뒤 겹침을 보고 물러나 **1건** — 두 갈래가 서로의 대조군 |
| DB-ISO-004 | 유니크 인덱스에 같은 키를 교차 INSERT | B의 INSERT가 A의 커밋까지 **대기**한 뒤 `IntegrityError`(`errno=-670`) · 같은 키 1행 — `#1631`이 기대는 성질 |
| DB-ISO-005 | 조건부 UPDATE(`WHERE status = 'DRAFT'`)가 미커밋 UPDATE 뒤에 온다 (`#1079` 형태) | A의 커밋 뒤 **최신 값으로 재평가**돼 `rowcount = 0` · 조건 없는 UPDATE(ORM 기본)는 `rowcount = 1`로 마지막 쓰기가 이긴다 |
| DB-ISO-006 | 부모 X 보유 중 자식 INSERT (`#1860` ⑻) | 부모 행의 **S 잠금을 기다린다** — `errno=-75`가 부모 표 이름을 싣는다 |
| DB-ISO-007 | 자식 INSERT 뒤 커밋 전에 머문다 → 다른 세션이 그 부모 `FOR UPDATE` (`#1868` ⑽-a) | **기다린다** — FK 검사의 S는 문장 뒤가 아니라 커밋까지 남는다 |
| DB-ISO-008 | 부모 X 보유 중 자식의 **비-FK 열** UPDATE (`#1868` ⑽-b) | **기다린다** — FK 열을 바꾸지 않아도 재검사한다. 대조군: 다른 부모의 자식 UPDATE는 대기 없음 |
...
| `test_active_key_precheck.py` | 3 | **§5.6 `DB-SOFT-007` · `061` 사전 검사**(`#1631`) — DB 없이 돈다. `migration_stub`으로 `alembic`을 갈아 끼우고 중복 그룹 조회에 「있다」로 답하게 한 뒤 `upgrade()`를 **실제로 불러** 예외 전에 `DROP TRIGGER`가 하나도 세어지지 않았는지 본다 · 두 표 0이면 `047` 트리거 4개를 걷는다 · `duplicate_message`에 그룹 수·절차만 있고 `@`·7자리 수가 없다 |
| `test_active_key_unique_db.py` | 11 | **§5.6 DB · `DB-SOFT-003`~`006` 활성 키 유니크 인덱스 + §4.7 `AT-AUTH-019` 동시 가입**(`#1631` · 마이그레이션 `061`) — 세 층을 따로 잠근다 ⑴ **DB 층**(`conn` 롤백 세션): 트리거가 `imo_active`·`email_active`를 채우고(INSERT) 비우고(soft delete) 되살리고(복원) 따라가며(IMO 변경) 엉뚱한 값을 바로잡는가 · 삭제 행 여러 개 + 활성 하나는 되는가(NULL 다중) · 활성 둘은 `IntegrityError`이고 `violated_unique_index`가 **인덱스 이름**을 집는가 ⑵ **경합 층**(세션 둘 · 각자 커밋): 첫 세션이 flush 뒤 `commit`을 붙들고 있는 사이 두 번째가 `create_vessel` → 정확히 하나만 저장, 패자는 사전 조회와 같은 `ConflictError` 문구 · 회원가입은 TestClient를 **스레드 둘**로 보내고 `_issue_session`을 가로채 커밋 전에 머물게 한다 → 201 + 409(정본 문구) · 계정 1개 ⑶ **계약 층**: 다른 인덱스의 위반은 409로 둔갑하지 않는다. 교차가 실제로 「flush 뒤 커밋 전」에서 났는지는 **스파이**(`vessel_repo.insert` 호출 수 · 가입은 `is_initial_admin` 호출 수 = 사전 확인을 지난 횟수)로 단언하고 첫 세션은 그 신호를 이벤트로 받아 커밋한다 — 사전 조회의 409로 끝나면 실패한다. 유니크 인덱스를 빼면 행 2개, 예외 변환을 빼면 `IntegrityError` 누출로 실패하게 짰다(돌연변이 실측은 로컬 DB 복구 뒤) |
...
| `test_audit_events_db.py` | 4 | §3 통합 · 감사 로그 · **서명 헤더가 맞으면 감사 로그·세션에 원 IP, 틀리면 소켓 상대**(`#1889`) |
...
| `test_compose_env_wiring.py` | 20 | **§5 인프라 · 배포 배선** — compose가 `.env`를 컨테이너에 주입하는지, `environment:`가 `DATABASE_URL`을 덮는지 (`#508`) · **`.env.example`이 `APP_ENV`를 설정하지 않는지**(`#810`) · **프로덕션 `app`이 호스트 포트를 열지 않는지**(`#811`) · **본보기가 앱이 읽는 변수를 전부 적는지 — 상수를 경유해 읽는 것까지**(`#1290`) · **`.env.app.example`이 적는 값을 OCI 분리 토폴로지 compose가 실제로 쓰는지**(`#1290` — 그 파일에는 `env_file:`이 없어 `environment:`에 없으면 `.env`에 채워도 닿지 않는다) · **자동 배포가 `APP_ENV`를 렌더링하는지(기본 staging)**(`#1201` — 첫 자동 배포가 이 값 없이 `production`으로 떨어져 SMTP 가드에서 죽었다. 수동 절차 §3.3은 `staging`을 적었다). 뒤의 셋이 조용하다 — 본보기를 그대로 `.env`로 복사하면 `docker-compose.prod.yml`의 `${APP_ENV:-production}` 치환이 그 값을 읽어 **프로덕션 스택이 development로 뜨고**, `:8000`이 열린 채 `#786` ⑵가 `USE_FORWARDED_FOR=true`로 바꾸면 공격자가 그 포트에 직접 붙어 `X-Forwarded-For`를 위조해 **요청 한도를 완전히 우회**한다. 개발 compose의 `8000:8000`은 그대로 유지되는지도 함께 본다 · **OCI 분리 토폴로지의 두 구멍**(`#1331` — `SMTP_PORT`를 빈 값이 아니라 `587`로 넘기는지 · `LOG_FILE`과 **이름 있는 로그 볼륨**이 있는지 · 그 경로가 `docs/OPERATIONS.md §8.2.1`이 적는 경로와 같은지). 둘 다 단일 호스트 compose에는 있었고 분리 토폴로지 파일만 빠져 있었다 — 「문서대로 했는데 안 된다」에 이르는 길이다 · **DB 포트 게시가 db-01 사설 IP에만 붙는지**(`#1641` — 종전 `"33100:33000"`은 모든 인터페이스에 열렸고, 그 노출을 막는다던 ufw는 Docker가 publish한 포트에 관여하지 않는다(패킷이 FORWARD 체인으로 흘러 INPUT을 거치지 않는다). 게시 항목이 `IP:호스트:컨테이너` 세 토막인지 · IP가 `${OCI_DB_PRIVATE_IP…}` 치환인지 · 그 치환이 **필수(`:?`)**인지를 본다 — `:-`면 값이 빌 때 `:33100:33000`이 되어 다시 모든 인터페이스로 열린다 · `.env.db.example`이 그 compose의 치환 변수를 전부 적는지). 돌연변이 2종(두 토막 복원 · `:?`→`:-`) **2/2 검출** |
| `test_cubrid_volume_guard.py` | 8 | **§5 인프라 · 배포 배선** — (2026-09-25 이후) 두 compose 파일이 `cubrid-data`를 **이미지의 데이터 경로**에 붙이는가 · 배포 DB 단계가 `up -d` **전에** 「데이터가 익명 볼륨에 있고 `cubrid-data`는 비었다」를 보고 멈추는가(같은 compose 프로젝트의 볼륨만 본다 — 같은 호스트의 ourtax) · 마운트 줄 근처 주석이 옮기는 절차(`OPERATIONS §9.2.1`)를 가리키는가. 아래는 옮기기 전(09-24)의 서술이다 — CUBRID 데이터가 명명 볼륨이 아니라 이미지가 선언한 익명 볼륨에 있다는 것을 두 자리에서 잠근다(`#1867`). ⑴ `docker-compose.yml`·`docker-compose.prod.db.yml` 양쪽의 `cubrid-data:/var/lib/cubrid` 마운트 근처에 「익명 볼륨」·`#1867`을 담은 위험 주석이 있는가(지우면 다음 사람이 「명명 볼륨이니 안전하다」고 다시 믿는다) ⑵ `deploy.yml`의 db-01 단계가 `docker compose … down`을 **`FORCE_DB_INIT` 분기 밖에서** 쓰지 않는가 — if/fi 중첩을 스택으로 추적한다. 밖에서 쓰면 평소 배포(재생성 경로)마다 새 익명 볼륨이 생겨 매번 데이터가 사라진다. ⚠️ 가드 자체가 사라져 검사가 공허하게 통과하지 않도록 `down -v`가 실재하는지도 함께 본다. DB 없이 파일만 읽는다(`test_deploy_sha_pinning.py`와 같은 근거 — 배포 워크플로는 실행하면 운영 서버를 건드린다) |
...
| `test_deploy_env_rendering.py` | 4 | **§5 인프라 · 배포 배선** — 배포가 렌더하는 `.env`가 **compose가 쓰는 값을 빠뜨리지 않는지** (`#1475`). 체인은 세 고리인데(`본보기 → compose → deploy.yml의 .env 렌더`) 검사는 **둘까지만** 있었다 — `#1290`이 첫 고리를 메우고 멈췄고, 그래서 `INITIAL_ADMIN_EMAILS`가 본보기에도 compose `environment:`에도 있는데 **배포가 쓰지 않아 항상 빈 값**이었다. ⚠️ **`cat > .env`가 통째로 덮어쓰므로** app-01에 손으로 적어 두어도 다음 배포가 지운다 — `#508`·`#1290`·`#1331`과 같은 「적어 뒀는데 아무 일도 없다」 계열이다. ⚠️ **증상이 환경에 따라 갈린다** — `production`이면 기동 거부로 배포 로그에 빨갛게 남지만 **`staging`이면 조용히 관리자 0명으로 뜬다**(`role_bootstrap` 독스트링이 그 공백을 미리 적어 두었다). 배포본은 `staging`이라 **더 조용한 쪽**이었다. 옛 이름 `INITIAL_OFFICE_EMAILS`를 **렌더하지 않는 것**도 함께 본다 — 렌더하면 이름이 바뀐 것을 알리는 기동 거부가 무의미해진다(`#1301`). 돌연변이 3종 **3/3 검출** · **db-01 렌더도 같은 세 고리**(`#1641` — `docker-compose.prod.db.yml`이 치환하는 값(`OCI_DB_PRIVATE_IP` — CUBRID 게시의 바인드 주소)이 `deploy-db` 잡의 `env:` → `ssh … NAME='${NAME}'` 원격 전달 → `cat > .env` heredoc 셋에 다 있는지. 둘째 고리에서 빠지면 렌더 줄은 있는데 **빈 문자열**을 적는다). 돌연변이 2종(렌더 줄 삭제 · ssh 전달 삭제) **2/2 검출** |
...
| `test_data_quality_db.py` | 23 | **§4 API · 데이터 점검 서비스**(`#513` · `API_SPEC §2.16`) — 네 심각도가 실제 행에서 갈리는가 · ⚠️ **CII 영향이 같은 함수로 따로 구한 두 누적값의 차와 같은가** · 실적·계획 모두 빈 행이 대체가 아니라 계산 불가인가 · 이상치를 계산에서 빼지 않는가 · 실적 미입력은 완결성에서 빼지 않는가 · **이상치 항차는 완결성의 실측에서 빼는가** · 선박 단위 사유가 `§2.8`과 같은 어휘인가 · 그룹 순서 · 응답 필드 집합(영향 블록이 있는 행과 없는 행 둘 다) · **실제 HTTP 경로**(인증 401 · 봉투 · 연도 범위 422) · `#1532` — 완결성의 **분자·분모·제외 내역**(`completeness`)이 응답에 실리고 「실측 + 제외 합 = 누적」이 성립하는가 · 유일한 항차가 이상치일 때 0%가 이상치 축으로 설명되는가 · 두 심각도가 겹친 항차는 **한 축에만** 들어가는가(계산 불가 > 대체 > 이상치) · 비율이 없으면 내역도 없는가 · **공적 기록과 다름**(`#1197` · `PRD §17.4.4`) — 출항 12시간 어긋남이 뜨고 출처·받은 시각·차이를 싣되 완결성은 그대로인가 · 6시간 정각과 호출부호 없는 배는 띄우지 않는가 · 항차에 매인 정박 구간 시작은 입항과 견주고 드라이독 구간은 견주지 않는가 |
...
| `test_request_bounds.py` | 8 | **§5 API · 입력 경계**(`#1086`) — **속력 여섯 필드는 저장 형식이 아니라 VAL-009의 1.0~60**(`#1269` — 실시간 CII `speed_kn`만 상한이 없던 것 포함)이고 모델 CHECK 네 개·마이그레이션 `062`가 같은 60을 쓰는가 · 60.01·125는 422 · 항차·시나리오·정박 요청 15필드의 스키마 경계가 **ORM 컬럼 `Numeric(p,s)`와 같은가**(`#860`의 선박 검사와 같은 방식) · `regulation_year`가 DB CHECK(2019~2050)와 같은가 · 종전 500이던 항차 5입력(연도 2010/2080 · 거리 0.001/초과 · 속력 10000)이 422 · 정박 연료 0·0.001 거부 · 소요시간 0.01h 미만은 계산 단계에서 422(`_db_rows`) |
...
| `test_deploy_host_key_pinning.py` | 9 | **§5 인프라 · 배포 SSH 호스트 키 고정**(`#1637` · `F-11`) — 배포가 실행 중에 `ssh-keyscan`으로 받은 키를 믿지 않는가. 종전에는 배포 직전에 원격의 공개키를 받아 `known_hosts`에 넣어, 경로에서 잘못된 키가 주입돼도 **같은 연결 흐름이 그 키를 신뢰**했다. 지금은 사람이 두 경로(서버 콘솔 · 다른 경로의 `ssh-keyscan`)의 지문을 대조해 커밋한 `ops/host/known_hosts`를 러너의 `~/.ssh/known_hosts`로 **덮어쓰고** `-o StrictHostKeyChecking=yes`로 접속한다. 파일에서 보는 것 — `ssh-keyscan`이 사라졌는가 · `ssh` 호출 둘 다 엄격 검사인가 · 두 잡이 저장소를 받아 파일을 **복사(이어 붙이지 않고)**하는가 · 파일에 호스트가 없으면 `ssh-keygen -F`가 1로 끝나 **`ssh` 호출 전에 `exit 1`**인가 · 멈추는 메시지에 시크릿 참조가 없는가 · 항목이 `<IP> ssh-ed25519 <키>` 모양이고 해시되지 않았는가 · `ssh-keygen -lf`가 모든 항목의 지문을 내는가(한 글자만 깨져도 실패) · 🔴 **db-01·app-01 두 줄이 있는가** — 비어 있는 채 머지되면 머지 즉시 도는 배포가 `SSH 설정`에서 서므로 이 검사가 머지를 막는다. 배포는 실행해야 재현되는데 그 실행이 운영 서버를 건드리므로 파일에서 본다. 돌연변이 4종(키 한 글자 훼손 · `ssh-keyscan` 복원 · 엄격 검사 제거 · `exit 1` 제거) **4/4 검출** |
| `test_deploy_freeze.py` | 5 | **§5 인프라 · 시연 동결**(`#789` · 결정 E-2) — 배포 입구 둘(`preflight` · `deploy-frontend`)이 **push이면서 `DEPLOY_FROZEN == 'true'`일 때만** 건너뛰는가 · 수동 실행(예외 배포)은 막지 않는가 · 백엔드 뒤 잡이 `preflight`에 걸려 함께 건너뛰는가 · 건너뛴 사실을 남기는 `frozen` 잡 · 운영 문서의 켜고 끄는 자리 |
| `test_ops_workflow.py` | 14 | **§5 인프라 · 운영 워크플로**(`#788` · `#789` · `#1635`) — `ops.yml`의 원격 스크립트가 `bash -n`을 통과하는가(9개 이상) · 러너 쪽 셸도 · `restore`는 `confirm=cii`가 없으면 시작하지 않고 교체가 실패하면 앱을 켜지 않는가 · 롤백 실습은 실패해도 `trap`으로 원래 이미지에 돌아오고 `alembic/` 변경이 섞이면 시작하지 않는가 · 배포와 **같은 동시 실행 그룹**인가 · 수동 실행 전용인가 · 백업 아티팩트는 **암호화한 파일만**(저장소가 공개 · `#788` 정정 결정 A — 암호 시크릿이 없으면 받지도 올리지도 않고 평문은 올리기 전에 지운다 · 보존 14일 · **푸는 법 안내의 `-iter`가 암호화와 같은가** — `ops.yml` 주석과 `OPERATIONS §3.8` 둘 다) · `restore`가 **리허설한 바로 그 덤프**로 교체하는가 · `app-start` 작업 · `shell: bash`(pipefail) · 파괴적 명령(`down -v` · `force_db_init` · `volume rm/prune` · `deletedb`)이 없는가 · 백업이 **root 소유 `backups/`를 덤프 전에 되돌리는가**(쓸 수 없을 때만 `sudo -n chown`) · 점검이 **db 컨테이너 상태**(`compose ps` — `#1634` 헬스체크)를 보이는가 · **챗봇 감사 흐름**이 본문 표·사용자·IP를 고르지 않고 해시를 지우는가(`#1535` · 공개 로그). 실행하면 운영 서버를 건드리므로 파일에서 본다 |
| `test_deploy_backend_binding.py` | 3 | **§5 인프라 · 백엔드 외부 노출**(`#786`) — `docker-compose.prod.app.yml`의 backend가 **`127.0.0.1:`에만 게시**되는가 · 배포 헬스체크가 공인 IP `:8001`이 아니라 **터널 주소**(`bluelog-api.kpubdata.com`)로 묻는가(게시만 좁히면 모든 배포가 헬스체크에서 실패한다) · 배포마다 공인 `:8001`이 닫혀 있는지 확인하고 **응답이 오면 실패**하는 단계가 있는가. 종전 `"8001:8000"`은 모든 인터페이스에 열려 터널과 프록시 서명 헤더(`#1483`)를 거치지 않는 길이 됐다. 배포는 실행하면 운영 서버를 건드리므로 파일에서 본다 |
| `test_deploy_map_assets.py` | 4 | **§5 인프라 · 배포 지도 배선**(`#1909`) — 배포된 화면에서 **지도가 실제로 뜨는** 배선 둘을 잠근다. ⑴ **Cloudflare Pages가 `Range`를 무시한다**(실측 2026-09-25: `bytes=0-6` → `200`·전체 15 MB·`accept-ranges` 없음 · `/favicon.svg`도 같다). 타일은 PMTiles 아카이브 **파일 하나**에서 조각을 읽으므로(`#985`) byte serving이 없으면 `pmtiles`가 *「Check that your storage backend supports HTTP Byte Serving」*으로 멈추고, `hasBasemap()`은 `206`이 아니면 「자산 없음」으로 접어(`#1144`) 화면이 **개략도로 떨어진다.** `functions/basemap/[[path]].ts`가 그 경로에 `206`을 채우는지 · 순수부(`_byteRange.ts`)와 그 검사가 있는지 · `416`으로 답하는지(그래야 `pmtiles`가 크기를 보고 다시 묻는다)를 본다. ⑵ **빌드가 maplibre 타일 워커를 내보내는지** — v6는 워커 주소를 런타임에 조립하고(이름이 삼항 연산) 번들러가 그것을 정적으로 읽지 못해 자산이 통째로 빠졌다. 없는 자산은 SPA 폴백이 `index.html`을 `200`으로 돌려주므로 워커가 HTML을 파싱하다 죽는데, `#1144`가 적은 그대로 **maplibre는 `error`를 내지 않고** 지도는 회색 사각형이 되며 타일 요청이 한 건도 나가지 않는다. 지도를 만드는 두 곳(`mapLibreRenderer`·`playbackRenderer`)이 `ensureMapLibreWorker()`를 먼저 부르는지 · 그 모듈이 `?worker&url`과 `setWorkerUrl`을 쓰는지 · **CI가 산출물에서 워커를 확인하는지**를 본다. ⚠️ **개발 서버는 Range를 지원하고 워커 파일을 패키지에서 그대로 서빙한다**(`vite.config.ts`의 `optimizeDeps.exclude` · `#1144`) — 그래서 `e2e/map-smoke.spec.ts`까지 통과했다. **개발과 배포가 갈리는 자리**라 `#1322`·`#1290`·`#1331`과 같은 조용한 배선 결함이다 |
| `test_ops_scripts_host_python.py` | 5 | **§5 인프라 · 운영 스크립트 실행 환경**(`#1915`) — 문서가 **호스트에서 `python3 scripts/…`로 돌리라고 적는** 스크립트가 그 호스트에서 실제로 서는가. `docs/OPERATIONS.md §9.2.1`의 DB 이전 **1단계(백업)**가 그대로는 실패했다 — `datetime.UTC`는 3.11+인데 운영 호스트는 Ubuntu 22.04 · **3.10.12**다(app-01·db-01 실측). 같은 덫이 `purge_expired.py`에도 있었고, 그쪽은 `README`가 **crontab 한 줄**로 돌리라 적는 자리라 **실패해도 사람이 보지 않는다.** ⚠️ **조용한 이유** — `requires-python = ">=3.12"`는 **패키지**의 요구사항이고 이 스크립트들은 패키지에 들어가지 않는다(프로덕션 이미지는 wheel만 담아 `scripts/`가 없다). CI도 개발 PC도 3.12라 **호스트에서 돌려 봐야만 드러난다.** 셋을 본다 — ⑴ `ast.parse(feature_version=(3, 10))`로 **문법**이 서는가 ⑵ 3.11+ 표준 라이브러리 **이름**을 쓰지 않는가(문법은 3.10인데 이름만 새것인 경우가 이번 사고다) ⑶ `cii_platform`을 import하지 않는가(호스트에 설치돼 있지 않다). 가드 자신이 **그 사고를 잡는지도** 되돌려 확인한다 |
| `test_deploy_secret_transport.py` | 8 | **§5 인프라 · 배포 시크릿 전달**(`#1634`) — 시크릿이 원격 셸과 `.env`를 **바이트 그대로** 건너는지 본다. 종전에는 시크릿을 ssh 명령줄에 `'${SECRET}'`로 이어 붙이고 원격이 `cat > .env <<EOF`로 **따옴표 없이** 적어, 값에 `'`가 있으면 **ssh 인용이 끊겨 배포가 섰고** `$`·` #`·개행이 있으면 compose가 `.env`를 읽으며 `${…}`를 치환하고 ` #` 뒤를 주석으로 잘라 **값이 조용히 바뀌었다**. 지금은 러너가 `emit`(compose dotenv 큰따옴표 규칙)으로 `.env`를 만들어 **base64로** 넘긴다. 가짜 값 9종(따옴표 · `$(…)` · `${…}` · ` #` · 역슬래시 · 탭 · 개행 · 비ASCII · 빈 값)의 왕복을 우리 역변환과 **실제 compose 컨테이너**(docker가 없으면 건너뛴다) 양쪽으로 대조하고, ssh 명령줄의 원문 시크릿 · base64 값의 `::add-mask::`(할당마다) · `umask 077` 뒤 해독 · `set -x` 부재를 정적으로 본다. 돌연변이 4종 **4/4 검출** · 🔴 **db-01 원격이 푼 `CUBRID_PASSWORD`가 끝 줄바꿈까지 같은가**(러너 할당과 원격 해독 줄을 워크플로에 적힌 그대로 bash로 — `pw\n` · `pw\n\n` · `pw` · 따옴표·`$`·역슬래시). `#1798`은 원격에서 `"$(… | base64 -d)"`로 풀어 **명령 치환이 끝 줄바꿈을 지웠고**, 운영 `CUBRID_PASSWORD`는 끝에 줄바꿈이 1개라(`#1944` 진단) 브로커 대기 60회가 전부 실패해 `#1876`으로 되돌렸다. 되살리면서 끝에 `x`를 붙였다 떼는 해독으로 고쳤고, 원래 모양으로 되돌리는 돌연변이를 2/4 값에서 잡는다 |
...
| `test_port_calls_mof.py` | 10 | **§5 외부 연동 · 공적 재항 기록 제공자**(`#1197` B단계 ①) — 해양수산부 선박운항정보 **실제 응답 표본**(2026-09-25 · 부산 `020` · 2척)을 기항·신고로 읽는가 · 묘박지로 들어와 부두에서 나간 기항을 한 체류로 보는가(결정 G-7 ⓐ — 가장 이른 입항 ~ 가장 늦은 출항) · `최초`·`최종`이 섞이면 `최종`만 · GT는 `grtg` 먼저이고 0은 값이 아니다 · 조회를 달력 월로 나누고(1년은 오류 · #1623) `totalCount`에 닿으면 멈추는가 · 오류 코드·HTTP 오류 문구에 키가 새지 않는가 |
| `test_port_call_store_db.py` | 6 | **§5 외부 연동 · 공적 재항 기록 저장·수집**(`#1197` B단계) — 기항마다 제공자 **원문(`raw`)이 그대로** 따라오고 원문은 같은 기항 판정에 쓰지 않는가 · 같은 기항을 다시 받으면 **한 행을 갱신**하는가(공적 기록의 `최초` → `최종` 정정) · 호출부호별 조회 · (선박, 항만청) 한 쌍의 실패가 다른 쌍을 막지 않고(`PRD §16.2`) 호출부호 없는 배는 묻지 않으며 다른 배가 섞인 응답은 버리는가 · 항구 이름 → 항만청코드는 **응답을 확인한 11개 안에서만** 잇고 모르는 이름은 대조 불가(`None`)인가 |
| `test_port_call_reconcile.py` | 7 | **§5 외부 연동 · 공적 기록 대조 규칙**(`#1197` · `PRD §17.4.4` · 결정 G-7) — **6시간 정각은 띄우지 않고 6시간 1분은 띄우는가** · 방향(이르다·늦다)을 보지 않는가 · 48시간 밖·모르는 항구·다른 항만청·빈 값은 대조 불가인가 · 출항은 가장 늦은 출항과, 도착은 가장 이른 입항과 견주는가 · 가장 가까운 기항을 고르는가 · 정박 구간 두 끝 · 칸 종류 순 |
| `test_public_record_fill_db.py` | 10 | **§3.29 통합 · 공적 기록으로 채우기**(`IT-FILL-001`~`008` · `#1923` · `API_SPEC §3.12`) — 출처 열 넷의 트리거(`064`)가 스키마와 같은 값만 받는가 · 데이터 점검 조회만으로는 아무것도 바뀌지 않는가 · **누른 칸 하나만** 바뀌고 옆 칸은 그대로인가 · 확정 항차는 `revert_confirmed` 없이 422, 있으면 되돌리기(`VOYAGE_TRANSITION`) → 채움(`VOYAGE_ACTUALS_FILL`)이 남는가 · **감사 INSERT가 실패하면 되돌림도 값 변경도 남지 않는가** · 없는 기록 404 · 다른 배 422 · 화면이 본 값과 다르면 409 · 채운 뒤 **같은 시각** 재전송은 출처를 지우지 않고 **다른 시각**은 `null`로 되돌리는가 · 정박 구간 시작도 같은가. 응답 필드는 HTTP에서 확인한다(`#433`) |
...
| `test_soft_delete_db.py` | 10 | **§3.9 통합 · §5.6 DB · 소프트 삭제** — 조회·집계에서 빠지는가 · 삭제된 IMO 자리를 비우는가(활성 키 `imo_active`의 유니크 인덱스 · `061`) · 행이 남아 있는가 (`#66`) |
...
| `test_not_underway_crud_db.py` | 36 | **§4 API · not under way 구간 CRUD** — 구간 겹침 금지 · **동시 요청에서도 겹침이 하나만 남는다**(`#1629` · `F-8` — 두 연결을 겹침 조회 직후에 교차시켜 생성·수정·CSV 가져오기 셋을 본다 · 돌연변이(선박 `lock_row` 제거) **3/3 실패 실측** 2026-09-24) · CF snapshot · 소프트 삭제 · 집계 도달 · **귀속 연도는 UTC 기준**(`#1333`) — 종전에는 `started_at.year`를 그대로 읽어 **오프셋 붙은 값의 현지 연도**가 나왔다. `API_SPEC §8.2` 예시 형식(`+09:00`)대로 KST 1/1 새벽 정박을 올리면 UTC로는 전년도인데 **다음 해 CII 분자·분모에 들어갔고**, 올바른 연도를 명시하면 거부됐다. 항차 CSV는 처음부터 `astimezone(UTC)`로 정규화한다 — **같은 저장소 안에서 두 경로가 갈려 있었다** |
...
| `test_cubrid_errors.py` | 4 | **§5 인프라 · 드라이버 예외 분류**(`db/cubrid_errors.py` · `#1631`) — DB 없이 돈다. 061의 활성 키 채움 트리거는 행을 넣은 **뒤에** 자기 행을 갱신하므로 유니크 위반이 트리거 액션 안에서 나고 드라이버가 `-670`이 아니라 **`-528`**(「트리거 액션 평가 오류」)로 올린다. ⑴ 유니크 위반 문구를 담은 `-528`만 `IntegrityError`로 옮기고 ⑵ 다른 이유의 `-528`은 그대로 두며(중복으로 오인하지 않는다) ⑶ `violated_unique_index`가 `-670`·`-528` 두 모양에서 같은 **인덱스 이름**을 집는가. 메시지 원문은 실측이다 |
...
| `test_dbschema_head_sync.py` | 5 | **§14 문서 동기화 · `DB_SCHEMA` ↔ alembic head · ORM**(`#1342`) — **이미 있는 트리거 이름에 다른 본문으로 `create_trigger`를 부르는 리비전이 없다**(`#1861` — 헬퍼가 조용히 건너뛰어 운영과 새 DB가 갈린다 · 본문을 바꾸려면 `replace_trigger`)와 그 대조군(조건만 바꾼 가짜 리비전을 끼우면 잡힌다) · 헤더는 v1.33인데 절 일곱이 `051` 이전에 멈춰 있었다. 기계적으로 셀 수 있는 셋만 잠근다: §8.1.0 리비전 그래프의 끝 = `alembic/versions` head · §7.4 「지금 DB에 있는 트리거」 head 열 합계 = `upgrade()`가 내는 `CREATE TRIGGER` 누적 − `DROP TRIGGER`(`op`를 스텁해 DB 없이 센다 — 148은 `051` 시점, head는 160) · §2.6 `annual_simulation_run` 열 집합 = ORM 열 집합(`052` `as_of`·`053` `alternative_fuel`이 빠져 있었다). 돌연변이 셋(그래프 051 · 합계 148 · `as_of` 행 삭제) 각각 한 함수씩 실패(실측) |
| `test_design_semantic_sync.py` | 3 | **§14 문서 동기화 · `DESIGN_SYSTEM §2.3` ↔ 생성 토큰**(`#1881`) — `§2.3` 표의 시맨틱 4색(라이트·다크)이 `tokens.generated.css`의 `--semantic-*`와 같은가 · `§0.2` 제약 2(시맨틱 색이 등급 fill과 같은 값이 아니다)를 **생성 토큰으로** 실측 · 옛 사본(`#2f855a` · `#c53030`)이 변경 이력·정정·해소 기록 밖에 다시 나타나지 않는가. 선언이 「문서 안의 값끼리」 비교해 통과를 선언하던 상태를 막는다 |
...
| `test_health.py` | 17 | §4 API · 공통·운영 · **`commit`**(`#789`) — 배포 빌드의 커밋을 싣고, 없으면 빈 문자열이 아니라 `null` · 배포 확인이 그 값을 빌드한 커밋과 대조하는 배선 |
...
| `test_rate_limit.py` | 27 | **§4 API · 요청 한도** — `API_SPEC §13.2` 계약. 카운터 자체(고정 윈도·IP 분리·`0` 비활성·`X-Forwarded-For` 무시·429 봉투의 `meta.request_id`)와 **경로 버킷**(`#811`)을 함께 본다. 버킷은 세 갈래다 — 인증 10 · 계산 60 · 그 밖 300. 종전에는 전역 한도 하나(300)뿐이라 **로그인 무차별 대입에 분당 300회**가 허용됐고, 정본이 규정한 계산 60회는 적용되지 않았다. ⚠️ **경로 목록이 실제 라우트와 어긋나면 한도가 조용히 풀리므로** 두 집합의 모든 경로가 앱에 실재하는지 대조한다 — `app.routes`가 아니라 **OpenAPI**를 읽는다(`include_router`한 경로는 `app.routes`에 펼쳐지지 않아 0개로 보이고, 그러면 검사가 「없는 것끼리 비교해」 통과한다. `#634`가 같은 함정에 걸릴 뻔했다). 경계가 **넓어지는** 방향도 함께 막는다: `/auth/logout`·`/auth/me`·`GET /calculations`·`POST /scenarios/{id}/adopt`가 기본 버킷에 남는지. **프록시 서명 헤더**(`#1483`) — 비밀 값이 맞을 때만 원 IP로 세고, 틀림·없음·위조·IP 아닌 값은 종전 규칙으로 내려가며, 미들웨어를 거쳐서도 두 사용자가 따로 세어지는지. IPv6는 `/64` 대역으로 센다 |
| `test_audit_client_ip.py` | 6 | **§4 API · 감사 IP 판정**(`#1889` · `#1483` 후속) — 감사용 판정이 요청 한도와 같은가(서명 헤더 맞음 → 원 IP · 틀림 → 소켓 상대) · 알 수 없으면 `unknown`이 아니라 NULL인가 · `X-Forwarded-For` 첫 항도 IP여야 쓰는가(45자 칸) · **소켓 상대 직접 참조가 판정 함수와 접근 로그 `peer` 밖에 없는가**(grep 가드) · 파일별 `_client_ip` 사본이 없는가 |
...
| `test_tracked_files_are_text.py` | 4 | **§14 인벤토리 동기화** — 추적 소스에 NUL이 섞이면 git이 바이너리로 보아 **PR diff와 `grep`이 막힌다.** `.gitattributes`는 보이게 할 뿐 유입을 막지 못해 들어오는 자리에 신호를 둔다 (`#572` 발견 · `#575`) · **풀리지 않은 병합 표시**(`| `test_trigger_ddl.py` | 10 | **§5 DB · 트리거 DDL의 멱등 계약** (`db/trigger_ddl.py` · `#1373`) — CUBRID는 같은 이름의 트리거를 두 번 만드는 것을 막지 않고 중복이 생기면 이름으로는 지울 수 없어(`-503` · `DB_SCHEMA §7.4` 10항), 트리거를 만들고 지우는 마이그레이션 14개가 전부 이 모듈을 지난다. 가짜 `op`로 DB 없이 다섯을 고정한다 — 있으면 만들지 않고 없으면 만든다 · 없으면 지우지 않고 있으면 지운다 · **카탈로그에는 있는데 `-503`이 오면 경고를 남기고 넘어가되 다른 오류는 그대로 올린다**(조용히 삼키면 진짜 실패까지 가린다) · `existing` 집합을 주면 카탈로그를 한 번만 묻고 지운·만든 이름을 그 집합에 반영한다(`050`·`051`·`057`의 지우고-다시-만들기가 「있으면 건너뜀」에 걸리지 않는 근거) · **`-503`은 드라이버 `errno`로도 문구로도 알아본다** · **`replace_trigger`(upgrade의 교체)는 지운 뒤에도 이름이 남아 있으면 `README` 「테스트 DB 복구」를 가리키며 멈추고 옛 것 위에 만들지 않는다** — 관용은 downgrade에만 둔다(롤백이 갇히지 않게) |
...
| `test_speed_upper_bound_db.py` | 5 | **§5.1 DB · `DB-CHK-023` 속력 상한 트리거**(`#1269`) — `062`가 만든 트리거 여덟 개가 실재하는지 · API를 거치지 않은 INSERT(선박)·UPDATE(항차 두 칸)도 60을 넘으면 막히는지 · 60 정각과 고속선의 실제 속력 58.1과 NULL은 들어가는지. 서버 스키마가 422로 막아도 시드·복원·수기 SQL로 들어온 `120`kn은 연료 모델에서 속력 배율 1,000이 된다 |
...
| `test_db_isolation_premise_db.py` | 8 | **§5.10 DB · 격리 수준 전제 가드 `DB-ISO-001`~`008`**(`#1796` · `#1631` PR에 동봉) — 동시성 방어 세 갈래(`#1079` 조건부 UPDATE · `#1630` 부모 행 잠금 · `#1631` 유니크 인덱스)가 기대는 「READ COMMITTED에서 미커밋 변경은 보이지 않고 읽기를 막지도 않는다」를 **동작으로** 잠근다. 앱 세션 팩토리로 세션 둘을 교차시키고 B는 `SET TRANSACTION LOCK TIMEOUT 1`로 대기 여부를 시간으로 판정한다 — 미커밋 INSERT 불가시·무대기 / 미커밋 UPDATE 옛 값(같은 행 `FOR UPDATE`는 `errno=-75`) / 범위 SELECT→INSERT 교차가 잠금 없이는 2건·부모 `FOR UPDATE`면 1건 / 유니크 인덱스는 대기 후 `IntegrityError`(-670) / 조건부 UPDATE 재평가 `rowcount 0`. 표 `probe_iso_*`는 파일이 만들고 지운다 |
...
| `test_voyage_row_lock_db.py` | 7 | **§3.1 통합 · 항차 쓰기의 직렬화**(`IT-STATE-009` · `#1626` · `F-8`) — **두 연결을 실제로 교차시킨다.** 첫 세션의 `commit`을 갈아 끼워 **잠금을 쥔 채 커밋 직전에** 머물게 하고 그 사이에 두 번째를 넣는다 — 서비스가 스스로 커밋하므로 호출이 끝난 뒤에 교차시키면 잠금이 있든 없든 같은 결과가 나온다(`#1629` 코멘트). **`get_by_id`의 `for_update` 돌연변이를 검출하는 네 짝**: 전환×전환(종결 뒤 `IN_PROGRESS` 422 · 새 상태를 **보고** 거절했는지 문구로 확인) · 전환×PATCH(출항 뒤 계획값 422) · 전환×실적(취소 뒤 실적 422) · 전환×삭제(`PLANNED` 뒤 삭제 422). 돌연변이(`get_by_id`의 `for_update` 갈래 제거) **실측**(2026-09-24): 넷 다 **실패** — 셋(전환 × 전환 · PATCH · 실적)은 두 번째가 예외 없이 성공하고, 전환 × 삭제는 첫 세션의 UPDATE가 사라진 행을 겨눠 `StaleDataError`로 터진다. (`#1850` 당시 로컬 DB가 내려가 있어 CI로만 확인했다.) **채택×채택은 이 넷에 들지 않는다**(`#1869` 확정) — 같은 돌연변이에서 **통과한다**(행 1건 · 단독 실행도 통과 · `#1870` 최초 관찰). `UPDATE_EXISTING_PLAN`이 두 채택 모두 같은 `voyage` 행을 **무조건** 쓰므로, 그 UPDATE 자체의 통상적 쓰기 잠금이 `get_by_id`의 명시 `FOR UPDATE`와 무관하게 상대를 이미 직렬화한다 — 「채택 하나」는 여전히 지켜지지만 **이 잠금이 지키는 게 아니다.** `#1869`가 인위적 hold 없이 두 `adopt_scenario`를 `asyncio.gather`로 맞부딪히는 **자연 동시 실행**(`#1796` ⑺과 같은 기법)으로 재확인했다 — 같은 돌연변이에서 **15/15** 예외 없이 채택 행 1건만 남아, 교차 시점을 바꿔도 이 짝으로는 돌연변이가 드러나지 않는다(원문은 PR #1880). `#1796` ⑺의 「채택 행 2건」은 이 PR에서 재검증하지 않았다 — 그 기록이 쓰인 시점의 `adopt_scenario` 코드가 지금과 같았는지는 확인 대상이 아니다. 여섯째 짝(`#1860`): **정박 구간 × 채택 `CREATE_NEW_VOYAGE`의 교차 잠금** — 첫 세션이 선박 X를 쥔 채 INSERT 전에 머무는 사이(커밋 직전이 아니다 — 교착은 두 INSERT가 서로의 부모를 기다릴 때 난다) 채택을 넣는다. 채택이 선박을 먼저 잠가 둘 다 성공(구간 1 · 새 항차 1). 돌연변이(선박 선잠금 제거 — `#1868`부터 그 자리는 `get_by_id(for_update=True)` 안이다) 기대 결과: CUBRID가 교착을 감지해 한쪽을 끊는다(`errno=-72` unilaterally aborted · 실측 확인). 일곱째(`#1868` · 파라미터 5건): **정박 구간 × 항차 X 경로 다섯**(전환 · PATCH · 실적 · 소프트 삭제 · 채택 UPDATE) — 같은 첫 세션(선박 X → INSERT 전 머묾) 사이에 항차 X를 쥐고 `voyage`를 UPDATE하는 경로를 넣는다. CUBRID 11.4는 **FK 열을 바꾸지 않는 UPDATE에도 부모 S를 요구**하므로(`#1868` ⑽-b 3/3) 잠금 순서가 항차 → 선박이면 교착이다. 저장소가 선박을 먼저 잠가 둘 다 성공(구간 1 · 항차는 기대 상태). 돌연변이(같은 선박 선잠금 제거) **실측**(2026-09-24): 5/5 실패(`errno=-968` 또는 `-72`). 세션은 `app_fresh_engine`이 갈아 끼운 것을 호출 시점에 받는다 — 모듈 수준 `get_sessionmaker` 묶음은 파일만 따로 돌리면 `_setup`이 `NOT NULL` 오류로 끝났다 |
...
| `test_migration_guard.py` | 21 | **§5 DB · 마이그레이션 · 되돌릴 수 없는 downgrade** — 프로덕션에서만 막고 리비전을 하나씩 명시해야 풀리는지(와일드카드 없음) · **명시해도 24시간 안의 백업 기록이 없으면 무엇이든 지우기 전에 막히는지**(`#827`) · 목록의 모든 리비전이 **실제 호출에서** 무엇이든 지우기 전에 끊기는지 · 파괴적 `downgrade()`가 전부 세 분류 중 하나에 들어 있는지 (`DB_SCHEMA §8.1.2` · `#819`) |
...
| `test_db_backup_script.py` | 47 | **§5 DB · 백업 · 복구 리허설 · 교체** (`scripts/db_backup.py`) — 컨테이너 호출을 대역으로 바꿔 판단을 본다: `pg_restore`로 읽히지 않는 덤프는 **확정도 기록도 하지 않는다** · 가드가 찾는 `DB_BACKUP` 행을 남긴다 · 보존 개수는 같은 DB의 덤프만 센다 · 손상된 덤프(sha256)는 **서버에 닿기 전에** 거절 · 리허설이 행 수가 다른 테이블을 **이름으로** 말한다 · 교체는 운영 DB 이름을 그대로 적어야 하고, 대조에 실패하면 **앱을 멈추기 전에** 끝나며, 성공하면 이전 DB를 지우지 않고 남긴다 · 스크립트가 **표준 라이브러리만** 쓴다(배포 호스트에 가상환경이 없다). 실제 `pg_dump`·`pg_restore`는 CI docker 잡이 프로덕션 스택에서 매 실행 돌린다 (`#827`) · **교체가 중간에 실패하면**(`#1635` · 결정 F-13) — 첫 이름 변경 실패는 운영 DB를 다시 켜고 앱을 켠다 · 둘째 실패는 보관 이름을 되돌린 뒤 앱을 켠다 · 되돌림까지 실패하거나 새 운영 DB가 켜지지 않으면 **앱을 켜지 않고** 두 이름과 수동 명령을 준다 · 분리 배포(`APP_SERVICE=none`)는 `--app-stopped` 없이 아무것도 바꾸지 않고 앱 서비스를 부르지 않는다 |
...
| `test_zz_active_key_precheck_db.py` | 1 | **§5.6 `DB-SOFT-007` · `061` 사전 검사를 실제 CUBRID에서**(`#1631`) — `downgrade 060` → `047` `_ins` 트리거를 잠시 걷고 같은 IMO 활성 행 둘 → 되살림 → `upgrade head`가 0이 아닌 코드로 멈추고 문구에 그룹 수·`§3.6.4`는 있고 IMO는 없다 · `db_trigger`에 `047` 4개가 그대로, `db_attribute`에 `imo_active` 없음 · 한쪽 소프트 삭제 뒤 같은 명령이 지나간다. `test_zz_*`·`try/finally` 복원은 `test_zz_roundtrip.py`와 같은 이유 |
| `test_zz_roundtrip.py` | 7 | §5 DB · 제약·마이그레이션 (데모 seed 분리 후 롤백 — `#451`) · **seed migration 멱등(#1201)** — 행은 있고 `alembic_version`만 직전인 운영 상태(2026-09-15 수동 seed)를 `stamp`로 재현해 `upgrade head`가 UNIQUE 위반 없이 성공하는지 본다 · **head 트리거 집합 둘**(`#1373` · `D-20`) — 왕복을 돌리기 **전에** ⑴ 같은 이름의 트리거가 둘 이상인 것이 0건인가(`GROUP BY name HAVING count(*) > 1` — 있으면 그 이름은 `-503`으로 지울 수 없고 왕복이 거기서 끊긴다. `db/trigger_ddl.drop_trigger`가 그 `-503`을 넘어가므로 여기서 세지 않으면 조용하다) ⑵ `upgrade head` 뒤 `db_trigger`의 이름 집합이 마이그레이션 `upgrade()`가 만든다고 적은 집합(`CREATE` 누적 − `DROP` · `op` 스텁으로 DB 없이 센다)과 **이름 하나하나** 같은가 — 합계 160은 `test_dbschema_head_sync`가 `DB_SCHEMA §7.4`와 대조하고, 수가 같아도 남은 것 하나와 빠진 것 하나가 상쇄되는 경우는 여기서만 보인다. 두 단언은 `test_downgrade_upgrade_roundtrip`이 되올린 **뒤에도** 다시 부른다 — 종료 코드 0은 집합이 온전하다는 뜻이 아니다. 기대 집합은 `tests/migration_stub.py`(두 검사 파일이 공유 · `db_trigger` 조회에 만든−지운 집합으로 답해 DROP도 실제처럼 집계)  · **왕복 전후 트리거 정의(조건·액션·시점)가 같은가**(`#1861` — 이름·개수가 같아도 정의가 갈릴 수 있다) |

**합계 223개 파일 · 2910 함수 · 3639 수집.** (2026-09-26 실측 — #788 백업 폴더 소유자·점검 보강 · #1923 공적 기록으로 채우기 · #1634 시크릿 전달 되살림·끝 줄바꿈 보존 · #788 운영 워크플로 · #786 백엔드 루프백 게시 · #1889 감사 IP 판정 통일 · #789 시연 동결 · #1197 공적 기록 시연 표본 · #1197 공적 기록 대조·데이터 점검 · #1197 공적 재항 기록 저장·수집 · #1867 CUBRID 이름 있는 볼륨 가드 · #1915 운영 스크립트 실행 환경 · #1909 배포 지도 배선(Range·워커) · #789 헬스 커밋 식별자 · #1197 공적 재항 기록 제공자 · #1483 프록시 서명 헤더 · #1635 복구 교체 실패 경로 · #1269 속력 상한 60kn · #1868 FK 잠금 전제 가드 · #1861 트리거 정의 대조 · #1881 시맨틱 색 대조 · #1625 감사 로그 같은 트랜잭션 · #1629 정박 구간 겹침 직렬화 · #1626 항차 행 잠금 · #1300 해상 경로망·우회 경유지 · #1373 트리거 DDL 멱등화 · #1860 교차 잠금 · #1631 활성 키 유니크 인덱스 · #1875 볼륨 가드 · #1868 항차 경로 다섯 × 정박 구간 교차 · #1637 배포 SSH 호스트 키 고정 · #1641 DB 포트 사설 IP 게시 반영)
...
| 2026-09-24 | `#1849` | **v1.30 — §3.28 「공개 해상 경로망 위의 바닷길」 신설**(`test_sea_route.py` 17함수 · `#1300` E-6) · §4.2에 `AT-SC-005` 우회 경유지 행 · §14 `test_scenario_compare_api.py` 37 → **46함수**(⚠️ 머지 당시 36 → 45로 적었으나 `#1851`이 먼저 37로 올려 실제는 37 → 46 — `#1862`에서 정정) · 합계 실측 갱신(201파일·2708함수·3398수집(#1625·#1629·#1626 머지 뒤) → **202파일·2734함수·3425수집**). 절 신설이라 버전을 올린다(`AGENTS §4.3`) (#1300) |
...
| 2026-09-24 | `#1866` | §3.1 `IT-STATE-009`에 **교차 잠금 짝** 추가 · §14 `test_voyage_row_lock_db.py` 5 → **6** · 합계 실측 갱신(203파일·2746함수·3437수집 → **203파일·2747함수·3438수집**). CUBRID 11.4는 자식 행 INSERT의 FK 검사로 **부모 행에 S 잠금을 요구한다**(`#1860` ⑻ 실측 · 부모 X 보유 중 자식 INSERT 3/3 대기 · 커밋까지 쥐는지는 미측정) — 그래서 항차 X를 쥔 채 선박을 참조하는 항차를 넣는 채택 `CREATE_NEW_VOYAGE`와 선박 X를 쥔 채 항차를 참조하는 구간을 넣는 정박 구간 생성이 교착했다(⑼ 3/3 · `errno=-968`). 채택이 선박 행을 먼저 잠가 순서를 선박 → 항차로 맞춘다. 검사의 요점은 **교차 시점**이 커밋 직전이 아니라 **첫 세션의 INSERT 전**이라는 것이다. 같은 파일의 세션 취득을 `app_fresh_engine` 호출 시점으로 바꿨다 — 파일만 따로 돌리면 앱의 캐시된 엔진을 써 `_setup`이 `NOT NULL`로 끝났다(전체 실행에서는 앞선 검사가 만든 캐시 덕에 통과하던 것). `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1860) |
| 2026-09-24 | `#1870` | §14 두 행에 **돌연변이 실측**을 적는다(로컬 DB 복구 뒤 · 원복은 백업 cp와 `cmp` 동일 확인). `test_not_underway_crud_db.py`: 선박 `lock_row` 제거 시 동시성 3건 **3/3 실패**(`#1629` — PR #1852 때는 로컬 DB가 내려가 실측하지 못했다). `test_voyage_row_lock_db.py`: 항차 `for_update` 갈래 제거 시 **4/5 실패 · 채택 × 채택만 통과** — 종전 기대값 「채택 행 2건」은 이 교차 시점에서 성립하지 않았다. 이 짝이 무엇을 지키는지 다시 정하는 일은 `#1869`로 남긴다. 함수 수 변화 없음. `AGENTS §4.3`상 행 보강이라 버전은 올리지 않는다 (#1869) |
| 2026-09-23 | `#1798` | §14 `test_deploy_secret_transport.py` **신설(7함수)** · 합계 실측 갱신(203파일·2747함수·3438수집 → **204파일·2754함수·3453수집** — 수집 수는 `origin/main` 병합분 위에 이 PR 몫 +15(파라미터라이즈 9종 포함 · `test_emit_then_base64_round_trips_byte_exact`)를 더한 값이다). 배포 시크릿이 ssh 명령줄과 따옴표 없는 `.env` heredoc을 건너며 **바뀌거나 배포를 세우던** 경로를, 러너 렌더(`emit`) + base64 전달로 바꾼 것을 고정한다. 가짜 값 왕복을 실제 compose 컨테이너로 바이트 대조한다. `test_deploy_env_rendering.py`·`test_compose_env_wiring.py`는 읽는 자리만 `emit` 줄로 옮겼고 함수 수는 그대로다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1634) |
| 2026-09-24 | `#1876` | **`#1798` 되돌림** — §14 `test_deploy_secret_transport.py` 삭제(7함수) · 합계 203파일·2747함수·3438수집으로 복귀. `#1798` 머지 뒤 운영 배포 `deploy-db`가 브로커 대기 60회를 모두 실패했다(run 35976595865) — 같은 컨테이너(`Running` · 재생성 없음)에 직전 배포는 1회차에 비밀번호 인증으로 붙었으므로, 새 전달 방식의 비밀번호 값이 달라진 것으로 본다(정황 · 운영 서버 로그는 볼 수 없다). 운영 DB는 살아 있었다(공개 로그인 응답이 DB 조회를 거친 `INVALID_CREDENTIALS`). 배포를 알려진 정상 상태로 돌리고 원인은 `#1634`에서 다시 본다. `AGENTS §4.3`상 버전은 올리지 않는다 (#1634) |
| 2026-09-24 | `#1848` | **v1.31 — §5.10 「격리 수준 전제 가드」 신설**(`DB-ISO-001`~`005` · `#1796`) · §5.6에 `DB-SOFT-003`~`006`(활성 키는 DB가 채운다 · 동시 등록 경합 · NULL 다중 허용 · 중복 아닌 위반은 409로 둔갑 안 함) · §4.7에 `AT-AUTH-019`(동시 중복 가입 — 201 + 409 정본 문구) · §3.9·§5.6·`DB-SOFT-001/002`의 「partial unique index」를 활성 키 유니크 인덱스(`061`)로 정정 — `047`의 트리거는 `NOT EXISTS`가 미커밋 행을 못 봐 동시 등록에서 중복이 남았다(`#1631` 실측). §14.2에 **`test_active_key_unique_db.py`(11함수) · `test_db_isolation_premise_db.py`(5함수) · `test_cubrid_errors.py`(4함수) · `test_active_key_precheck.py`(3함수 · DB 없이) · `test_zz_active_key_precheck_db.py`(1함수) 신설 등재** · §5.6에 `DB-SOFT-007`(`061` 사전 검사 — 중복이면 아무것도 바꾸기 전에 멈추고 문구에 값이 없다 · G-1 「다′」 2026-09-24) · `test_migration_guard.py` 20 → **21** · 합계 실측 갱신(203파일·2747함수·3438수집 → **208파일·2772함수·3463수집** — `#1876`(`#1798` 되돌림) 뒤 main 실측에 이 PR의 +5파일·+25함수·+25수집). 리뷰 반영(PR #1848) — 경합 검사는 `vessel_repo.insert`·`is_initial_admin` **스파이**로 「flush 뒤 커밋 전」 교차를 단언하고 채움 트리거가 `updated_at`을 밀지 않는지 본다 · `test_dbschema_head_sync.py` 스텁이 `db_trigger` 조회에 사슬의 누적 집합으로 답한다(061의 「있을 때만 지운다」 DROP을 세기 위해 · 함수 수 불변). 절 신설이라 버전을 올린다 (#1631) |
| 2026-09-24 | `#1873` | **결정요청 v2 F-2/G-9 「가」 채택** — `.gitattributes`에 `*.sh text eol=lf` 한 줄을 추가한다(`#575` 주석 체계에 맞춘 예외 사유 한 문단). Windows `core.autocrlf=true` checkout에서 `scripts/demo_up.sh`가 CRLF가 되면 Git Bash·WSL의 `bash -n`이 `do\r` 구문 오류로 실행 자체를 막는 것을 고친다. 나머지 체크아웃 동작은 그대로 둔다 — `text`·`eol`을 두지 않기로 한 `#575`의 원칙에 `.sh`만 예외를 둔다. 추적 파일이 이미 전부 LF라 `git add --renormalize .`로 변경 0건을 확인했다(재정규화 커밋 없음). §14 `test_demo_up_script.py` 38 → **40함수** — `.gitattributes`가 `*.sh`를 `eol=lf`로 고정하는지, `git ls-files --eol '*.sh'`가 전부 `i/lf`인지 잠근다. 합계 실측 갱신(203파일·2747함수·3438수집 → **203파일·2749함수·3440수집**). ⚠️ **Windows 네이티브 Git Bash에서의 최종 재확인은 이 PR 범위 밖이다** — 로컬 clone 검증(`core.autocrlf=true`로 clone 후 `git ls-files --eol`이 `w/lf`)까지만 여기서 하고, 완료 기준의 Windows 네이티브 확인은 별도로 남긴다 (Refs #1620) |
| 2026-09-24 | `#1875` | §14 `test_cubrid_volume_guard.py` **신설(6함수)** · 합계 실측 갱신(208파일·2774함수·3465수집 → **209파일·2780함수·3471수집** — `#1848` · `#1873` 머지 뒤 main 기준). 운영 CUBRID 데이터가 명명 볼륨이 아니라 이미지가 선언한 익명 볼륨에 있고, `docker compose down`이 `-v` 없이도 빈 DB를 만든다는 사실(2026-09-24 로컬 실측)을 두 자리에서 잠근다 — 두 compose 파일의 `cubrid-data` 마운트 근처 위험 주석 · `deploy.yml` db-01 단계의 `down`이 `FORCE_DB_INIT` 분기 밖에서 쓰이지 않는지(if/fi 스택 추적). DB 없이 파일만 읽는다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1867) |
| 2026-09-24 | `#1877` | §3.1 `IT-STATE-009`에 **항차 X 경로 다섯 × 정박 구간** 교차 추가 · §14 `test_voyage_row_lock_db.py` 6 → **7**(파라미터 5건) · 합계 실측 갱신(`#1875` 머지 뒤 main 기준 209파일·2780함수·3471수집 → **209파일·2781함수·3476수집**). `#1868` 실측 — CUBRID 11.4는 **FK 열을 바꾸지 않는 UPDATE에도 부모 행 S 잠금을 요구한다**(3/3 · 다른 부모의 자식 UPDATE는 기다리지 않아 FK 재검사로 확정) · 자식 INSERT가 얻은 S는 **커밋까지 쥔다**(3/3). 그래서 항차 X를 쥔 채 `voyage`를 UPDATE하는 경로 다섯이 전부 정박 구간 생성과 교착했다(실제 표 3/3 · `errno=-968` · 수정 전 검사 5/5 실패). 선박 선잠금을 `voyage_repo.get_by_id(for_update=True)`로 옮겨 다섯 경로가 선박 → 항차 순이 되게 했고, 수정 뒤 11/11 통과 · 돌연변이(선박 선잠금 제거) 실측은 PR 본문. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1868) |
| 2026-09-24 | `#1880` | §3.1 `IT-STATE-009` · §14 `test_voyage_row_lock_db.py` 행 — **채택×채택 돌연변이 결과를 확정**한다(`#1869` — `#1870`의 「정황」을 닫는다). 짐작 확인 실측(잠금 없는 두 세션 · 3/3): 첫 채택의 자기 행 UPDATE(미커밋 보유) ↔ 둘째 채택의 해제 UPDATE(`_clear_previous_adoption`)는 **대기하지 않는다**(rowcount=0 · 짐작이 든 `voyage_scenario` 행 잠금 메커니즘은 근거가 아니었다). 대신 인위적 hold 없이 `asyncio.gather`로 두 `adopt_scenario`를 맞부딪히는 **자연 동시 실행**(`#1796` ⑺과 같은 기법)으로 재확인 — 수정 전(잠금 있음) 10/10, 돌연변이(`get_by_id`의 `for_update` 제거) 15/15 모두 채택 행 1건, 위반 0건. 「채택 하나」는 `UPDATE_EXISTING_PLAN`이 두 채택 모두 같은 `voyage` 행을 무조건 쓰는 데서 오는 **통상적 쓰기 잠금**이 지킨다 — `get_by_id`의 명시 `FOR UPDATE`는 이 짝에서 여분이다. 짝을 항차 잠금 돌연변이표에서 빼고(테스트 자체는 행동 계약 회귀 검사로 유지) 사유를 테스트 docstring·§14 행·§3.1 `IT-STATE-009`에 적었다. 함수 수 변화 없음. `AGENTS §4.3`상 행 보강이라 버전은 올리지 않는다 (#1869) |
| 2026-09-24 | `#1844` | §14 `test_deploy_host_key_pinning.py` **신설(9함수)** · 합계 실측 갱신(`#1878` 머지 뒤 main 기준 209파일·2781함수·3476수집 → **210파일·2790함수·3488수집**). 배포 워크플로가 실행 중 `ssh-keyscan`으로 받은 호스트 키를 믿던 것을 끊었다(`#1637` · `F-11`) — 사람이 두 경로의 지문을 대조해 커밋한 `ops/host/known_hosts`만 믿고 `StrictHostKeyChecking=yes`로 접속하며, 파일에 호스트가 없으면 원격 명령 전에 멈춘다. 절차·실패 동작·키 교체는 `docs/OPERATIONS.md §4.7`. 돌연변이 4종 4/4 검출. 키 두 줄은 이 PC와 GitHub Actions 러너 두 네트워크에서 받은 지문이 같음을 확인해 넣었다(2026-09-24 · 결정 G-2 라). `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1637) |
| 2026-09-24 | `#1843` | §14 `test_compose_env_wiring.py` 18 → **20** · `test_deploy_env_rendering.py` 3 → **4함수** · 합계 실측 갱신(`#1844` 머지 뒤 main 기준 210파일·2790함수·3488수집 → **210파일·2793함수·3491수집**). 잠그는 것은 **CUBRID 포트 게시가 db-01 사설 IP에만 붙는가**다(`#1641` · 결정 F-14 「가」). 종전 `docker-compose.prod.db.yml`의 `"33100:33000"`은 모든 인터페이스에 열렸고, 2층이라던 ufw(`ops/host/ufw-db-01.sh`)는 **Docker가 publish한 포트에 관여하지 않는다** — 패킷이 DNAT 뒤 FORWARD 체인(`DOCKER-USER` → `DOCKER`)으로 흘러 호스트 INPUT을 거치지 않으므로, 규칙이 있어도 막지 못했고 없어도 열리지 않았다. 게시를 `${OCI_DB_PRIVATE_IP:?…}:33100:33000`으로 바꿔 소켓 자체를 사설 인터페이스에만 두고, ⚠️ **치환을 필수(`:?`)로 잠근다** — `:-`면 값이 빌 때 `:33100:33000`이 되어 다시 모든 인터페이스로 열린다(비어서 열리기보다 비어서 안 뜨는 쪽). `deploy-db`가 같은 이름의 시크릿을 db-01 `.env`에 렌더하므로 **세 고리**(`env:` → ssh 전달 → heredoc)를 `#1475`와 같은 모양으로 본다. 돌연변이 4종 **4/4 검출**. ⚠️ **외부(공인 IP)에서 포트가 닫혔는지의 실측은 사람이 서버 앞에서 한다**(`OPERATIONS §4.1` · 미실측). `AGENTS §4.3`상 행 갱신이라 버전은 올리지 않는다 (#1641) |
| 2026-09-25 | `#1898` | §14 `test_design_semantic_sync.py` **신설(3함수 · 수집 4)** · 합계 실측 갱신(210파일·2793함수·3491수집 → **211파일·2796함수·3495수집**). `DESIGN_SYSTEM §2.3`의 제약 2 선언·`§4.3`·`§15`가 옛 사본으로 대조돼 있던 것을 현재 값으로 바로잡으며(`#1881`) 표 = 생성 토큰 · 제약 2 실측 · 옛 사본 재출현을 잠근다 (#1881) |
| 2026-09-25 | `#1895` | §14 `test_dbschema_head_sync.py` 3 → **5함수** · `test_zz_roundtrip.py` 서술 보강 · 합계 실측 갱신(2796함수·3495수집 → **2798함수·3497수집**). head 대조가 트리거 **이름과 개수**만 봐서, `create_trigger`(「있으면 만들지 않는다」)로 조건만 바꾼 리비전이 운영 DB에는 옛 정의를 남겨도 알 수 없었다. 스텁 체인이 그 호출을 잡고(대조군 포함), 왕복 테스트가 전후 정의를 비교한다 — CUBRID가 조건을 다시 써서 저장해(`[new].[x]` · `in set{…}`) 원문과 바이트 대조는 성립하지 않아(로컬 160/160 불일치 실측) 같은 DB의 전후를 본다 (#1861) |
| 2026-09-25 | `#1894` | 변경 이력 `#1849` 행의 `test_scenario_compare_api.py` 서술 **36 → 45를 37 → 46으로 정정**(`#1851`이 먼저 37로 올렸다 · §14 표는 46으로 맞았고 CI는 표만 본다). §14 인벤토리 수치 변화 없음 — `conftest.py`의 워밍 no-op 픽스처는 테스트 함수가 아니다 (#1862) |
| 2026-09-25 | `#1891` | §5.10 **`DB-ISO-006`~`008` FK 잠금 전제** 추가 · §14 `test_db_isolation_premise_db.py` 5 → **8함수** · 합계 실측 갱신(2798함수·3497수집 → **2801함수·3500수집**). `#1860`·`#1868`(PR #1877)의 잠금 순서(선박 → 항차)가 기대는 세 성질 — 부모 X 보유 중 자식 INSERT는 부모 S를 기다린다 · 그 S는 커밋까지 남는다 · FK 열을 바꾸지 않는 자식 UPDATE도 부모 S를 기다린다(대조군: 다른 부모의 자식은 대기 없음). CUBRID 판올림이나 설정으로 이 성질이 바뀌면 잠금 순서의 근거가 사라지는데 지금까지는 어느 검사도 알려 주지 않았다. `#1848` 머지 뒤로 미뤄 둔 `#1860` 체크리스트 4 (#1868) |
| 2026-09-25 | `#1896` | §3.16 `IT-MAP-001`에 **두 항구 이름** — 선대 요약 `route`가 출발항·도착항 이름을 싣는다(`#1882` · 지도의 항구 핀이 이름을 읽어 준다). §14 인벤토리 수치 변화 없음(기존 함수의 기대값만 넓혔다). 화면 쪽 핀 검사는 `FleetMap.test.tsx`(vitest)다 (#1882) |
| 2026-09-25 | `#1890` | §5.1 **`DB-CHK-023` 속력 물리 상한** 추가 · §14 `test_speed_upper_bound_db.py` 신설(5함수) · `test_request_bounds.py` 6 → **8함수** · `test_vessel_spec_bounds.py` 수집 24 → 25 · 합계 실측 갱신(211파일·2801함수·3500수집 → **212파일·2808함수·3516수집**). 속력의 상한이 저장 형식(9,999.99)뿐이라 `120`kn이 화면·서버·DB를 통과했다 — VAL-009가 「1.0 이상 60 이하」가 되며, 스키마 경계 대조를 **컬럼 정밀도가 아니라 VAL-009의 두 끝**으로 바꾸고(속력만 도메인 상한) 모델 CHECK·마이그레이션 `062`와 값이 같은지 본다 (#1269) |
| 2026-09-25 | `#1900` | §14 `test_db_backup_script.py` 40 → **47함수** · 합계 실측 갱신(2808함수·3516수집 → **2815함수·3523수집**). DB 복구의 교체 단계가 `finally`에서 **무조건 앱을 켰다** — 두 번째 이름 변경이 실패하면 운영 이름의 DB가 없는데 앱이 켜졌다. 실패 단계마다 되돌리거나 멈추고, 운영 DB가 제자리일 때만 앱을 켜는지 대역으로 실패를 주입해 본다(결정 F-13). 분리 배포 모드(`APP_SERVICE=none` · `--app-stopped`) 포함 (#1635) |
| 2026-09-25 | `#1888` | §14 `test_rate_limit.py` 21 → **27함수**(프록시 서명 헤더 5종 · IPv6 `/64` 카운터 키 1종) · 합계 실측 갱신(212파일·2815함수·3523수집 → **212파일·2821함수·3529수집**). 클라우드에서 요청이 프록시·터널을 거쳐 들어와 소켓 상대가 전원 같았다 — `auth` 10/분을 현장 전원이 나눠 썼다. 고정하는 것은 **비밀 값이 맞을 때만 원 IP로 센다**는 경계다: 맞음 · 틀림(`:8001` 직접 위조) · 서버에 비밀 값 없음(빈 헤더와 빈 설정이 「일치」하지 않는다) · IP 아닌 값 · 미들웨어를 거친 두 사용자 분리. 프록시 쪽은 `frontend/functions/_proxy.test.ts`(vitest)가 헤더 이름 일치·브라우저 위조 헤더 제거를 본다 (#1483) |
| 2026-09-25 | `#1893` | §14 `test_port_calls_mof.py` **신설(10함수)** · 합계 실측 갱신(212파일·2821함수·3529수집 → **213파일·2831함수·3539수집**). 공적 재항 기록 제공자의 첫 구현(해양수산부 선박운항정보 · 09-23 결정)을 **실제 응답 표본**으로 고정한다 — 표본은 임시 브랜치 워크플로로 한 번 받고 브랜치·실행 기록을 지웠다(`#1623`과 같은 방식). 저장·대조·화면은 뒤따르는 단계다 (#1197) |
| 2026-09-25 | `#1901` | §14 `test_health.py` 14 → **17함수**(수집 +5) · 합계 실측 갱신(213파일·2831함수·3539수집 → **213파일·2834함수·3544수집**). 헬스 응답의 `commit`(`#789`) — 떠 있는 커밋을 밖에서 알 수 있게 하고, 배포 확인이 배포한 커밋과 같을 때만 성공으로 친다 (#789) |
| 2026-09-25 | `#1909` | §14 `test_deploy_map_assets.py` **신설(4함수)** · 합계 실측 갱신(213파일·2834함수·3544수집 → **214파일·2838함수·3548수집**). 운영에서 **지도가 한 번도 뜬 적이 없었다** — 원인이 둘 겹쳐 있었고 둘 다 조용했다(배포 성공 · CI 초록 · 개발 서버에서는 정상). ⑴ Cloudflare Pages가 `Range`를 무시해(`bytes=0-6` → `200`·전체 15 MB) `hasBasemap()`이 「자산 없음」으로 접고 개략도로 떨어졌다. 판정은 그대로 두고 **서빙을 고쳤다** — Range가 없으면 `pmtiles`가 어차피 멈추므로 회색 사각형보다 개략도가 낫다(`#1144`). ⑵ 빌드가 maplibre 워커를 내보내지 않아 SPA 폴백의 `index.html`을 워커가 파싱하다 죽었고, maplibre는 오류 없이 회색 사각형만 남겼다. **기존 검사로는 잡히지 않는다** — 개발 서버가 Range를 지원하고 워커 파일을 그대로 서빙해 `map-smoke`까지 통과한다. 그래서 소스 배선(이 파일)과 **산출물**(CI의 「지도 타일 워커 자산 확인」 · 성능 예산의 워커 항목) 두 층에 가드를 둔다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1909) |
| 2026-09-25 | `#1915` | §14 `test_ops_scripts_host_python.py` **신설(5함수)** · 합계 실측 갱신(214파일·2838함수·3548수집 → **215파일·2843함수·3553수집**). 문서가 호스트에서 돌리라고 적은 스크립트 둘이 **그 호스트에서 돌지 않았다** — `datetime.UTC`(3.11+) vs Ubuntu 22.04의 3.10.12. `#1867` 운영 DB 이전의 **1단계가 백업**이라 안전망 없이는 이전을 시작할 수 없어 그 자리에서 막혔다. `purge_expired.py`는 crontab으로 도는 자리라 **실패해도 사람이 보지 않는다.** 조용한 이유는 `requires-python`이 **패키지**의 요구사항이고 이 스크립트들은 패키지에 들어가지 않기 때문이다(이미지는 wheel만 담는다) — CI·개발 PC 모두 3.12라 어디서도 빨개지지 않는다. 문법(`feature_version`)·이름(3.11+ 표준 라이브러리)·의존(`cii_platform`) 셋을 보고, 가드가 그 사고를 실제로 잡는지도 되돌려 확인한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1915) |
| 2026-09-25 | `#1892` | §14 `test_cubrid_volume_guard.py` 6 → **8함수** · 합계 실측 갱신(215파일·2843함수·3553수집 → **215파일·2845함수·3555수집**). `cubrid-data`를 이미지의 데이터 경로로 옮기며(결정 G-12 「가」) 잠그는 것이 바뀌었다 — 두 compose 파일의 마운트 대상이 이미지 경로인가 · 배포 DB 단계가 옮기기 전 상태(익명 볼륨 + 빈 `cubrid-data`)를 `up -d` 전에 보고 멈추는가. 마운트를 옮긴 첫 배포가 곧 이전 실행이라, 옮기는 절차를 건너뛴 머지가 운영 데이터를 가리지 않게 한다 (#1867) |
| 2026-09-26 | `#1920` | §14 `test_port_call_store_db.py` **신설(6함수)** · 합계 실측 갱신(215파일·2845함수·3555수집 → **216파일·2851함수·3561수집**). 공적 재항 기록을 **원문 그대로** 저장하고 다시 받으면 갱신하는지, 수집이 (선박, 항만청) 단위로 실패를 가두는지, 항구 이름 → 항만청코드를 확인된 범위에서만 잇는지를 고정한다. 대조 규칙·화면은 뒤따르는 PR이다 (#1197) |
| 2026-09-26 | `#1921` | §14 `test_port_call_reconcile.py` **신설(7함수)** · `test_data_quality_db.py` 19 → 23 · 합계 실측 갱신(216파일·2851함수·3561수집 → **217파일·2862함수·3572수집**). 공적 기록 대조의 경계(6시간 정각 · 48시간 창 · 대조 불가)와 데이터 점검 다섯째 심각도 `PUBLIC_RECORD`를 고정한다 (#1197) |
| 2026-09-26 | `#1922` | §14 `test_dashboard_seed.py` 23 → 24 · 합계 실측 갱신(217파일·2862함수·3572수집 → **217파일·2863함수·3573수집**). 공적 기록 대조의 시연 표본(실존 `D7ZY` 부산 출항 12시간 어긋남 한 건)을 시드 수준에서 고정한다 (#1197) |
| 2026-09-26 | `#1924` | §14 `test_deploy_freeze.py` **신설(5함수 · 6수집)** · 합계 실측 갱신(217파일·2863함수·3573수집 → **218파일·2868함수·3579수집**). 시연 동결(`DEPLOY_FROZEN`)이 push 자동 배포만 건너뛰고 수동 실행은 막지 않는지를 워크플로 파일에서 고정한다 (#789) |
| 2026-09-26 | `#1926` | §14 `test_audit_client_ip.py` **신설(6함수)** · `test_audit_events_db.py` 3 → 4 · 합계 실측 갱신(218파일·2868함수·3579수집 → **219파일·2875함수·3586수집**). 감사 로그·세션 IP가 요청 한도와 같은 판정을 쓰고 소켓 상대를 직접 읽는 자리가 다시 생기지 않게 고정한다 (#1889) |
| 2026-09-26 | `#1941` | §14 `test_deploy_backend_binding.py` **신설(3함수)** · 합계 실측 갱신(219파일·2875함수·3586수집 → **220파일·2878함수·3589수집**). 백엔드를 루프백에만 게시하고 배포 헬스체크를 터널 주소로 옮기며, 배포마다 공인 `:8001`이 닫혀 있는지 확인하는 단계를 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#786) |
| 2026-09-26 | `#1945` | §14 `test_ops_workflow.py` **신설(11함수)** · 합계 실측 갱신(220파일·2878함수·3589수집 → **221파일·2889함수·3600수집**). 운영 워크플로(`ops.yml` — 점검·백업(+**암호화** Actions 아티팩트 14일 · 저장소가 공개)·리허설·수집·롤백 실습·복구 교체·앱 기동)가 되돌릴 수 없는 일에 잠금을 거는지와 배포와 겹치지 않는지를 파일에서 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#788) |
| 2026-09-26 | `#1944` | §14 `test_deploy_secret_shape.py` **신설(3함수)** · 합계 실측 갱신(221파일·2889함수·3600수집 → **222파일·2892함수·3603수집**). PR #1798 되돌림의 원인을 가리려고 넣은 임시 진단 단계가 시크릿 값을 찍지 않는지 실제 실행으로 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1634) |
| 2026-09-26 | `#1947` | §14 `test_deploy_secret_transport.py` **되살림(8함수 — `#1798`의 7 + 끝 줄바꿈 보존 1)** · `test_deploy_secret_shape.py` **삭제(3함수 — 진단 끝)** · `test_deploy_env_rendering.py` db-01 렌더 검사를 러너 `emit` 블록 기준으로 · 합계 실측 갱신(222파일·2892함수·3603수집 → **222파일·2897함수·3619수집**). `#1944` 진단이 운영 `CUBRID_PASSWORD` 끝의 줄바꿈 1개를 확인해, `#1798`의 전달 방식을 되살리고 원격 해독만 끝 줄바꿈을 보존하게 고쳤다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1634) |
| 2026-09-26 | `#1948` | §3.29 **「공적 기록으로 채우기」 신설**(`IT-FILL-001`~`008`) · §14 `test_public_record_fill_db.py` **신설(10함수)** · 합계 실측 갱신(`#1947` 머지 뒤 main 기준 222파일·2897함수·3619수집 → **223파일·2907함수·3636수집** — 새 파일 15건 + `test_api_spec_request_fields_sync.py`의 `§3.12` 표 대조 파라미터 2건). 잠그는 것은 **누른 칸 하나만 바뀌고 그 출처가 남으며, 확정 항차는 동의 표시 없이 풀리지 않고, 되돌리기·채우기·감사가 한 트랜잭션**이라는 사실이다(`PRD §17.4.4` · `API_SPEC §3.12`). 함께 `test_voyages_api.py`·`test_voyage_state_machine.py`의 대역에 출처 속성 둘을 넣었다(함수 수 불변) · `test_response_contract_db.py`의 항차·구간 계약에 출처 키 넷과 새 라우트의 `ROUTE_COVERAGE`(함수 수 불변). 절 신설이라 `AGENTS §4.3`에 따라 버전을 올린다 (#1923) |
| 2026-09-26 | `#___` | §14 `test_ops_workflow.py` **11 → 14함수** · 합계 실측 갱신(223파일·2907함수·3636수집 → **223파일·2910함수·3639수집**). 운영 첫 `backup`이 root 소유 `backups/`에 `Permission denied`로 막혀(run `36244211821`) 덤프 전에 소유자를 되돌리는 줄을 넣었고 그 순서를 고정한다. 함께 점검에 db 컨테이너 상태(`#1634`)와 챗봇 감사 흐름(`#1535` — 폐기된 답은 저장·로그 어디에도 남지 않아 감사 순서로만 되짚는다)을 더하고, 공개 로그에 본문·사용자·IP·해시가 나가지 않음을 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#788) |
