...
| 2026-09-25 | `#___` | §14 `test_cubrid_volume_guard.py` 6 → **8함수** · 합계 실측 갱신(2793함수·3491수집 → **2795함수·3493수집**). `cubrid-data`를 이미지의 데이터 경로로 옮기며(결정 G-12 「가」) 잠그는 것이 바뀌었다 — 두 compose 파일의 마운트 대상이 이미지 경로인가 · 배포 DB 단계가 옮기기 전 상태(익명 볼륨 + 빈 `cubrid-data`)를 `up -d` 전에 보고 멈추는가. 마운트를 옮긴 첫 배포가 곧 이전 실행이라, 옮기는 절차를 건너뛴 머지가 운영 데이터를 가리지 않게 한다 (#1867) |
| 2026-09-25 | `#1892` | §14 `test_cubrid_volume_guard.py` 6 → **8함수** · 합계 실측 갱신(214파일·2838함수·3548수집 → **214파일·2840함수·3550수집**). `cubrid-data`를 이미지의 데이터 경로로 옮기며(결정 G-12 「가」) 잠그는 것이 바뀌었다 — 두 compose 파일의 마운트 대상이 이미지 경로인가 · 배포 DB 단계가 옮기기 전 상태(익명 볼륨 + 빈 `cubrid-data`)를 `up -d` 전에 보고 멈추는가. 마운트를 옮긴 첫 배포가 곧 이전 실행이라, 옮기는 절차를 건너뛴 머지가 운영 데이터를 가리지 않게 한다 (#1867) |
