...
| 2026-09-25 | `#___` | §14 `test_dbschema_head_sync.py` 3 → **5함수** · `test_zz_roundtrip.py` 서술 보강 · 합계 실측 갱신(2796함수·3495수집 → **2798함수·3497수집**). head 대조가 트리거 **이름과 개수**만 봐서, `create_trigger`(「있으면 만들지 않는다」)로 조건만 바꾼 리비전이 운영 DB에는 옛 정의를 남겨도 알 수 없었다. 스텁 체인이 그 호출을 잡고(대조군 포함), 왕복 테스트가 전후 정의를 비교한다 — CUBRID가 조건을 다시 써서 저장해(`[new].[x]` · `in set{…}`) 원문과 바이트 대조는 성립하지 않아(로컬 160/160 불일치 실측) 같은 DB의 전후를 본다 (#1861) |
