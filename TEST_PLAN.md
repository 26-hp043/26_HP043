...
| `test_auth_api.py` | 18 | **§4.7 인증 API** — 계정 존재 여부 비노출 · 가입 중복 고지 · 이메일 정규화 · **가입 게이트 왕복**(`AT-AUTH-016` · `#808`) |
...
| `test_data_quality_db.py` | 13 | **§4 API · 데이터 점검 서비스**(`#513` · `API_SPEC §2.16`) — 네 심각도가 실제 행에서 갈리는가 · ⚠️ **CII 영향이 같은 함수로 따로 구한 두 누적값의 차와 같은가** · 실적·계획 모두 빈 행이 대체가 아니라 계산 불가인가 · 이상치를 계산에서 빼지 않는가 · 실적 미입력은 완결성에서 빼지 않는가 · **이상치 항차는 완결성의 실측에서 빼는가** · 선박 단위 사유가 `§2.8`과 같은 어휘인가 · 그룹 순서 · 응답 필드 집합(영향 블록이 있는 행과 없는 행 둘 다) · **실제 HTTP 경로**(인증 401 · 봉투 · 연도 범위 422) |
...
| `test_reports_db.py` | 33 | **§4 API · 리포트 데이터 수집** — 진행 중 항차 제외 · 시나리오 인용 · 값 재계산 금지 · **문서 어디에도 UTC ISO가 남지 않는다**(`#646`) · **유종도 원문 코드가 남지 않는다**(`#598`) · **제출 전 자체 점검 절**(`#770`) — 새 리포트가 아니라 절인가 · 용도 고지가 실리는가 · 대체 계산을 축으로 나누는가 · 제원이 비면 **리포트 자체가 막히는가**(그래서 그 행을 두지 않았다) |
...
| `test_annual_simulation_read_db.py` | 39 | **§4 API · 기능③ 조회·재실행** — 조회가 다시 계산하지 않는지 · 스냅샷 항차 표현 · 재현 판정(파라미터 변경 409 / 재현 실패 500) (`#443`) · **선박 제원 스냅샷**(`#493` — 제원·capacity·선종을 고쳐도 재현이 흔들리지 않는다 · `037` 이전 실행은 사유를 밝히고 끊는다) · ⚠️ **실행·조회·재현 응답에 `reduction_plan`이 실리는가**(`#433` — PR #1054가 저장 본문에만 넣고 응답을 조립하는 `_envelope`를 고치지 않아 **한 번도 나가지 않았다**) · **실적 보정계수**(`#363` — ⚠️ **켜고 돌린 실행이 재현되는가** · 켠 실행과 끈 실행의 `input_hash`가 갈리는가 · 켜지 않아도 계수를 싣는가 · 표본이 모자라면 적용하지 않고 경고하는가) |
...
| `test_annual_simulation.py` | 69 | **§2 단위 · 기능③ 시뮬레이터** — seed 재현성 · 확률 누적 · 방향 · `§12.8` 예외 · `parameters_used` 스키마 버전 동결(`#816`) · **거리 지렛대가 강도 차이만 잰다**(`#756` — 혼합비가 아니다) · `PRD §12.6` 다섯 변수 ↔ 구현 지렛대 대응 · **필요 감축량 역산**(`#433` — ⚠️ **줄이라는 만큼 줄이면 목표 경계에 정확히 정착하는가** · 목표 경계가 등급 판정과 **같은 표**에서 오는가 · 연료 환산이 구성비 비례인가 · **잔여 계획 없음 ≠ 줄일 것 없음** · 전부 없애도 못 닿으면 그렇게 말하는가 · **데이터를 다시 읽을 수단이 없는가**) · **실적 보정계수**(`#363` — 강도의 비인가(총량 비가 아니다) · 최소 표본 미만은 `null` · 연료만 곱하고 거리는 그대로인가) |
| `test_cii_current_db.py` | 36 | **§4 API · 실시간 CII 3종 값** — 등급이 ⑴에만 붙는 것 · 진행분 반쪽 주입 금지 · `as_of` 재현성 · **⑶ 연말 예상이 「남은 거리 기반」인 것**(`#798`) · **대표 유종이 행 순서에 흔들리지 않는 것**(`#867`). 마지막이 조용했다 — 종전 방식(`YTD_DAILY_AVERAGE`)은 거리·연료를 같은 비율로 더해 `M/W`가 보존되므로 ⑶이 **구조적으로 ⑴과 항상 같은 값**이었고, 그 상태를 잡는 검사가 없었다. 지금은 잔여 계획이 있으면 ⑶ ≠ ⑴임을, 그리고 ⑶이 **기능③의 `projected_attained_cii`와 문자 단위로 같음**을 단언한다 |
| `test_not_underway_import_db.py` | 6 | **§3.4 통합 · 정박 구간 CSV 적재**(`#765`) — 구간·연료가 함께 들어가고 CF 스냅샷이 붙는가 · 부분 성공(틀린 행의 번호와 필드) · **겹치면 그 행만 거부**(겹침을 받으면 같은 연료가 두 번 세어진다) · 시간대 없는 시각 거부 · `dry_run`이 **겹침을 보지 않았다는 사실**을 말하는가 |
...
| `test_llm_guard.py` | 14 | **§3.17 통합 · 챗봇 가드 둘**(`#120`) — 전송 화이트리스트가 **정본 표와 같은가**(한쪽만 고치면 걸린다) · 선박명이 막히는가 · **모르는 새 필드가 조용히 빠지지 않고 터지는가** · 파생 수치(빼기도 계산이다)가 막히는가 · 표기 차이(`4.980`)로 정상 응답을 막지 않는가 · 연도·한 자리 수 예외가 너무 넓지 않은가 |
| `test_chat_store_db.py` | 6 | **§3.17 통합 · 대화 보존**(`#120`) — 만료일이 **만든 시점에 확정**되는가(조회 때 계산하면 정책 변경이 과거를 흔든다) · 만료 세션을 지우면 **메시지도 함께** 사라지는가 · 만료 전 세션이 남는가 · 이력이 최근 N건으로 **시간순** 잘리는가 · `role`이 둘로 제한되는가 · **감사 로그가 원문이 아니라 해시를 남기는가** |
...
| `test_chat_api_db.py` | 15 | **§3.18 통합 · 오케스트레이션**(`#121`) — **가드가 실제 경로에 꽂혀 있는가**(함수가 있어도 부르지 않으면 못 막는다) · 면책이 정본과 글자 그대로 같은가 · **모델이 받은 메시지에 선박명이 없는가** · 지어낸 수치를 버리고 **저장도 하지 않는가** · 호출 상한에서 끊는가 · 남의 대화가 404인가 |
...
| `test_chat_tools_db.py` | 9 | **§3.24 통합 · 챗봇 도구의 실제 실행 경로**(`#120`) — ⚠️ **화이트리스트가 진짜 계산 응답에 대해 도는가**(단위 검사는 손으로 만든 dict를 본다) · 검색이 선박명·IMO·id를 돌려주지 않는가 · API 이름이 정본 이름으로 바뀌는가 · 시나리오를 **순위로 정렬하지 않는가** · 연말 예상 성공·실패 두 경로가 다 도는가 · ⚠️ **없는 선박의 id가 오류 문구로 새지 않는가** |
...
| `test_hashing.py` | 27 | §2 단위 · 계산 엔진 · **기능③ `input_hash` 필드 목록**(`#493` — 기능③이 기능①의 목록을 써서 일곱 키 중 둘만 살아남고 있었다. 필드마다 따로 본다: 한 필드만 빠져도 조용히 통과한다) · ⚠️ **보정계수를 끈 실행의 해시가 종전과 같은가**(`#363` — 바뀌면 저장된 실행 전부가 재현 불가) |
...
| `test_voyage_attribution_db.py` | 4 | **§3 통합 · 계산 이력의 항차 귀속**(`#817` · 결정 2-③) — 밝힌 요청만 귀속(결과·`input_hash` 무영향) · 계획 변경 시 그 항차 계산만 재계산 필요 · 다른 선박 항차 422 · 없는 항차 404 · 이력 있는 항차 삭제 409 |
...
| `test_weather_fallback_db.py` | 14 | **§3 통합 · 기상 fallback** — `PRD §11.6` 네 칸(최신·6h·6~24h·없음) · 실험 모델 배지 · 「보정하지 않았다」를 조용히 넘기지 않는다 (`#62`) |
...
**합계 154개 파일 · 1995 함수 · 2449 수집.** (2026-09-15 실측)
...
| 2026-09-15 | `#1058` | **v1.25 — `AT-AUTH-012`·`AT-AUTH-014`·`AT-AUTH-015`를 「배포 환경(`production`·`staging`)」으로 다시 썼다** · `test_dev_auth.py` 6 → **7함수**(`test_should_register_dev_returns_false_in_staging` 신설) · `test_docs_exposure.py`의 `test_docs_are_open_outside_production`을 `test_docs_are_open_only_in_development_and_test`로 고쳐 허용값 **넷을 모두** 돌게 했다 · `test_demo_user_seed.py`의 `test_production_does_not_get_the_account`를 `test_deployed_environments_do_not_get_the_account`로 바꿔 `production`·`staging` 파라미터라이즈 · 인벤토리 드리프트 3건 실측 정정(`test_demo_up_script.py` 22 → **24** · `test_url_normalize.py` 3 → **4** · `test_dev_auth.py` 6 → **7**) · 합계 실측 갱신(152파일·1968함수·2457수집 → **152파일·1972함수·2412수집** — 수집 수가 준 것은 CUBRID 전환으로 마이그레이션 검사가 합쳐진 결과다). ⚠️ **이 표가 결함을 정답으로 고정하고 있었다** — `production`만 적어 두어, 판정이 `not is_production()`인 동안 `staging`이 **한 번도 검사되지 않았다.** `#524`가 `APP_ENV=production` + `MAIL_BACKEND=console`을 기동 실패로 막기 때문에 **SMTP 준비 전 배포는 `staging`이 정상 경로**이고(`docs/OPERATIONS.md §4.5`), 2026-09-15 OCI 배포(app-01:8001, Security List `0.0.0.0/0`)에서 `POST /auth/dev-login`과 `/docs`가 실제로 **200**을 냈다 — 누구나 미인증 세션을 받을 수 있었고, 시연 계정 시드까지 같은 판정이라 `README.md`에 공개된 비밀번호가 그대로 로그인 수단이 됐다. 돌연변이(`_DEV_SURFACE_ENVS`에 `staging`을 되돌려 넣기)로 새 검사 3종이 전부 실패하는 것을 확인했다. 판정을 **여는 목록**으로 뒤집어 모르는 값·새 환경에서 **닫는 쪽으로** 틀리게 한 것이 이 묶음의 핵심이다(`#810`과 같은 판단) (#1058) |
| 2026-09-15 | `#1058` | §14.2 인벤토리에 `test_constraint_triggers_db.py`(13함수) 등재 · 합계 실측 갱신(153파일·1985함수·2430수집). **CUBRID가 CHECK를 강제하지 않아** ORM에 살아 있는 CHECK 32개가 전부 무력하다 — 빈 테이블로 재현했다(`CHECK (n > 0)`에 `-5`가 들어간다). 전환 분기점(`0f4b062`)과 대조하면 **CHECK 6 · FK 3**이 모델에서 사라졌고, 이 DB의 트리거는 **0개**였다 — 즉 `TECH_SPEC §5.4` 재현성 계약(`calculation_run`·`simulation_snapshot` immutable)이 **아무것도 막지 않는 상태**였다. 그중 재현성·참조 정합에 직결되는 9가지를 트리거로 되살린다(해시 형식 4 · 연료 코드 참조 3 · 불변성 2 → 트리거 15개). ⚠️ **FK로는 안 된다** — CUBRID의 FK는 참조 대상이 **PK여야** 하는데 `fuel_type`은 PK가 `id`이고 `code`는 별도 UNIQUE라 `errno=-920`으로 거부된다(빈 테이블로 재현 확인). ⚠️ **`ON UPDATE CASCADE`도 지원하지 않는다.** 상관명도 `OLD`가 아니라 `obj`다. `calculation_run`을 전면 불변으로 만들지 않은 것이 중요하다 — `024`가 `needs_recalc` 0 → 1 플립만 허용하고 `#283`·`#944`가 서비스에서 그 플립을 한다. 돌연변이(`upgrade()`가 아무것도 만들지 않게)로 18건 중 15건이 검출되고, 통과한 3건은 트리거 없이도 옳은 것들이다(정상 해시 통과 1 · 열 목록 대조 2). 왕복 실측: 트리거 15 → 0 → 15 (#1058) |
| 2026-09-15 | `#1058` | §14.2 인벤토리에 `test_db_types.py`(10함수) 등재 · 합계 실측 갱신(153파일·1985함수·2430수집 → **154파일·1995함수·2449수집**). **`.hex` 59건을 타입 한 곳으로 끝냈다.** 모델 23파일 45곳과 `demo_seed.py` 12곳의 `sa.Uuid`를 `db/types.py`의 신설 `UuidText`로 바꿨다. ⚠️ **원인은 전환이 입력을 조용히 좁힌 것**이다 — 전환 전 `postgresql.UUID(as_uuid=True)`는 psycopg가 문자열을 받아 줬는데 `sa.Uuid`의 bind processor는 `value.hex`를 불러 문자열이 오면 그 자리에서 선다(`sqlalchemy/sql/sqltypes.py:3738`). 부르는 쪽 수십 곳을 고치는 대신 **타입 한 곳**에서 종전 계약을 되돌렸고, `impl`이 `sa.Uuid` 그대로라 **DDL도 마이그레이션도 바뀌지 않는다.** 전체 pytest 실측 **238 failed / 2081 passed → 185 failed / 2134 passed**(`.hex` 59 → **0**). 돌연변이(`str` 강제 변환 제거)로 19건 중 7건이 검출된다 (#1058) |
