...
| `test_compose_env_wiring.py` | 6 | **§5 인프라 · 배포 배선** — compose가 `.env`를 컨테이너에 주입하는지, `environment:`가 `DATABASE_URL`을 덮는지 (`#508`) · **`.env.example`이 `APP_ENV`를 설정하지 않는지**(`#810`). 마지막이 조용하다 — 이 파일을 그대로 `.env`로 복사하면 `docker-compose.prod.yml`의 `${APP_ENV:-production}` 치환이 그 값을 읽어 **프로덕션 스택이 development로 뜬다** |
...
| `test_cii_current_db.py` | 26 | **§4 API · 실시간 CII 3종 값** — 등급이 ⑴에만 붙는 것 · 진행분 반쪽 주입 금지 · `as_of` 재현성 |
...
**합계 109개 파일 · 1481 함수 · 1813 수집.** (2026-09-08 실측)
