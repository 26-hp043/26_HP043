...
| 2026-09-26 | `#1945` | §14 `test_ops_workflow.py` **신설(10함수)** · 합계 실측 갱신(220파일·2878함수·3589수집 → **221파일·2888함수·3599수집**). 운영 워크플로(`ops.yml` — 점검·백업(+**암호화** Actions 아티팩트 14일 · 저장소가 공개)·리허설·수집·롤백 실습·복구 교체·앱 기동)가 되돌릴 수 없는 일에 잠금을 거는지와 배포와 겹치지 않는지를 파일에서 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#788) |
