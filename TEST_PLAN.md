...
| `test_cubrid_volume_guard.py` | 6 | **§5 인프라 · 배포 배선** — CUBRID 데이터가 명명 볼륨이 아니라 이미지가 선언한 익명 볼륨에 있다는 것을 두 자리에서 잠근다(`#1867`). ⑴ `docker-compose.yml`·`docker-compose.prod.db.yml` 양쪽의 `cubrid-data:/var/lib/cubrid` 마운트 근처에 「익명 볼륨」·`#1867`을 담은 위험 주석이 있는가(지우면 다음 사람이 「명명 볼륨이니 안전하다」고 다시 믿는다) ⑵ `deploy.yml`의 db-01 단계가 `docker compose … down`을 **`FORCE_DB_INIT` 분기 밖에서** 쓰지 않는가 — if/fi 중첩을 스택으로 추적한다. 밖에서 쓰면 평소 배포(재생성 경로)마다 새 익명 볼륨이 생겨 매번 데이터가 사라진다. ⚠️ 가드 자체가 사라져 검사가 공허하게 통과하지 않도록 `down -v`가 실재하는지도 함께 본다. DB 없이 파일만 읽는다(`test_deploy_sha_pinning.py`와 같은 근거 — 배포 워크플로는 실행하면 운영 서버를 건드린다) |
