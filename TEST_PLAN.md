...
| `test_deploy_secret_transport.py` | 7 | **§5 인프라 · 배포 시크릿 전달**(`#1634`) — 시크릿이 원격 셸과 `.env`를 **바이트 그대로** 건너는지 본다. 종전에는 시크릿을 ssh 명령줄에 `'${SECRET}'`로 이어 붙이고 원격이 `cat > .env <<EOF`로 **따옴표 없이** 적어, 값에 `'`가 있으면 **ssh 인용이 끊겨 배포가 섰고** `$`·` #`·개행이 있으면 compose가 `.env`를 읽으며 `${…}`를 치환하고 ` #` 뒤를 주석으로 잘라 **값이 조용히 바뀌었다**. 지금은 러너가 `emit`(compose dotenv 큰따옴표 규칙)으로 `.env`를 만들어 **base64로** 넘긴다. 가짜 값 9종(따옴표 · `$(…)` · `${…}` · ` #` · 역슬래시 · 탭 · 개행 · 비ASCII · 빈 값)의 왕복을 우리 역변환과 **실제 compose 컨테이너**(docker가 없으면 건너뛴다) 양쪽으로 대조하고, ssh 명령줄의 원문 시크릿 · base64 값의 `::add-mask::`(할당마다) · `umask 077` 뒤 해독 · `set -x` 부재를 정적으로 본다. 돌연변이 4종 **4/4 검출** |
...
**합계 204개 파일 · 2754 함수 · 3453 수집.** (2026-09-24 실측 — #1625 감사 로그 같은 트랜잭션 · #1629 정박 구간 겹침 직렬화 · #1626 항차 행 잠금 · #1300 해상 경로망·우회 경유지 · #1373 트리거 DDL 멱등화 · #1860 교차 잠금 · #1634 배포 시크릿 전달 반영)
