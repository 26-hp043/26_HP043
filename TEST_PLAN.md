...
>
> 값 판정은 규제 원문 확인(`AGENTS §2.1`)에 달려 있어 **아직 못 채운다.** 그래서 빈 자리를 **사유와 함께 목록으로 붙잡고**, 목록과 정확히 같을 때만 통과시킨다 — 새 누락이 생겨도, 채워진 뒤 목록을 안 지워도 실패한다(**낡은 목록은 거짓말이다**).
...
| IT-REG-004 | ⚠️ **조용히 실패하지 않는다** | HSC 선박 계산 | **409** + 문구에 선종 이름 |
...
| `test_chat_api_db.py` | 14 | **§3.18 통합 · 오케스트레이션**(`#121`) — **가드가 실제 경로에 꽂혀 있는가**(함수가 있어도 부르지 않으면 못 막는다) · 면책이 정본과 글자 그대로 같은가 · **모델이 받은 메시지에 선박명이 없는가** · 지어낸 수치를 버리고 **저장도 하지 않는가** · 호출 상한에서 끊는가 · 남의 대화가 404인가 |
...
| `test_regulation_ship_type_sync_db.py` | 4 | **§3.20 통합 · 규정 표 정합**(`#834`) — 기준선과 등급 경계의 선종 집합이 어긋나는가 · 알려진 누락이 **사유와 함께** 목록에 있는가 · ⚠️ 경계 없는 선박이 **조용한 `null`이 아니라 409**로 거부되는가(조용하면 화면이 「등급 없음」으로 그려 값이 없는 것처럼 보인다) |
...
**합계 147개 파일 · 1893 함수 · 2369 수집.** (2026-09-13 실측)
