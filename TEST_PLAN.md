...
**합계 192개 파일 · 2624 함수 · 3269 수집.** (2026-09-23 실측 — #1639 --check 읽기 전용 가드 2함수 추가 반영)
...
| 2026-09-23 | `#___` | §14 `test_demo_up_script.py` 36 → **38** · 합계 실측 갱신(2622함수·3267수집 → **2624함수·3269수집**). `--check`가 `dev-login`·계산 호출(둘 다 쓰기)을 하지 않는지, 그 두 호출이 6단계의 비-`--check` 분기 안에만 있는지 본다. 수정 전 스크립트로는 첫 검사가 실패한다(실측) (#1639) |
