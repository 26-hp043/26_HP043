...
| `test_voyage_row_lock_db.py` | 6 | **§3.1 통합 · 항차 쓰기의 직렬화**(`IT-STATE-009` · `#1626` · `F-8`) — **두 연결을 실제로 교차시킨다.** 첫 세션의 `commit`을 갈아 끼워 **잠금을 쥔 채 커밋 직전에** 머물게 하고 그 사이에 두 번째를 넣는다 — 서비스가 스스로 커밋하므로 호출이 끝난 뒤에 교차시키면 잠금이 있든 없든 같은 결과가 나온다(`#1629` 코멘트). 다섯 짝: 전환×전환(종결 뒤 `IN_PROGRESS` 422 · 새 상태를 **보고** 거절했는지 문구로 확인) · 전환×PATCH(출항 뒤 계획값 422) · 전환×실적(취소 뒤 실적 422) · 전환×삭제(`PLANNED` 뒤 삭제 422) · 채택×채택(**채택 행 하나**, 나중 것이 남는다). 돌연변이(`get_by_id`의 `for_update` 갈래 제거) 기대 결과: 앞 넷은 두 번째가 **예외 없이 성공**, 채택은 **행 2건** — `#1796` ⑹·⑺가 같은 형태를 실측했다. ⚠️ 로컬 DB가 내려가 있어 이 PR에서는 CI로만 확인한다. 여섯째 짝(`#1860`): **정박 구간 × 채택 `CREATE_NEW_VOYAGE`의 교차 잠금** — 첫 세션이 선박 X를 쥔 채 INSERT 전에 머무는 사이(커밋 직전이 아니다 — 교착은 두 INSERT가 서로의 부모를 기다릴 때 난다) 채택을 넣는다. 채택이 선박을 먼저 잠가 둘 다 성공(구간 1 · 새 항차 1). 돌연변이(채택 갈래의 `vessel_repo.lock_row` 제거) 기대 결과: CUBRID가 교착을 감지해 한쪽을 끊는다(`errno=-72` unilaterally aborted · 실측 확인). 세션은 `app_fresh_engine`이 갈아 끼운 것을 호출 시점에 받는다 — 모듈 수준 `get_sessionmaker` 묶음은 파일만 따로 돌리면 `_setup`이 `NOT NULL` 오류로 끝났다 |
...
| 2026-09-24 | `#1866` | §3.1 `IT-STATE-009`에 **교차 잠금 짝** 추가 · §14 `test_voyage_row_lock_db.py` 5 → **6** · 합계 실측 갱신(203파일·2746함수·3437수집 → **203파일·2747함수·3438수집**). CUBRID 11.4는 자식 행 INSERT의 FK 검사로 **부모 행에 S 잠금을 걸어 커밋까지 쥔다**(`#1860` ⑻ 실측 · 부모 X 보유 중 자식 INSERT 3/3 대기) — 그래서 항차 X를 쥔 채 선박을 참조하는 항차를 넣는 채택 `CREATE_NEW_VOYAGE`와 선박 X를 쥔 채 항차를 참조하는 구간을 넣는 정박 구간 생성이 교착했다(⑼ 3/3 · `errno=-968`). 채택이 선박 행을 먼저 잠가 순서를 선박 → 항차로 맞춘다. 검사의 요점은 **교차 시점**이 커밋 직전이 아니라 **첫 세션의 INSERT 전**이라는 것이다. 같은 파일의 세션 취득을 `app_fresh_engine` 호출 시점으로 바꿨다 — 파일만 따로 돌리면 앱의 캐시된 엔진을 써 `_setup`이 `NOT NULL`로 끝났다(전체 실행에서는 앞선 검사가 만든 캐시 덕에 통과하던 것). `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#1860) |
