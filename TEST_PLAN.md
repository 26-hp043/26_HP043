...
> **[ORACLE-S-1]** PRD §12.4.3은 "Mersenne Twister (MT19937) 또는 동등한 결정적 알고리즘"을 언급하나, TECH_SPEC §2.1에 의해 **PCG64DXSM**으로 확정됨. 본 테스트 계획은 TECH_SPEC 기준을 따르며, PRD v3.2에서 정정 예정.
...
```json
{
  ...
  "comparison_rule": {
    ...
    "assert": "첫 번째 실행 결과 JSON == 두 번째 실행 결과 JSON (유효숫자 기준)"
  }
}
```
...
def test_bulk_just_below_boundary_278999_uses_actual():
    """[ORACLE-S-7] DWT=278,999는 condition_expr을 만족하지 않으므로 실제 DWT 사용"""
...
> PRD §9.4.2 위험도 임계값(ratio_to_required 기반) 검증.

| TC ID | 테스트 | 입력 (ratio) | 기대 결과 |
|---|---|---|---|
| UT-RISK-001 | LOW 임계값 | ratio = 0.80 (superior 미만) | `risk_level = LOW` |
| UT-RISK-002 | MEDIUM 임계값 | ratio = 0.90 (B 등급 구간) | `risk_level = MEDIUM` |
| UT-RISK-003 | HIGH 임계값 | ratio = 1.00 (required 근방) | `risk_level = HIGH` |
| UT-RISK-004 | CRITICAL 임계값 | ratio = 1.20 (D/E 등급) | `risk_level = CRITICAL` |
...
| IT-WX-002 | API 실패 + 6h 캐시 | API timeout, 캐시 존재 | 캐시 factor 사용, `STALE_CACHE` warning |
| IT-WX-003 | API 실패 + 캐시 없음 | API timeout, 캐시 없음 | weather_model = NONE, `WEATHER_UNAVAILABLE` warning |
...
> **[ORACLE-M-4]** 기존 요약 (unit 38, integration 19, API 21, DB 18, 합계 104)은 실제 테이블 행 수와 불일치. 실제 카운트 후 정정하였으며, Oracle 리뷰 피드백 반영으로 추가된 테스트를 포함하여 164건으로 증가.
...
### 11.3 Oracle 리뷰 전후 비교

| 항목 | v1.0 (리뷰 전) | v1.1 (리뷰 후) | 증감 |
|---|---|---|---|
| 단위 (Unit) | 42 | 51 | +9 |
| 통합 (Integration) | 19 | 37 | +18 |
| API | 24 | 26 | +2 |
| DB 제약 | 22 | 42 | +20 |
| 성능 | 4 | 4 | 0 |
| 접근성 | 4 | 4 | 0 |
