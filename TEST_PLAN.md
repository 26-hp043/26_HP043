...
| 2026-09-25 | `#___` | §14 `test_db_backup_script.py` 40 → **47함수** · 합계 실측 갱신(2808함수·3516수집 → **2815함수·3523수집**). DB 복구의 교체 단계가 `finally`에서 **무조건 앱을 켰다** — 두 번째 이름 변경이 실패하면 운영 이름의 DB가 없는데 앱이 켜졌다. 실패 단계마다 되돌리거나 멈추고, 운영 DB가 제자리일 때만 앱을 켜는지 대역으로 실패를 주입해 본다(결정 F-13). 분리 배포 모드(`APP_SERVICE=none` · `--app-stopped`) 포함 (#1635) |
