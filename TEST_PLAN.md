...
| 2026-09-26 | `#___` | §14 `test_port_call_store_db.py` **신설(6함수)** · 합계 실측 갱신(215파일·2845함수·3555수집 → **216파일·2851함수·3561수집**). 공적 재항 기록을 **원문 그대로** 저장하고 다시 받으면 갱신하는지, 수집이 (선박, 항만청) 단위로 실패를 가두는지, 항구 이름 → 항만청코드를 확인된 범위에서만 잇는지를 고정한다. 대조 규칙·화면은 뒤따르는 PR이다 (#1197) |
