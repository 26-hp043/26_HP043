...
| `test_ops_workflow.py` | 8 | **§5 인프라 · 운영 워크플로**(`#788` · `#789` · `#1635`) — `ops.yml`의 원격 스크립트가 `bash -n`을 통과하는가(9개 이상) · 러너 쪽 셸도 · `restore`는 `confirm=cii`가 없으면 시작하지 않고 교체가 실패하면 앱을 켜지 않는가 · 롤백 실습은 실패해도 `trap`으로 원래 이미지에 돌아오고 `alembic/` 변경이 섞이면 시작하지 않는가 · 배포와 **같은 동시 실행 그룹**인가 · 수동 실행 전용인가 · 백업 아티팩트 보존 14일(`#788` 결정 ①) · 파괴적 명령(`down -v` · `force_db_init` · `volume rm/prune` · `deletedb`)이 없는가. 실행하면 운영 서버를 건드리므로 파일에서 본다 |
...
| 2026-09-26 | `#1945` | §14 `test_ops_workflow.py` **신설(8함수)** · 합계 실측 갱신(220파일·2878함수·3589수집 → **221파일·2886함수·3597수집**). 운영 워크플로(`ops.yml` — 점검·백업(+Actions 아티팩트 14일)·리허설·수집·롤백 실습·복구 교체)가 되돌릴 수 없는 일에 잠금을 거는지와 배포와 겹치지 않는지를 파일에서 고정한다. `AGENTS §4.3`상 행 추가라 버전은 올리지 않는다 (#788) |
