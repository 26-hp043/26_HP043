...
| 2026-09-26 | `#1948` | §3.29 **「공적 기록으로 채우기」 신설**(`IT-FILL-001`~`008`) · §14 `test_public_record_fill_db.py` **신설(10함수)** · 합계 실측 갱신(222파일·2892함수·3603수집 → **223파일·2902함수·3620수집** — 새 파일 15건 + `test_api_spec_request_fields_sync.py`의 `§3.12` 표 대조 파라미터 2건). 잠그는 것은 **누른 칸 하나만 바뀌고 그 출처가 남으며, 확정 항차는 동의 표시 없이 풀리지 않고, 되돌리기·채우기·감사가 한 트랜잭션**이라는 사실이다(`PRD §17.4.4` · `API_SPEC §3.12`). 함께 `test_voyages_api.py`·`test_voyage_state_machine.py`의 대역에 출처 속성 둘을 넣었다(함수 수 불변) · `test_response_contract_db.py`의 항차·구간 계약에 출처 키 넷과 새 라우트의 `ROUTE_COVERAGE`(함수 수 불변). 절 신설이라 `AGENTS §4.3`에 따라 버전을 올린다 (#1923) |
