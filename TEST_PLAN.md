...
| 2026-09-25 | `#1888` | §14 `test_rate_limit.py` 21 → **26함수**(프록시 서명 헤더 5종) · 합계 실측 갱신(212파일·2815함수·3523수집 → **212파일·2820함수·3528수집**). 클라우드에서 요청이 프록시·터널을 거쳐 들어와 소켓 상대가 전원 같았다 — `auth` 10/분을 현장 전원이 나눠 썼다. 고정하는 것은 **비밀 값이 맞을 때만 원 IP로 센다**는 경계다: 맞음 · 틀림(`:8001` 직접 위조) · 서버에 비밀 값 없음(빈 헤더와 빈 설정이 「일치」하지 않는다) · IP 아닌 값 · 미들웨어를 거친 두 사용자 분리. 프록시 쪽은 `frontend/functions/_proxy.test.ts`(vitest)가 헤더 이름 일치·브라우저 위조 헤더 제거를 본다 (#1483) |
