...
**합계 163개 파일 · 2205 함수 · 2697 수집.** (2026-09-20 실측 — #1294 4함수 추가 반영)
...
| 2026-09-20 | `#1312` | §14.2 `test_demo_up_script.py` 24 → **28함수** · 합계 실측 갱신(163파일·2201함수·2691수집 → **163파일·2205함수·2697수집**). `demo_up.sh` 2단계가 실패 원인을 말하게 한 것을 고정한다 — 포트를 쥔 **다른** 컨테이너 이름 짚기(가짜 docker로 실제 실행) · 컨테이너 이름·포트 ↔ compose 대조 · `compose up` 실패 메시지 보존 · healthy인데 호스트 포트가 빈 상태 감지. 돌연변이 2종(우리 컨테이너 필터 제거 · 로그를 `/dev/null`로) 검출. 함수 추가라 `AGENTS §4.3`상 버전은 올리지 않는다 (#1294) |
