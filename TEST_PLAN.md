...
| `test_llm_guard.py` | 11 | **§3.17 통합 · 챗봇 가드 둘**(`#120`) — 전송 화이트리스트가 **정본 표와 같은가**(한쪽만 고치면 걸린다) · 선박명이 막히는가 · **모르는 새 필드가 조용히 빠지지 않고 터지는가** · 파생 수치(빼기도 계산이다)가 막히는가 · 표기 차이(`4.980`)로 정상 응답을 막지 않는가 · 연도·한 자리 수 예외가 너무 넓지 않은가 |
...
**합계 147개 파일 · 1882 함수 · 2358 수집.** (2026-09-13 실측)
