...
| `test_migration_guard.py` | 19 | **§5 DB · 마이그레이션 · 되돌릴 수 없는 downgrade** — 프로덕션에서만 막고 리비전을 하나씩 명시해야 풀리는지(와일드카드 없음) · **명시해도 24시간 안의 백업 기록이 없으면 무엇이든 지우기 전에 막히는지**(`#827`) · 목록의 모든 리비전이 **실제 호출에서** 무엇이든 지우기 전에 끊기는지 · 파괴적 `downgrade()`가 전부 세 분류 중 하나에 들어 있는지 (`DB_SCHEMA §8.1.2` · `#819`) |
...
**합계 164개 파일 · 2247 함수 · 2798 수집.** (2026-09-20 실측 — #1247 19함수 추가 반영)
