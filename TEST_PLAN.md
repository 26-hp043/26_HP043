...
| 파일 | 함수 | 대응 절 |
|---|---:|---|
| `test_account_self_service_db.py` | 19 | **§4.7 API · 인증** — 계정 관리(비밀번호 변경·표시 이름 변경·탈퇴). **응답이 아니라 DB를 다시 읽어** 확인한다 — detached 객체를 고치면 200이 나가는데 아무것도 안 쓰인다 (`#506`). 세션 없음(`UNAUTHORIZED`)과 현재 비밀번호 오입력(`INVALID_CREDENTIALS`)이 **같은 401, 다른 code**로 오는 것도 본다 (`#902`) |
...
| `test_korean_validation_422.py` | 14 | **§4 API · 공통·운영 — Pydantic 422 한국어 문구·라벨** (`#900`). 세 층: 타입 매핑 단위(키·`ctx` 키를 실물 Pydantic과 대조) · 이슈 실측 3건(가입 `display_name` 초과 · `email` 형식 · 로그인 `password` 누락) · **전체 가드**(실앱 OpenAPI를 걸어 본문·쿼리·경로 전 필드의 한글 라벨 등록을 검사 — #55가 기능①을, #900이 나머지 85종을 채운 같은 누락이 반복되지 않게 한다). 미매핑 타입의 폴백이 **영문을 내보내지 않는다**까지 본다 |
...
| `test_voyage_import_db.py` | 28 | **§3.4 통합 · CSV 가져오기 · 커서 페이지네이션** — 수식 주입 4종 escape · 숫자 열은 거부 · 부분 성공(행 번호 보고) · 1000행 상한은 자르되 알린다 · dry-run (`#60`) · **커서 페이지네이션 3종** — 페이지 크기를 넘는 항차에 도달 · **발급한 커서를 서버가 읽는다** · 깨진 커서는 422 (`#627`) · **항차 시각 선택 컬럼** — 시각 저장·빈 칸 NULL · 7컬럼 양식 호환 · 잘못된 형식은 행 오류 · `compute_progress` seam에서 0 기여 대조 · dry-run 안내 (`#906`) |
