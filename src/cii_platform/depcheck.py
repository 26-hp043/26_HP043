...
- **버전은 아예 보지 않는다.** 설치 여부만 확인하며, 범위를 벗어난 버전이 깔려 있어도
  경고조차 내지 않는다. 범위 밖 버전도 대개 동작하고, 여기서 기동을 막으면 실제로 도는
  환경을 세우게 된다. 막는 것은 **아예 없는 패키지**뿐이다 — 그것이 실제 관측된 실패
  모드다.

  ⚠️ 종전 머리말은 *「버전 상한 위반은 **경고로만 남긴다**」*로 적어, **하지 않는 일을
  한다고** 말하고 있었다(`#1350`). 읽는 사람은 범위 밖 버전이면 로그에 무언가 남으리라
  기대하는데 아무것도 남지 않는다. 버전 대조가 필요해지면 그때 넣고 이 문단을 고친다.
"""
