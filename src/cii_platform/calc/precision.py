...
#: **응답 직렬화에서 자릿수를 줄일 때**의 반올림 — `TECH_SPEC §1.2.1` 「응답 직렬화의 절사」
#: (`#1349` → `#1600`). 저장소 공통 정책(:data:`LAYER1_ROUNDING`)의 **유일한 예외**다.
#:
#: 화면·보고서는 받은 문자열을 표시 자릿수로 **반올림**한다(`PRD §9.3` · `DESIGN_SYSTEM §4.2`).
#: 전송 단계도 반올림이면 반올림이 두 번이 되어 드물게 끝자리가 1 올라간다 — `4.9824996`은
#: 원값에서 바로 3자리면 `4.982`인데, 6자리 HALF_UP `"4.982500"`을 거치면 `4.983`이 된다.
#: 절사로 잃는 양은 `10⁻ⁿ` 미만이라 표시 반올림 경계를 넘지 못하므로, 「절사 뒤 표시 반올림」은
#: 「원값 직접 반올림」과 언제나 같다(음수도 대칭 — `ROUND_DOWN`은 0 방향 절사다).
#:
#: **적용 범위 — 전송 자릿수가 표시 자릿수보다 큰 모든 필드다**(`#1600` 결정 「가」). `#1349`는
#: CII 값에 한했고, `#1600`이 비율(`ratio_to_required` · `margin_ratio` · `completeness_ratio`)과
#: 물리량(연료 · CO₂ · 거리 · 시간 · 일수)으로 넓혔다. **그대로 `ROUND_HALF_UP`인 것** — 전송 =
#: 표시 자릿수인 필드(연료 소수 1자리 · 연료 비중 1자리 — 절사하면 그 문자열이 곧 표시다) ·
#: Layer 2 확률(소수 4자리가 곧 정본) · 표시 자릿수가 정해지지 않은 값(``quantity`` 6자리 ·
#: 금액 2자리 · 좌표) · 보고서 표시 단계.
SERIALIZATION_ROUNDING = ROUND_DOWN
