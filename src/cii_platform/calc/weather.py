...
``WeatherSnapshot``은 ``wave_period_s``를 받아 저장하지만 **두 모델 어느 쪽도 쓰지
않는다** — Townsin–Kwon α식은 파고(BN 환산)와 풍속만 쓰고 주기 항이 없고,
``SIMPLE_RULE``은 그 둘의 선형 근사다. **쓰지 않는 것이 맞고, 적혀 있지 않은 것이
결함이었다**(`TECH_SPEC §3.6` 한계 표에 행을 두었다).

저장은 계속한다. 그때의 바다 상태 기록이라 **다른 모델을 쓸 때의 근거**가 되고,
저장을 멈추면 지난 구간은 소급해 복구할 수 없다.
