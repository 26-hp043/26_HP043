...
하드코딩하지 않는다"* 고 적는다. 그 표는 ``#434``에서 생겼다(``DB_SCHEMA §2.19``).

이 모듈은 그때도 지금도 분포를 **인자로 받고** 기본값을 :data:`DEFAULT_PROFILE`로
둔다 — 표를 읽는 것은 호출부(``services/annual_simulation.py``)의 몫이라 여기는
바뀌지 않았다.

⚠️ **종전 이 자리는 「그 테이블은 DB_SCHEMA에 정의가 없고 실제로 존재하지도
않는다」로 적혀 있었다** (``#1347`` 정정). ``#434``가 만든 뒤로 낡은 채 남아,
읽는 사람에게 **있는 것을 없다고** 말하고 있었다.
"""
