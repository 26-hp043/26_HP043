...
중간 단계에서 자릿수를 자르지 않고 끝까지 이어 계산한다. 그 규칙은 ``TECH_SPEC §1.2.1``에
있다 — **작업 정밀도**(정본값 자릿수 + 최소 20, 지금 50)를 **Layer 1 진입점에서만** 걸고
(:func:`~cii_platform.calc.precision.layer1_context`), 결과 자릿수는 그와 따로 둔다
(`#166` · `#179`가 신설). 종전 이 문단은 「정본에 중간 처리 규정이 없다」고 적었는데
그 조항이 생긴 뒤에도 남아 있었다(`#759` 정정).
