...
**소요시간은 같지 않다** (`#1405`). 없는 계정은 곧바로 돌아오고, 있는 계정은 토큰 발급과
메일 발송을 거친다. 시간을 맞추려면 발송을 응답 뒤로 미뤄야 하는데, 그러면 아래
「메일 실패」를 사용자에게 알릴 자리가 사라진다. 시간 균등화는 로그인 하나만
보증한다(`API_SPEC §1.2`) — 이 시간 차가 드러내는 「가입돼 있다」는 회원가입 409가
이미 알리는 사실이다(`PRD §6.3` 비대칭).
