...
async def list_calculations_route(
    ...
    ``vessel_id``로 최신 20건씩 받아 ``needs_recalc``를 「재계산 필요」로 보인다.
    `#556`·`#776`은 이 엔드포인트를 「화면에 연결하지 않는다」로 판정했다 — `PRD §5`가
    그 화면을 규정하지 않았다는 것이 근거였다. 2026-09-11 결정 3-② 「범위 밖은 없다」로
    `PRD §5.1` 「선박 상세」 행에 계산 이력이 들어갔고(`AGENTS §3.2.3` — 범위 판정이
    먼저), 화면이 이 엔드포인트를 쓴다. 해시 두 개로 찾는 재현성 검증 용법은 그대로다.
    """
