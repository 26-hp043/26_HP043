...
async def list_calculations_route(
    ...
    **`PRD §12.4.3` seed 정책** · **`PRD §8.4` 재계산 정책**이다. ⚠️ `#556`은 앞의 셋만
    보고 판정했고 그래서 `reproduce`가 `§12.4.3`의 버튼 요구를 놓쳤다(`#776`). 이
    엔드포인트는 다섯을 모두 보고 **다시 범위 밖으로 확정**했다 — `§8.4`의 「재계산 필요
    표시」는 ``needs_recalc`` 플래그를 세우는 처리이고(`#944`), 그 플래그를 보여 주는
    화면은 규정하지 않는다. 용법을 적은 곳은 하위 문서 `API_SPEC §1.8`·`§1.9`뿐이다.
