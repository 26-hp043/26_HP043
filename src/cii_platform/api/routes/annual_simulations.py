...
async def list_snapshot_voyages_route(
    ...
    **페이지네이션을 두지 않는다.** 한 실행의 스냅샷은 그 자체가 하나의 근거 묶음이라
    ...
    ## 화면 — 기능③ 결과의 「이 실행에 쓴 항차」 (#992)

    재현 정보 아래 펼침이 **펼칠 때** 이 목록을 받는다. `#556`·`#776`은 「화면에 연결하지
    않는다」로 판정했으나(근거는 `PRD §5`의 부재), 2026-09-11 결정 3-② 「범위 밖은 없다」로
    `PRD §5.1` 「기능③」 행에 들어갔다(`AGENTS §3.2.3` — 범위 판정이 먼저).
    """
...
async def reproduce_annual_simulation_route(
    ...
    **`POST`인데 아무것도 만들지 않는다.** 계산을 다시 돌리는 것은 부작용이 없는
    조작이 아니므로(비용·시간) `GET`으로 두지 않았고, `API_SPEC §6.4`가 `POST`로 규정한다.
