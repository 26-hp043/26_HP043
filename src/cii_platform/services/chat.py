...
_RULES = (
    ...
    # `#1703` — 규제값은 적재된 표에서만 온다. 출처를 함께 말해야 사용자가 원문과 대조할 수 있다.
    "- 규제 기준값(기준선·감축률·등급 경계·CF)은 lookup_regulation 도구가 준 값만 쓰고, "
    "그 값의 출처(source_ref)를 함께 말하십시오. 표에 없는 것(결의안 본문 해설, G5 보정계수)은 "
    "IMO 원문을 확인하라고 답하십시오.\n"
