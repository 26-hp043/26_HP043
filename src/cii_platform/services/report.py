...
async def _self_check_section(
    ...
    **리포트 생성 자체가 422로 막힌다**(`services/ytd_cii.py`). 이 표에 행을 두면 늘
    ...
    def _verdict(count: int) -> str:
        """판정 두 말 — `PRD §25.3` (2026-09-18 확정 · `#1052` ⓶).

        종전에는 0건을 **「이상 없음」**이라 적었다. 그러면 절의 고지가
        *「이 표의 「이상 없음」은 규제 적합 판정이 아닙니다」*라고 **자기 말을
        부인**해야 했다 — 부인이 필요하다는 것 자체가 단어가 과했다는 증거다.

        「해당 없음」은 **건수가 0이라는 사실**만 말한다. 적합을 주장하지
        않으므로 부인할 것도 없다.
        """
        return "확인 필요" if count else "해당 없음"
    ...
    return TableSection(
        ...
        note=(
            ...
            " 내부 자료이며, 이 표의 판정은 규제 적합 판정이 아닙니다."
