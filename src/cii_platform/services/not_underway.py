...
2. **구간은 겹치지 않는다.** 같은 정박이 두 번 들어가면 ``M``이 두 배가 된다 —
   ...
   **동시 요청에서도** 그렇다 — 겹침을 보기 전에 선박 행을 잠가 직렬화한다
   (``_assert_no_overlap`` · `#1629`).
...
async def _assert_no_overlap(
    ...
    *,
    ...
    ## 조회 전에 선박 행을 잠근다 (`#1629` · `F-8` 결정 · `TECH_SPEC §16.3`)

    두 요청이 같은 선박에 겹치는 구간을 **동시에** 올리면, 잠금 없이는 둘 다 「겹치는
    것이 없다」를 읽고 둘 다 저장된다 — READ COMMITTED에서 상대의 미커밋 INSERT는
    보이지 않고 기다리지도 않는다(`#1796` 실측 · 3/3에서 2건이 남았다). 그러면 같은
    정박의 연료가 두 번 세어져 등급이 조용히 나빠진다.

    잠그는 것은 **부모 행(선박)** 이다 — 겹칠 구간은 아직 없는 행이라 잡을 수 없다.
    두 번째 요청은 첫 요청이 커밋할 때까지 이 자리에서 기다렸다가 **커밋된 구간을
    보고** 409로 떨어진다. 생성·수정·CSV 가져오기가 전부 이 함수를 지나므로 잠금은
    여기 한 곳에 둔다 — 호출부마다 두면 하나가 빠진다. 트랜잭션 경계는 그대로다
    (커밋·롤백 때 함께 풀린다).
    """
