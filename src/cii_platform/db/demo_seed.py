...
#: 하는 배분이다 — 항해 중(남중국해)·운하 통과(수에즈)·묘박(부산).
SEED_STATE_UPDATES: list[tuple] = [
    # (vessel_id, underway_state, detail_status, lat, lon, position_updated_at)
    #
    # 벌크선은 **진행 중 항차 `2026-02`(부산 → 싱가포르) 위에 있어야 한다** (#1672).
    # 종전 값은 대한해협(34.51, 128.50) · **적재 10일 전**이라, 5일 전에 출항한 항차와
    # 한 화면에 놓이면 「출항 전 시각에 항해 중」인 배가 됐다. 화면은 이 위치를 **기록된
    # 시각과 함께** 그대로 보인다(보간하지 않는다 — `#1672` 결정 A). 그래서 시드가 맞아야 한다.
    #
    # 시각은 적재 기준 자정(`_rel(0)`) — 출항(`_rel(-5, 6)`) 뒤다. 그때까지 4.75일 × 14kn
    # ≈ 1,596nm, 계획 2,300nm의 약 69%다. 좌표는 그 지점 근처의 남중국해(베트남 동쪽
    # 먼바다 · 해안선은 동경 109도대)로, 부산·싱가포르 대권거리 비로 약 67%에 놓인다.
    (BULK, "UNDER_WAY", "SAILING", "13.200000", "111.100000", _rel(0).isoformat()),
    (
        CONTAINER,
        "NOT_UNDER_WAY",
        "CANAL_TRANSIT",
...
SEED_VOYAGES: list[dict[str, object]] = [
    ...
    },
    {
        ...
        #
        # 5일 전 출항 · 8일 뒤 도착 예정. 2,300nm ÷ 14kn ≈ 6.8일이고 창은 13일이다 —
        # 그 차이는 **항만 체류·대기 여유**다. 적재 직후 진행률은 **약 69%**이고
        # 약 2.1일 뒤 계획 거리를 채운다 — 실시간 CII 화면의 「진행 중 항차 기여」
        # 카드가 자라다가 멎는 모습을 함께 보여 준다.
        #
        # ⚠️ **종전 이 주석은 「13일 창은 정박 구간을 포함한 여유 · 적재 직후 약
        # 40%」로 적혀 있었다** (`#1321` 정정). 이 선박에는 정박 구간 시드가 없고
        # 실제 값은 69%였다. 그 여유가 정박으로 들어가 있지 않았으므로 **여유가
        # 그대로 거리로 바뀌어** ETA에서 계획의 **186%**가 나왔다.
        #
        # 값이 날마다 자라는 문제는 둘로 나뉜다 — 예정일 초과는 `#649`가,
        # **예정일 안에서 계획 거리를 넘는 것**은 `#1321`이 막았다. 여기서 상대
        # 시각을 쓰는 이유는 **값이 아니라 화면**이다 — 예정일이 지나면
        # `IN_PROGRESS_PAST_ETA` 경고가 서고, 시연에서 「도착 예정일이 한 달 지난
        # 진행 중 항차」가 보인다.
...
SEED_VOYAGES_WATCH: list[dict[str, object]] = [
    ...
    },
    {
        ...
        #
        # `#1321` — 창을 **계획 거리에 맞춘다.** 500nm ÷ 13kn ≈ **1.6일**인데 창이
        # 6일이었다. 시계가 `속력 × 경과시간`으로만 거리를 내던 때는 적재 직후부터
        # 이미 계획의 **109%**였고 ETA에서 **359%**였다. 상한이 생긴 지금도 창을
        # 그대로 두면 카드가 **적재 첫 순간부터 100%**로 서서 시연 소재를 잃는다.
        # 1일 전 출항(실적 +5h) · 1일 뒤 도착 예정 → 적재 직후 약 **49%**이고
        # 19.5시간 뒤 계획 거리를 채워 `IN_PROGRESS_PLANNED_DISTANCE_REACHED`가 선다.
        "planned_departure_at": _rel(-1),
        "planned_arrival_at": _rel(+1),
        "actual_departure_at": _rel(-1, 5),
        "actual_arrival_at": None,
    },
    {
