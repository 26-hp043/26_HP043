...
REGENERABLE: dict[str, str] = {
    ...
    # 시각 출처 열 넷(#1923) — 059와 같은 성격. 컬럼·트리거 모두 재생 가능하고 NULL 허용.
    # 되돌리면 「공적 기록에서 채움 · 직접 입력」 표시가 사라지지만 시각 값은 그대로이고, 그
    # 결과는 064 이전과 같은 「모른다」다 — 계산·등급·집계 어디에도 들어가지 않는 표시 값이다.
    "064": "voyage·not_underway_period 시각 출처 넷 — 표시 값이라 사라져도 「모른다」로 돌아갈 뿐",
}
