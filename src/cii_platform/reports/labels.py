...
## 세 번째 구분 — 문서 전용 문구 (#645)

위 두 구분은 **옮겨 적을 원본이 있을 때** 성립한다. 그런데 **화면에 나오지 않는 값**이
있다 — :data:`FUEL_SOURCE_LABELS`(``voyage_fuel_use.source``)가 그렇다. 화면은 이 값을
만들어 전송만 하고 표시하지 않으므로(``voyage-management/apiProvider.ts``), 「화면 쪽이
원본이다」가 성립하지 않는다. 그렇다고 정본이 문구를 확정한 적도 없다.
...
정본 문구               정본이 원문을 확정                           정본
표시 문구               화면                                         화면
**문서 전용 문구**      **없음 — 문서에서만 사람이 읽는다**          **코드 집합만** 정본과
======================  ==========================================  ==========================

**문구는 이 파일이 정하되 코드 집합은 정하지 않는다.** ``source``의 4값은
``DB_SCHEMA §2.3``·``PRD §7``이 확정했고 ``voyage_fuel_use``의 ``chk_fuel_source``가
그것을 강제한다. 동기화 테스트는 **그 CHECK 제약에서 직접 읽어** 대조한다 — 목록이
갈리면 새 출처가 리포트에서 원문 코드로 나간다.

이 구분을 열어 둔 것은 예외를 늘리기 위해서가 아니다. **화면에 없는 값을 「표시 문구」로
분류하면 대조 상대가 없어 동기화 테스트가 아무것도 검사하지 않는다.** 어느 쪽인지
먼저 정하는 편이 낫다.
...
WARNING_LABELS: dict[str, str] = {
    ...
    "COMPLETED_FUEL_UNFILLED": (
        "연료 기록이 없는 실적 확정 항차가 있어 그 항차의 연료가 누적에 "
        "반영되지 않았습니다. 해당 항차에 연료를 입력해 주세요."
    ),
    "SLOW_SPEED_FLOOR": (
    ...
    "IN_PROGRESS_PLANNED_DISTANCE_REACHED": (
        "진행 중 항차가 계획 거리를 모두 채웠습니다. 누적은 계획 거리까지만 "
        "반영했으며, 도착 실적을 입력하면 확정됩니다."
    ),
...
#: 연료 출처 표기 — **문서 전용 문구**다 (`#645`). 화면은 이 값을 표시하지 않는다.
#:
#: 코드 집합의 정본은 ``voyage_fuel_use``의 ``chk_fuel_source`` CHECK 제약이며
#: (``DB_SCHEMA §2.3`` · ``PRD §7``), 이 표는 **문구만** 정한다.
#:
#: 출처를 문서에 남기는 근거는 ``DESIGN_SYSTEM §11``(🔒)이다 — *「표 안에서도 동일하게
#: 적용한다 (셀 단위)」*. 화면은 점선 밑줄·``ⓘ``·툴팁으로 실측과 추정을 가르지만
#: **CSV·PDF에는 그 수단이 없어 문자열이 유일한 채널**이다.
FUEL_SOURCE_LABELS: dict[str, str] = {
    "USER_INPUT": "직접 입력",
...
def fuel_source_label(code: str | None) -> str:
    """연료 출처 코드를 문서 표기로 (`#645`)."""
    return _label(code, FUEL_SOURCE_LABELS)
