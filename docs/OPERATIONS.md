...
> 최종 갱신: 2026-09-26 (§3.8 백업 전 `backups/` 소유자 되돌림 · 점검에 db 컨테이너 상태·챗봇 감사 흐름 · #788 · §3.1·§5.2 배포 시크릿을 러너에서 `.env`로 만들어 base64로 넘기고 `CUBRID_PASSWORD` 끝 줄바꿈을 보존 · #1634 · §3.8 운영 워크플로 `ops.yml` — 백업·리허설·수집·롤백 실습·복구 교체를 수동 실행으로 · #788 · §1.1 백엔드 `:8001`을 루프백에만 게시 · 배포 헬스체크를 터널 주소로 · #786 · §1.2.1 감사 로그·세션 IP도 같은 판정 · #1889 · §3.1.1 시연 동결 `DEPLOY_FROZEN` · §3.6.1 헬스 `commit` 확인 · #789 · §9.2.1 이름 있는 볼륨으로 옮기기 — 배포가 옮기기 전 상태를 보고 멈춘다 · #1867 · §1.2.1 프록시 서명 헤더 · #1483). 이 문서는 BlueLog(CII 플랫폼)의 OCI 배포 전체를 다룬다.
...
### 3.8 운영 워크플로 `ops.yml` — 서버에서만 할 수 있는 일 (#788 · #789 · #1635)

서버 SSH 키는 GitHub 시크릿에만 있다. 백업·복구·롤백처럼 **서버 안에서 돌려야 하는 일**은
**Actions → Ops → Run workflow**로 한다. 배포와 같은 동시 실행 그룹이라 배포 도중에 끼지 않는다.

| task | 하는 일 | 서비스 영향 |
|---|---|---|
| `inspect` | db-01 마운트·**컨테이너 상태**(`#1634` 헬스체크)·볼륨·디스크·백업 목록·표별 행 수·**챗봇 감사 흐름**(`#1535` — 질문·도구 호출·답 저장의 시각 순서. 본문·사용자·IP·해시는 찍지 않는다) · app-01 이미지·헬스·포트 | 없음 |
| `backup` | `backups/`가 SSH 계정으로 쓸 수 없으면(서버에서 `sudo`로 직접 돌린 백업이 root 소유로 남긴 경우) `sudo -n chown`으로 되돌린 뒤 `db_backup.py backup` + 덤프·매니페스트를 **암호화해서** Actions 아티팩트(`db-backup-<실행번호>` · `*.enc` · 14일)로도 올린다(`#788` 결정 ① · 정정 A). 시크릿 `BACKUP_ARTIFACT_PASSPHRASE`가 없으면 올리지 않는다 | 없음 |
...
| `CUBRID_PASSWORD` | dba 비밀번호 (<=31바이트, ASCII). ⚠️ **`'`는 쓰지 않는다** — 전달은 `#1634`로 문자와 무관해졌지만, db-01 첫 부트의 `ALTER USER dba PASSWORD '…'`(`deploy.yml`)가 값을 SQL 작은따옴표로 감싼다. ⚠️ **끝 줄바꿈도 값의 일부다** — 운영 값은 끝에 줄바꿈이 1개 있고 DB의 dba 비밀번호도 그 값으로 설정돼 있다(`#1944` 진단 · 2026-09-26). 배포는 그 줄바꿈까지 바이트 그대로 넘긴다(`#1634`). 시크릿을 다시 넣을 때 끝 줄바꿈을 빼면 DB 비밀번호와 달라져 `deploy-db` 브로커 대기가 실패한다 | |
