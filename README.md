...
| 2026-09-26 | `#___` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.29**로 — `§9.5` 마커 팝오버 구현 실측 반영(`#1831`). **겹친 배를 묶지 않는다**(같은 항구 3척이 중심 간격 2px로 겹치고 타일이 `z6`까지라 확대해도 풀리지 않으나, 좌측 패널 행의 「지도에서 보기」가 **선박 id로** 카드를 열어 겹친 배도 전부 열린다) · 닫는 길에 **닫기 버튼 · 지도 확대** · v2.28이 적은 **[실시간 CII] 버튼 제거**(그 경로가 항차 id를 요구한다) |
