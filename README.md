...
| [`TECH_SPEC.md`](./TECH_SPEC.md) | 기술 명세서 (**v1.10**, 서비스 레이어 아키텍처 #100 + 재현성 계약 #102 + Layer 1 계산 규칙 #166 + 시뮬레이션 분포 프로파일 #434 + 메일·리포트 절 신설 #446 + **§10.3 NumPy 업그레이드·재현성 한계 #833·#106** + **§16.2 디렉터리 트리 실측 갱신 #1081**) — 이중 정밀도 엔진, PCG64DXSM RNG, capacity 분리(transport/reference), canonical hashing, 스냅샷 격리(§11), 서비스 레이어(§16), 메일 발송(§18), 리포트 렌더링(§19) | ✅ 완료 |
...
| [`DESIGN_SYSTEM.md`](./DESIGN_SYSTEM.md) | 디자인 토큰 계약서 (**v2.10**, 등급 3색 체계 · 다크 모드 · 토큰 접두어 `--cii-*` #463 + `UIFLOW` 참조 정리·신뢰도 배지 표시 임계 #583 + §4.2 일수·속력 자릿수 #592 + §8.2 CII 적용 대상 배지 #653 + §4.2 용량 자릿수 #633 + **rlatnals4114 확정 12건 반영** — §16 미확정 6항목 종결·§0.2 제약 6·§5 〔확정〕/〔제안〕 표기·§12 아이콘 세트·§15 `--brand-*` #747 · #694 + **§8 BrandLogo·§7.2 브랜드 판 반응형 #934** + **rlatnals4114 2026-09-11 확정 반영** — §5 확정/제안 정의·오버레이 규정·§8 슬라이더·§8.3 등급 전이 #930 · #932 · #933 + **§8.4 폼 필드·필드 오류 #936**) — 컬러·타이포그래피·숫자 포맷·차트 규약·접근성 | ✅ 완료 |
...
| 2026-09-17 | `#1080` · `#1081` · `#1104` · `#1171` · `#1176` | **문서 동기화 부채 일괄 정정 (5이슈).** ⑴ **핵심 기능·MVP 표를 `PRD §5.1`(09-13 개정)에 맞춤** — 함대 감축 계획·데이터 점검·실적 보정계수·AI 어시스턴트(실험) 행 추가, 제외 목록의 「지도 기반 항로 렌더링」 삭제(`PRD §5.2` 범위 안 개정 · #763). ⑵ 문서 구조 표 5행 갱신 — `PRD` v4.14 · `TECH_SPEC` v1.10 · `DB_SCHEMA` v1.28(**20개 → 25개 테이블**, chat 2개 등재 #1080) · `TEST_PLAN` v1.26 · `UIFLOW` v2.13. ⑶ 헤더 최종 수정일 09-11 → 09-17(#1081 ⑼ — 본문 커밋일 09-16보다 5일 낡아 있었다) (#1081 ⑴⑵⑶⑼) |
