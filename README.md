...
> **단일 조직 배포 전제** — 한 선사(조직)가 자기 인스턴스를 쓰는 것을 전제로 합니다. 데이터 격리는 범위 밖이므로(`DB_SCHEMA §9.2` · #808) 여러 선사의 공동 사용은 **인스턴스 분리**로 대응합니다.
...
| 계층 | 기능 | 설명 |
|---|---|---|
| **선대** | **대시보드** | 보유 선박 전체의 현 등급·운항 상태·위치를 한 화면에서 조망하고 위험 선박을 식별 — **선박 위치 지도**(PMTiles 로컬 자산, 없으면 개략도 폴백 #763) |
| **선대** | 함대 감축 계획 | 선박별 감속으로 목표 등급 달성 여부·추가 항해일·비용을 함께 보고 계획안 저장 (`UIFLOW 2-10` · `PRD §12.3.2` · #513) |
| **선대** | 데이터 점검 | 실측이 아닌 값이 쓰인 항차를 선대 단위로 — 대체 계산 · 계산 불가 · 이상치 · 실적 확정 전 · 완결성 (`UIFLOW 2-11` · `PRD §17.4` · #513) |
| **선박** | **선박 상세** | 연도별 CII 이력, 올해 누적(YTD) 등급, 현재 위치·상태 |
| **선박** | 연간 CII 시뮬레이터 | 누적 실적 + 잔여 계획 기반 연말 예상 등급, 목표 달성 확률(Monte Carlo), 민감도 분석, **실적 보정계수 옵션**(`PRD §12.2.1` · #363) |
...
| 공통(실험) | AI 어시스턴트 | 자연어 질의 → 선박·계산 데이터 조회와 결과 설명 — 핵심 경로 제외 실험 기능(`UIFLOW 2-7` · `PRD §5.1` MAY · #120) |
...
| 문서 | 내용 | 상태 |
|---|---|---|
| [`PRD.md`](./PRD.md) | 제품 요구사항 정의서 (**v4.17**, **§17.4.4 공적 기록과 다름 #1197** + 자체 ID/PW 인증 전환 #413 + **역할 2종(사무직·현장직) #672** + **관리자 분리 — 역할 3종 #1301** + 관리 중심 전환 #343 + 보고서 절 #360 + 계산식 스코프·실시간 CII #358 + **§12.3.1 필요 감축량(목표 역산) #433** + **§8.4 선종 변경 재계산 #944** + **§3.3.8 진행분 유종별 배분 #885** + **§6.4 상태 문구 #931** + **§12.2.1 실적 보정계수 #363** + **§6.4 「선행 선택 필요」 패턴 #1171** + **§5.1 화면 언어 전환(실험) #1215**) — 이중 capacity 규칙(G1/G2 분리), 상태 모델, 값 우선순위(§8.3), 등급 하락 귀결(§3.3.7), 보고서 정의(§25) | ✅ 완료 |
| [`TECH_SPEC.md`](./TECH_SPEC.md) | 기술 명세서 (**v1.15**, 서비스 레이어 아키텍처 #100 + 재현성 계약 #102 + Layer 1 계산 규칙 #166 + 시뮬레이션 분포 프로파일 #434 + 메일·리포트 절 신설 #446 + **§10.3 NumPy 업그레이드·재현성 한계 #833·#106** + **§16.2 디렉터리 트리 실측 갱신 #1081** + **§5.2.1.2 기능③ parameters_used v1·v2 #1306** + **§19.2 CSV 수치 열 선언 #1247** + **§7.1·§7.3 기상 조회 계층 실측 정합 #968** + **§1.2.1 응답 직렬화의 절사 #1349**) — 이중 정밀도 엔진, PCG64DXSM RNG, capacity 분리(transport/reference), canonical hashing, 스냅샷 격리(§11), 서비스 레이어(§16), 메일 발송(§18), 리포트 렌더링(§19) | ✅ 완료 |
| [`API_SPEC.md`](./API_SPEC.md) | REST API 명세서 (**v1.47**, **공적 기록으로 채우기 §3.12 #1923** + 인증 재작성 #414 + **역할 2종·사무직 전용 경로 #672** + **관리자 전용 경로 #1301** + **호출부호 call_sign #1197** + **CSV 수치 열 선언 #1247** + **계획 거리 출처 planned_distance_source #1256** + **등급 경계 CII rating_boundary_cii #1371** + not under way CRUD #370 + 실시간 CII 3종 #354 + 리포트 #361 + 연간 시뮬레이션 #64 + 선대 요약 사유 구분 #419 + 대체 내역 기록 #449 + 항차 실적 입력 #440 + CII 적용 대상 표시 #653 + CSV 내보내기 컬럼 확정 #59 + 샘플 선박 목록 #982 + 챗봇 API #121 + **파라미터 Import CSV #673** + **vessel.block_coefficient #966** + **선박별 연간 실행 목록 §6.5 #1805** + **올해 누적 CII 추이 §2.18 #1671** + **해상 경로망 §3.11 #1300**) — 수치 문자열 직렬화(§1.7), `as_of` 공통 계약, field_label 오류 체계, CSV escape 보안 | ✅ 완료 |
| [`DB_SCHEMA.md`](./DB_SCHEMA.md) | 데이터베이스 스키마 (**v1.38**, **실제 시각 출처 열 넷 064 #1923** + **공적 재항 기록 063 #1197** + **활성 키 열·유니크 인덱스 061 #1631** + not under way 스키마 #345 + **app_user.role #672** + **role 3종·값 트리거 057 #1301** + **vessel.call_sign 058 #1197** + **voyage.planned_distance_source 059 #1256** + 운항 상태 2축 #346 + CF 스냅샷 #378 + 인증 전환 #414 + simulation_parameter #434 + 데모 seed 분리 #451 + **CUBRID 제약 전면 갱신 #1058** + **chat_session·chat_message 등재 #1080** + **활성-유니크 트리거 #673** + **block_coefficient #966** + **head 059 대조 #1342** + **스냅샷 정박 몫 060 #1803**) — **26개 테이블**, **CUBRID 11.4.6**, FK ON DELETE 정책, immutable 트리거, 마이그레이션 전략 | ✅ 완료 |
| [`TEST_PLAN.md`](./TEST_PLAN.md) | 테스트 계획서 (**v1.32**, **§3.29 공적 기록으로 채우기 #1923** + §5.10 격리 수준 전제 가드 · 활성 키 유니크 #1631 + §3.28 해상 경로망 #1300 + §4.11 올해 누적 CII 추이 API #1671 + §1.6 픽스처·테스트 격리 재작성 #1583 + Layer 1 픽스처 정본값 규칙 #166 + §14 파일 인벤토리 #394 + 방향 전환 반영 #398 + 배포 배선 고정 #508 + 테스트 격리 #507 + 계정 관리 #506 + **§3.25~3.27 신설 · §10 실CI 재작성 · §14.3~14.5 갱신 #1104·#1081** + **IT-IMPORT CSV 계약 #673**) — 파일·함수·수집 수는 `TEST_PLAN §14.2` 합계 문장이 정본(CI가 실측과 대조), Fixture 1~4, 정본값 생성기 계약(§1.7), 이중 capacity 검증 | ✅ 완료 |
...
| [`DESIGN_SYSTEM.md`](./DESIGN_SYSTEM.md) | 디자인 토큰 계약서 (**v2.30**, 등급 3색 체계 · 다크 모드 · 토큰 접두어 `--cii-*` #463 + `UIFLOW` 참조 정리·신뢰도 배지 표시 임계 #583 + §4.2 일수·속력 자릿수 #592 + §8.2 CII 적용 대상 배지 #653 + §4.2 용량 자릿수 #633 + **rlatnals4114 확정 12건 반영** — §16 미확정 6항목 종결·§0.2 제약 6·§5 〔확정〕/〔제안〕 표기·§12 아이콘 세트·§15 `--brand-*` #747 · #694 + **§8 BrandLogo·§7.2 브랜드 판 반응형 #934** + **rlatnals4114 2026-09-11 확정 반영** — §5 확정/제안 정의·오버레이 규정·§8 슬라이더·§8.3 등급 전이 #930 · #932 · #933 + **§8.4 폼 필드·필드 오류 #936** + **§8.5 어시스턴트 오버레이 「답이 아닌 것」의 표시 #1051** + **§16 항목 4 self-host 마감 #1171** + **§14 비활성 컨트롤의 사유 표기 확정·§16 항목 20 #1170** + **§12 아이콘 규격값의 출처를 Figma로 일원화·§15 `--icon-*` #1174** + **§14 경계와 장식을 가르는 1.4.11 적용 기준 #1202** + **§14 누르면 사유를 내는 버튼 #1415** + **§7.1 외곽 여백 24·산술 정정 · §7.2 사이드바 아이콘 허용 #1450** + **§3 영문 약어 병기의 범위와 예외 #1452** + **§2.5 (b) 위험도 단계별 색·아이콘 색 확정 #1674** + **§14 채움 Primary의 경계를 테두리가 맡는다 #1296** + **§8 텍스트 버튼 높이 확정 #1297** + **§4.4 기준 시각 형식 #1527** + **§8.6 결론 띠 · §8.7 입력-결과 2단 · §5 카드 예산 · §2.3 경고색 한 번 #1690** + **§8.6 위험도 pill의 조건 #1728** + **§14 문서 언어 경계 — 루트는 `ko`, 영문 자리에만 `lang="en"` #1652** + **§7.1 분할하지 않는 화면 · §8.6 감축 계획 보조 하나 #1756** + **§3 `page`(28) 신설 · 본문 15 #1763** + **§8 탭 신설 #1772** + **§9.5 본문 전체 지도 · 마커 방향 · 팝오버 #1821** + **§9.5 겹친 배를 묶지 않는다 · 닫는 길 둘 · [실시간 CII] 정정 #1831** + **§7.2 좁은 창에서 사이드바 축소(64) · 상단바 유틸리티 폭 #1885**) — 컬러·타이포그래피·숫자 포맷·차트 규약·접근성 | ✅ 완료 |
| [`UIFLOW.md`](./UIFLOW.md) | UI Flow 명세서 (**v2.17**, 선대 계층에 함대 감축 계획·데이터 점검 신설 #505 + 하위 절 번호 부여 #583 + 두 화면을 1차 심사 범위 밖으로 판정 #513 + §2.2.1 사이드바 순서 신설 #712 + 2-7 AI 어시스턴트 주요 UI #121 + **§2.2 역할 열 #672** + **§2.2 관리자 열·2-6 계정 관리 관리자 전용 #1301** + **임계값 문구 정정 #1081** + **2-6 「규제 기준값」 절 신설·SCR-006 범위 안 #1239·#1516** + **09-12~13 신설 화면 임시안 확정 #1052** + **`2-8` 선박 상세 탭 넷 #1772**) — 화면 목록·계층·흐름·진입 조건 | 🟡 초안 |
...
- **선대 대시보드 및 위험 선박 경고 배너** — 선박 위치 지도 포함(PMTiles 로컬 자산, 없으면 개략도 폴백 #763)
- **함대 감축 계획 · 데이터 점검** (`UIFLOW 2-10`·`2-11` · #513)
...
- 항차 CII 추정 (기능①) · 운항 시나리오 비교 (기능②) · 연간 CII 등급 시뮬레이터 (기능③, 실적 보정계수 옵션 포함 #363)
- 선박 등록·관리 및 샘플 선박 제공
...
**계층 스택** — 정보 구조(선대 → 선박 → 항차)를 그대로 따릅니다.

| 라벨 | 레이어 | 범위 |
|---|---|---|
| `layer:base` | L0 기반 | 데이터 · 스키마 · 시드 · 수집 경로 — 전 계층의 토대 |
| `layer:calc` | LC 계산 | CII 산식 · 규제값 · 재현성 · 시뮬레이션 엔진 |
| `layer:fleet` | L1 선대 | 대시보드 · 경고 배너 · 함대 계획 |
| `layer:vessel` | L2 선박 | 상세 · 연도별 이력 · YTD · 연간 시뮬 |
| `layer:voyage` | L3 항차 | 실시간 CII · 시나리오 · 운항 기록 |
| `layer:report` | L4 산출물 | 리포트 · 내보내기 · 통계 |

**계층을 가로지르는 축** — 어느 한 계층에 속하지 않는 관심사입니다.

| 라벨 | 레이어 | 범위 |
|---|---|---|
| `layer:account` | LU 계정 | 인증 · 세션 · 계정 관리 · 권한 |
| `layer:assistant` | LA 어시스턴트 | 챗봇 · 자연어 인터페이스 (`UIFLOW 2-7`) |
| `layer:ui` | LD 화면 규격 | 디자인 시스템 · 공통 컴포넌트 · 접근성 |
| `layer:platform` | LP 플랫폼 | 배포 · 인프라 · 운영 환경 |
| `layer:assurance` | LV 검증 | 테스트 가드 · 정본 정합 · 성능 · 추적 |

**별도 트랙** — 기존 계층에 섞으면 그 계층이 밀려 보이는 묶음입니다.

| 라벨 | 레이어 | 범위 |
|---|---|---|
| `layer:2.5d` | L2.5 2.5D 시각화 | 연간 시뮬 결과의 2.5D 장면(지도 · 선박 · 재생) — 38건 · 담당 미정 · 결정요청 제외 (`#1548`) |

**이 축은 「어느 계층인가」에만 답한다.** 「할 것인가」는 `PRD §5`가, 「지금 시작할 수 있는가」는 `blocked` 라벨이 답한다.
**한 이슈에 계층 라벨은 하나만 붙인다** — 둘 이상이면 `layer_of()`가 위 표 순서로 앞엣것을 쓰고, 그 귀속이 조용히 정해진다.

> **폐지 라벨** — `layer:backlog`(`#777`)와 `layer:cross`(`#794`)는 **비웠지만 지우지 않았다.** 닫힌 이슈가 그 라벨을 달고 있어 삭제하면 이력이 사라진다.
>
> `layer:cross`는 `LAYERS` **맨 뒤에 남긴다.** `#777`이 `layer:backlog`를 목록에서 아예 뺐는데, 그러면 그 라벨만 달린 open 이슈가 `(미부착)`으로 떨어져 **`#93`이 08-22에 겪은 「표에서 사라져 없는 것이 되는」 상태**가 재현된다. 0건 행이 폐지 사실 자체를 보이게 한다.

> **왜 갈랐나** (`#794`) — `layer:cross`(「횡단 — 품질·운영·추적」)는 **계층의 이름이 아니었고**, 열린 이슈 58건 중 **31건(53%)** 이 거기 몰려 있었다. 그 안에는 배포 인프라 8건 · 디자인 시스템 8건 · 테스트 가드와 문서 정합 8건 · 계정 2건 · 계산 엔진 4건이 섞여 있었다. **증상은 「분류가 거칠다」가 아니라 「보이지 않는다」이다** — 31건이 한 줄로 접히면 어느 축이 밀려 있는지 판단할 수 없다.
>
> `layer:calc`와 `layer:account`는 **원래 있어야 했는데 없던 계층**이다. CII 산식은 이 제품의 심장인데 자기 자리가 없었고(`#37`~`#44`·`#63`·`#166`), 인증·세션·권한은 선대·선박·항차 어디에도 속하지 않는다(`#413`~`#415`·`#408`·`#506`).
>
> `layer:assurance`가 문서 정합과 테스트 가드를 함께 담는 것은 **이 저장소에서 둘이 같은 메커니즘**이기 때문이다 — `#583`·`#591`·`#652`·`#758`은 전부 문서↔코드 드리프트를 CI가 잡는 검사다.
...
> **프로덕션 이미지에 PDF 렌더링 의존성이 들어 있다** (`#361`). `libpango-1.0-0`·`libpangoft2-1.0-0`(텍스트 셰이핑)과 `fonts-nanum`(한국어 폰트, SIL OFL 1.1 — 임베딩·재배포 허용)이며, 이미지가 **약 195MB 커졌다**(504MB → 699MB). 빌드 시간과 레지스트리 전송량이 그만큼 늘어나므로 배포 창을 잡을 때 감안한다. 폰트를 빼면 **PDF 요청이 500으로 거부된다**(`#689` — 메시지가 CSV를 안내한다). 종전에는 한글이 tofu(□)로 조용히 나갔다. 선택 의존성이 아니다.
...
`user_session`·`user_token`·`chat_session`은 **단조 증가한다.** 만료된 행을 지우는 작업이 없으면 쌓이기만 한다 — 느려지는 것보다 **남아 있으면 안 되는 것이 남는** 쪽이 문제다(90일 지난 대화는 `PRD §16.3`이 지우라고 정한다).

```bash
# 셀 뿐, 지우지 않는다
python3 scripts/purge_expired.py --dry-run

# 지운다 (감사 로그에 EXPIRED_PURGE 한 줄이 남는다)
python3 scripts/purge_expired.py
```
...
지도의 항로선은 **공개 해상 경로망 위의 바닷길**이다. 서버의 [`searoute`](https://github.com/genthalili/searoute-py) 패키지(Apache-2.0 · `pyproject.toml` 런타임 의존성 · PyPI sdist 약 10.6 MB · 설치 실측 `searoute` 4.0 MB + `networkx` 7.2 MB + `geojson` 0.04 MB, `__pycache__` 제외)가 [Eurostat SeaRoute](https://github.com/eurostat/searoute)의 해상 경로망 데이터(EUPL-1.2 · ORNL *Global Shipping Lane Network 2000* 기반)를 번들하고, `GET /api/v1/ports/sea-route`(`API_SPEC §3.11`)가 두 점 사이의 선을 낸다. **오프라인·결정론**이다 — 외부 호출이 없고 같은 입력은 같은 선이다. 첫 호출에 경로망 그래프를 올린다 — 리눅스 파일시스템 실측(2026-09-24 · WSL2 · `/tmp`) `import 1.39s + 그래프 0.57s`, 그 뒤는 같은 실측에서 2ms다(워커 스레드에서 돌고 첫 적재는 락으로 직렬화된다). 앱 기동 직후 백그라운드로 미리 올리므로(`api/main.py` lifespan · 실패해도 기동은 계속) 첫 사용자가 그 비용을 치르지 않는다. 경로망이 두 점을 잇지 못하면(통과 제한으로 막힌 바다) 라이브러리의 직선 대체를 내보내지 않고 404다.

**표시용이다.** CII 계산 거리는 사용자 입력 또는 대권거리이고(`PRD §15.2`) 이 선의 길이는 표에 들어가지 않는다. 실제 항해 계획도 아니다 — 화면 캡션이 그 사실을 말한다.

라이선스 고지는 세 곳에 있다 — 이 절 · 저장소 루트의 [`NOTICE`](./NOTICE) · 지도 오른쪽 아래 접힌 출처 컨트롤(「© OpenStreetMap」 옆).
...
>
> `scripts/demo_up.sh`는 `uvicorn --env-file .env`로 백엔드를 띄운다. **종전에는 `DATABASE_URL` 하나만 넘겨** `.env`의 `MAIL_BACKEND=smtp`·`APP_PUBLIC_URL`·`SMTP_*`가 서버에 닿지 않았고, **회원가입은 201로 성공하는데 인증 메일이 오지 않았다** — 메일은 로그에만 찍혔다.
>
> | 기동 경로 | `.env` |
> |---|---|
> | `docker compose` | ✅ `env_file: [.env]` |
> | `uvicorn --env-file .env` | ✅ |
> | `scripts/demo_up.sh` | ✅ (`#693` 이전에는 ❌) |
>
> - **`.env`가 없어도 기동한다.** 파일이 있을 때만 인자를 붙인다 — 없는 파일을 가리키면 uvicorn이 그 자리에서 실패해, 메일 설정 하나 때문에 시연 전체가 선다.
> - **`DATABASE_URL`은 스크립트 값이 이긴다.** uvicorn은 `load_dotenv(override=False)`라 이미 있는 환경변수가 우선한다.
> - **셸이 `.env`를 `source`하지 않는다.** `MAIL_FROM=BlueLog <...>`의 `<`·`>`를 셸이 리다이렉션으로 해석해 구문 오류가 난다. 읽는 주체는 uvicorn이다.
> - `bash scripts/demo_up.sh --check`의 **5b 단계**가 현재 메일 백엔드와 인증 링크 주소를 알려 준다. 판정 근거는 **서버가 남긴 로그**다 — `.env` 파일을 읽으면 「파일에 뭐라고 적혀 있나」만 알 뿐 그 값이 서버에 닿았는지는 모른다.
...
백엔드를 띄우는 절차는 위 「배포」 절에 있다.

### 시연 계정으로 로그인한다 (`#692`)

`bash scripts/demo_up.sh`가 만드는 계정이다. **로그인 화면을 그대로 통과한다.**
...
- 데모 시드(`python -m cii_platform.db.demo_seed`)가 넣으며 **멱등**이다 — 여러 번 돌려도 늘지 않고, **사람이 고친 값을 덮지 않는다.**
- **`APP_ENV=production`에서는 만들어지지 않는다.** 고정 비밀번호를 가진 계정이 프로덕션에 있으면 그 값이 알려진 순간 누구나 들어온다.
- 도메인이 `.local`인 것은 의도다 — 실존 도메인이면 가입 확인·재설정 메일이 **남의 주소로** 나간다.
- 계정이 사라지면 `bash scripts/demo_up.sh --check`의 **4d 단계가 알려 준다.** 시연 도중에 처음 알지 않게 하는 것이 목적이다.

> **왜 이 계정이 필요한가.** 종전에는 시드가 계정을 만들지 않아 DB를 다시 만들 때마다 사람이 직접 가입해야 했다. 유일한 계정 `dev@localhost`는 **비밀번호 로그인이 의도적으로 막혀 있어**(`auth_dev.py`) 로그인 화면으로 들어갈 수 없다. 그 설계는 옳다 — 알려진 이메일로 아무나 들어오는 것을 막는다. 문제는 **그 대신 쓸 계정이 없었다**는 것이다.

로그인 화면 자체를 건너뛰려면 종전대로 브라우저 콘솔에서 아래를 실행한다. 이 길은 **로그인 화면을 통과하지 않으므로** 화면 자체를 시연할 수는 없다.
...
**실제로 가입 계정이 이렇게 사라졌다**(`#507` · `#689` 처리 중 `#691`로 재발). 백업이 없어 복구되지 않는다.

> ### 스키마 드롭만이 아니다 (`#691`)
>
> `#507`은 위 판정을 만들고 **`test_zz_roundtrip.py` 한 파일에만** 걸었다. 그 사이 계정·세션·토큰을 지우는 **다른 12개 파일**은 아무 제약 없이 개발 DB에 붙었고, 2026-08-23에 가입 계정이 **예고된 그대로** 사라졌다.
>
> **테스트가 뒷정리하는 것 자체는 정상이다.** 조건 없는 삭제는 한 곳도 없고, 지우는 대상은 테스트가 스스로 만든 계정뿐이다. 결함은 「지운다」가 아니라 **「어디에 붙느냐」**였다.
>
> 그래서 판정을 `conftest.py`의 DB fixture로 올렸다. **DB를 건드리는 모든 테스트**가 같은 규칙을 받으며, 앞으로 추가되는 테스트도 자동으로 보호된다.
...
> 🔴 **`docker compose down -v`는 쓰지 않는다 — 데이터가 통째로 사라진다.** CUBRID 데이터는
> 이미지의 데이터 경로에 붙은 명명 볼륨 `cubrid-data`에 있어 `down`(`-v` 없이)·재생성 뒤에도
> 남는다(#1867). ⚠️ **그 전에 만든 로컬 스택**은 데이터가 옛 익명 볼륨에 있어, 마운트가 바뀐
> 뒤 처음 `up`하면 `cii`·`cii_test`가 빈 채로 보인다 — 한 번 옮기거나(`docs/OPERATIONS.md`
> §9.2.1) **`cubrid createdb`부터 다시** 한다(위 1번). 컨테이너를 다시 올리면(`docker compose
> up -d` · `start`) — 이때도 컨테이너 진입점이 기동하는 것은
...
- [searoute-py](https://github.com/genthalili/searoute-py) — 해상 경로망 위의 최단 경로 (Apache-2.0) · [PyPI](https://pypi.org/project/searoute/)
- [Eurostat SeaRoute](https://github.com/eurostat/searoute) — 해상 경로망 데이터의 출처 (EUPL-1.2)
...
| 2026-09-10 | `#929` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 v2.6으로 갱신 — **`rlatnals4114`의 2026-09-10 확정 12건** 반영(§16 항목 1·2·3·5·8·13 종결 · §0.2 제약 6 신설 · §5 전 항목 〔확정〕/〔제안〕 표기와 `--shadow-overlay` · §12 Lucide · §15 `--brand-*`). 값·규격을 정한 것은 확정자이며 개발은 반영만 했다 (`AGENTS §7.3`) (#747 · #694) |
| 2026-09-10 | `#433` | 문서 구조 표의 `PRD.md` 행을 **v4.5**로 갱신 — `§12.3.1` 「필요 감축량(목표 역산)」 신설·`§12.7` 출력 표 행 추가 반영. `§12`가 「목표 달성 **확률**」까지만 정의해 `UIFLOW 2-10`이 요구하는 값의 정의가 정본에 없었다 (#433) |
| 2026-09-10 | `#934` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.7**로 갱신 — `§8` `BrandLogo` 항목 신설·`§7.2` 브랜드 판 반응형 규칙 추가 반영. `rlatnals4114`의 확정 3-2·3-3이 **정본 어디에도 없어** 같은 질문이 다시 열릴 자리로 남아 있었다 (#934) |
| 2026-09-11 | `#840` | 문서 구조 표의 `API_SPEC.md` 행을 **v1.22**로 갱신 — §6.1 응답 예시에 `simulation_id` 추가 반영. 변경 이력·구현은 v1.15부터 이 필드를 냈는데 **예시만 빠져 있었다** (#840) |
| 2026-09-11 | `#944` | 문서 구조 표의 `PRD.md` 행을 **v4.6**, `API_SPEC.md` 행을 **v1.23**, `DB_SCHEMA.md` 행을 **v1.17**로 갱신 — §8.4 재계산 정책에 선종 변경을 추가하고 `needs_recalc` 설명 두 곳을 같은 문구로 맞췄다 (#944) |
| 2026-09-11 | `#860` | 문서 구조 표의 `API_SPEC.md` 행을 **v1.24**, `DB_SCHEMA.md` 행을 **v1.18**로 갱신 — 선박 제원의 저장 경계(`NUMERIC` 정밀도 = API 입력 경계) 기재 반영 (#860) |
...
| 2026-09-11 | `#759` | 헤더 최종 수정일을 본문이 마지막으로 바뀐 날(2026-09-11)로 정정 — `2026-08-22`로 남아 있었다(`AGENTS §4.2` · `#641` 선례) (#759) |
...
| 2026-08-23 | `#493` | 문서 구조 표의 `TECH_SPEC.md` 행을 v1.8로, `DB_SCHEMA.md` 행을 v1.16으로 갱신 — 시뮬레이션 스냅샷의 선박 제원·기능③ `input_hash` 필드 목록 신설 반영 (#493) |
| 2026-08-23 | `#587` | 문서 구조 표의 `TEST_PLAN.md` 실측 수치 갱신 — 데모 제원·시드 어긋남 감지 신설분 (#587) |
| 2026-08-23 | `#151` | 문서 구조 표의 `TEST_PLAN.md` 실측 수치 갱신 — §5.1 응답 예시 동기화 가드 신설분 (#151) |
| 2026-08-23 | `#513` | 문서 구조 표의 `UIFLOW.md` 행을 v2.6으로 갱신 — `2-10 함대 감축 계획`·`2-11 데이터 점검`을 **1차 심사 범위 밖**으로 판정하고 `PRD §5.2` 제외 범위에 등재한 것을 반영. `PRD`는 각주 신설이라 버전을 올리지 않았다(`AGENTS §4.3`) (#513) |
| 2026-08-23 | `#689` | 문서 구조 표의 `TEST_PLAN` 수치를 갱신 — **105파일·1378함수·1682수집 → 106파일·1395함수·1699수집**. 폰트 부재 시 PDF 거부 테스트 12종 신설분(`test_reports.py` +4 · `test_health.py` +5 · `test_demo_up_script.py` +3)이다. ⚠️ **기재가 이미 1파일·5함수 낮아 있었다** — `TEST_PLAN §14`는 106파일·1383함수로 갱신돼 있었는데 이 표만 뒤처졌다. `test_doc_version_sync`는 **버전만 보고 수치는 보지 않으므로**(`#641`이 같은 드리프트를 겪었다) 함께 맞췄다. 「배포」 절의 폰트 문구도 정정했다 — 폰트를 빼면 tofu가 나가는 것이 아니라 **PDF 요청이 500으로 거부된다** (#689) |
| 2026-08-23 | `#691` | **「전체 테스트는 대상 DB를 파괴한다」 절을 확장하고 「로컬에서 테스트를 돌리는 법」을 신설.** 종전 서술은 위험을 **스키마 드롭 하나**로만 적었는데, 실제로 계정을 지운 것은 그 파일이 아니라 **가드가 안 걸린 12개 파일**이었다. 「개발 DB를 가리키면 여섯 개가 skip된다」는 문장도 더 이상 사실이 아니다 — 이제 **DB를 쓰는 테스트가 실패한다**(skip은 조용해서 돌았다고 착각할 여지를 남긴다). `cii_test`를 한 번 만들면 끝이라는 것과, 그러면 **롤백 6건이 로컬에서도 돈다**는 것을 함께 적었다. 문서 구조 표 수치도 갱신 — `#689` 머지분 위에서 **106파일·1402함수·1708수집** (#691) |
| 2026-08-23 | `#692` | **「시연 계정으로 로그인한다」 절 신설.** 종전에는 로그인 방법으로 **브라우저 콘솔에 `dev-login`을 붙여넣는 길만** 적혀 있었다 — 그 길은 로그인 화면을 통과하지 않아 화면 자체를 시연할 수 없고, 개발자가 아닌 팀원은 쓰기 어렵다. 계정·비밀번호와 함께 **왜 `dev@localhost`로 대신할 수 없는지**(비밀번호 로그인이 의도적으로 막혀 있고 그 설계가 옳다) · **프로덕션에서는 만들어지지 않는다**는 것 · 도메인이 `.local`인 이유(실존 도메인이면 메일이 남의 주소로 나간다)를 함께 적었다. 문서 구조 표 수치도 갱신 — `#689`·`#691` 머지분 위에서 **107파일·1418함수·1727수집** (#692) |
| 2026-08-23 | `#693` | **「배포」 절에 「시연 경로도 `.env`를 읽는다」 각주 신설.** 세 기동 경로 중 compose와 `uvicorn --env-file`만 `.env`를 읽고 **`scripts/demo_up.sh`만 빠져 있었다** — 그래서 8/17부터 `.env`에 있던 `MAIL_BACKEND=smtp`가 서버에 닿지 않아 **회원가입은 201인데 인증 메일이 오지 않았다.** 세 가지를 함께 적었다: `.env`가 없어도 기동한다는 것(gitignore 대상이라 새 클론에는 없다) · `DATABASE_URL`은 스크립트 값이 이긴다는 것(`load_dotenv(override=False)`) · **셸이 `source`하지 않는 이유**(`MAIL_FROM=BlueLog <...>`의 `<`·`>`가 리다이렉션으로 해석돼 구문 오류가 난다). 문서 구조 표 수치도 갱신 — `#689`·`#691`·`#692` 머지분 위에서 **107파일·1426함수·1732수집** (#693) |
| 2026-08-24 | `#712` | 문서 구조 표의 `UIFLOW.md` 행을 v2.7로 갱신 — `§2.2.1 사이드바 순서`를 신설하고 순서를 계층 기준에서 **작업 흐름 기준**으로 바꾼 것을 반영 (#712) |
| 2026-09-11 | `#830` | 문서 구조 표의 `TEST_PLAN` 행에서 수치(107파일·1426함수·1732수집 · 08-23)를 빼고 **`TEST_PLAN §14.2` 합계를 가리키게** 했다 — 이 변경 이력에서 그 수치를 손으로 고친 행만 **16개**이고(`#445`·`#641`·`#631`·`#692`·`#693` 등) 그때마다 다시 낡았다. 수치를 다시 적으면 `tests/test_testplan_sync.py`가 실패한다 (#830) |
...
| 2026-09-12 | `#989` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.13**으로 갱신 — §3.11 「요청 캐시」 절 신설(`AGENTS §4.3`). 선대 요약이 요청 하나에서 같은 규제 파라미터를 다시 읽던 것을 없앤 건이다 (#989) |
| 2026-09-12 | `#768` | 문서 구조 표의 `API_SPEC.md` 행을 **v1.28**로 갱신 — §3.10 「항만명 좌표 조회」 절 신설(`AGENTS §4.3`). 목록 밖 항만의 좌표를 이름으로 찾는 경로다 (#768) |
| 2026-09-12 | `#768` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.20**으로 갱신 — §2.20 `port_geocode`(항만명 좌표 조회 캐시) 신설 (#768) |
| 2026-09-12 | `#764` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.21**로 갱신 — §2.21 `vessel_position_snapshot`(선박 위치 이력) 신설. 위치가 덮어쓰는 한 칸이라 항적이 남지 않던 것을 연다 (#764) |
| 2026-09-12 | `#764` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.15**로 갱신 — §3.15 「위치 스냅샷·AIS 수집」 절 신설(`AGENTS §4.3`) (#764) |
| 2026-09-12 | `#769` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.14**로 갱신 — §3.14 「연도별 CII 이력의 연료축」 절 신설(`AGENTS §4.3`). `PRD §21` 「통계 분석」 세 축 중 연료축을 연 건이다 (#769) |
| 2026-09-12 | `#775` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.22**로 갱신 — §4.2 파티셔닝·§9.2 다중 회사의 **착수 조건**을 숫자로 신설 (#775) |
| 2026-09-12 | `#763` | 문서 구조 표의 `UIFLOW.md` 행을 **v2.8**로 갱신 — `2-4 대시보드`에 선박 위치 지도 신설(PMTiles 로컬 자산 · 개략도 폴백) (#763) |
| 2026-09-12 | `#763` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.9**로 갱신 — §9.5 「지도 위 등급 표시」 신설 (#763) |
| 2026-09-12 | `#763` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.16**으로 갱신 — §3.16 「선대 요약의 항로 좌표」 절 신설 (#763) |
| 2026-09-13 | `#120` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.17**로 갱신 — §3.17 「챗봇 가드·대화 보존」 절 신설(`AGENTS §4.3`) (#120) |
| 2026-09-13 | `#121` | 문서 구조 표의 `API_SPEC.md` 행을 **v1.29**, `TEST_PLAN.md` 행을 **v1.18**로 갱신 — `API_SPEC §15` Chat API 신설과 `TEST_PLAN §3.18` 신설 반영 (#121) |
| 2026-09-13 | `#121` | 문서 구조 표의 `UIFLOW.md` 행을 **v2.9**로 갱신 — `2-7 AI 어시스턴트` 주요 UI 신설 반영 (#121) |
| 2026-09-13 | `#123` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.19**로 갱신 — §3.19 「챗봇 용어 풀이 ↔ 정본」 절 신설 (#123) |
| 2026-09-13 | `#834` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.20**으로 갱신 — §3.20 「두 규정 표의 선종 집합」 절 신설 (#834) |
| 2026-09-13 | `#827` | 「만료 행 정리」 절 신설 — `scripts/purge_expired.py`(`#827` ⑶). `user_session`·`user_token`·`chat_session`이 단조 증가하고 있었다 (#827) |
| 2026-09-13 | `#827` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.21**로 갱신 — §3.21 「만료 행 정리 스크립트」 절 신설 (#827) |
| 2026-09-13 | `#773` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.22**로 갱신 — §3.22 「연료 CF 표 ↔ 시드」 절 신설 (#773) |
| 2026-09-13 | `#363` | 문서 구조 표의 `PRD.md` 행을 **v4.9**, `DB_SCHEMA.md` 행을 **v1.23**으로 갱신 — `PRD §12.2.1` 「실적 보정계수」 신설 · `annual_simulation_run.apply_feedback_factor` 컬럼(마이그레이션 042) (#363) |
| 2026-09-13 | `#513` | 문서 구조 표의 `PRD.md` 행을 **v4.10**, `API_SPEC.md` 행을 **v1.30**, `UIFLOW.md` 행을 **v2.10**으로 갱신 — `PRD §17.4` 데이터 점검 판정 · `API_SPEC §2.16` 신설 · `UIFLOW 2-11` 범위 안으로 (#513) |
...
| 2026-09-18 | `#1080` · `#1081` · `#1104` · `#1171` · `#1176` | **문서 동기화 부채 일괄 정정 (5이슈).** ⑴ **핵심 기능·MVP 표를 `PRD §5.1`(09-13 개정)에 맞춤** — 함대 감축 계획·데이터 점검·실적 보정계수·AI 어시스턴트(실험) 행 추가, 제외 목록의 「지도 기반 항로 렌더링」 삭제(`PRD §5.2` 범위 안 개정 · #763). ⑵ 문서 구조 표 5행 갱신 — `PRD` v4.14 · `TECH_SPEC` v1.10 · `DB_SCHEMA` v1.28(**20개 → 25개 테이블**, chat 2개 등재 #1080) · `TEST_PLAN` v1.26 · `UIFLOW` v2.13. ⑶ 헤더 최종 수정일 09-11 → 09-17(#1081 ⑼ — 본문 커밋일 09-16보다 5일 낡아 있었다) (#1081 ⑴⑵⑶⑼) |
| 2026-09-18 | `#1170` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 v2.12로 갱신 — `§14` 「비활성 컨트롤은 「왜」를 함께 낸다」 확정(`disabled` 유지 + `aria-describedby`로 사유를 잇는다) · `§16` 항목 20(비활성 컨트롤을 초점에 남길지) 신설 · 「등급 없음」 배지 테두리를 등급 축에서 중립 경계로 정정 (#1170) |
...
| 2026-09-20 | `#1317` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.32**로 갱신 — §2.1 `vessel.call_sign VARCHAR(7)` 추가(마이그레이션 058 · `#1197` A단계) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-20 | `#1319` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.33**으로 갱신 — §2.2 `voyage.planned_distance_source VARCHAR(30)` 추가(마이그레이션 059 · `#1052` ⓷ 후속) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-20 | `#1362` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.34**로 갱신 — head `059` 대조(절 일곱이 `051` 이전에 멈춰 있던 것을 맞춤) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-21 | `#1519` | 문서 구조 표의 `UIFLOW.md` 행을 **v2.15**로 갱신 — `2-6 설정`에 「규제 기준값」 절 신설 · `SCR-006`(규제 파라미터 조회)을 post-MVP에서 범위 안으로 뒤집었다(`#1239` 결정 A — 유예 조건이던 `#673`이 09-18에 닫힘). `PRD.md`는 각주·값 정정(§6.2 `SCR-006` 「조회·수정」→「조회·개정 적재」 · §20 O-14 현행 각주)이라 버전을 올리지 않았다(`AGENTS §4.3`) (#1239 · #1516) |
| 2026-09-22 | `#1590` | 문서 구조 표의 `TECH_SPEC.md` 행을 **v1.15**로 갱신 — §1.2.1 「응답 직렬화의 절사」 절 신설(`#1349`) |
| 2026-09-22 | `#1606` | 화면 표의 데이터 점검 설명 「실적 미입력」 → 「실적 확정 전」(`#1532` — 정본 개명 미러) |
| 2026-09-22 | `#1676` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.18**로 갱신 — `§2.5 (b)` 🔒 위험도 단계별 색 · 아이콘 색 확정 |
| 2026-09-22 | `#1677` | 문서 구조 표의 `UIFLOW.md` 행을 **v2.16**으로 갱신 — 09-12~13 신설 화면의 개발 임시안 확정(`#1052`) |
| 2026-09-22 | `#1680` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.19**로 갱신 — `§14` 채움 Primary의 경계를 테두리가 맡는다(`#1296`) |
...
| 2026-09-23 | `#1731` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.23**으로 갱신 — `§8.6` 위험도 pill의 조건(`#1728`) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-23 | `#1734` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 v2.24로 갱신 — §14 문서 언어 경계(루트는 `ko` · 영문으로 바뀌는 자리에만 `lang="en"`) 신설 반영 (#1652) |
| 2026-09-23 | `#1758` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.25**로 갱신 — §7.1 「분할하지 않는 화면이 있다」 신설 · §8.6 감축 계획 행의 보조를 하나로(`#1756`) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-23 | `#1764` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.26**으로 갱신 — §3에 `page`(28) 행 신설 · `body` 14 → 15(`#1763`) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-23 | `#1773` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.27**로(§8 탭 신설) · `UIFLOW.md` 행을 **v2.17**로(`2-8` 선박 상세 「화면 구성 — 탭 넷」) 갱신(`#1772`) (`#1779` — 이 PR이 README 표를 고치면서 행을 싣지 않아 사후에 채운다) |
| 2026-09-23 | `#1743` | 문서 구조 표의 `API_SPEC.md` 행을 v1.43으로 갱신 — §1.2.1~§1.2.10 인증 요청 표 10종 신설(요청 필드 대조 가드의 `NO_TABLE_YET`을 비웠다) 반영 (#1602) |
| 2026-09-23 | `#1713` | 문서 구조 표의 `TEST_PLAN.md` 행을 **v1.28**로 — `§1.6` 「픽스처와 테스트 격리」 재작성 (#1583) |
| 2026-09-23 | `#1802` | **변경 이력에서 빠진 일곱 행을 채우고 재발 가드를 둔다** (`#1779`). 문서 구조 표를 고친 PR 일곱(`#1317` · `#1319` · `#1362` · `#1731` · `#1758` · `#1764` · `#1773`)이 이 표에 행을 싣지 않았다. `tests/test_doc_version_sync.py`는 **표의 최종 상태**만 봐서 잡지 못했다 — 표의 현재 판본마다 이 변경 이력에 그 문서·판본을 적은 행이 있는지 보는 검사를 더했다 |
...
| 2026-09-25 | `#1892` | **로컬 DB 경고를 새 마운트에 맞춤** — `cubrid-data`가 이미지의 데이터 경로에 붙어(#1867) `down`(`-v` 없이)은 더 이상 데이터를 지우지 않는다. 금지 대상은 `down -v`이고, 그 전에 만든 로컬 스택은 처음 한 번 옮기거나(`OPERATIONS §9.2.1`) 다시 만든다 (#1867) |
| 2026-09-26 | `#1920` | 문서 구조 표의 `DB_SCHEMA.md` 행을 **v1.37**로 · **26개 테이블** — `DB_SCHEMA §2.25` `port_call_record`(공적 재항 기록 원본 · 마이그레이션 063) (#1197) |
| 2026-09-26 | `#1921` | 문서 구조 표의 `PRD.md` 행을 **v4.17**로 — `PRD §17.4.4` 「공적 기록과 다름」 신설(데이터 점검 다섯째 심각도) (#1197) |
| 2026-09-26 | `#1929` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.29**로 — `§9.5` 마커 팝오버 구현 실측 반영(`#1831`). **겹친 배를 묶지 않는다**(같은 항구 3척이 중심 간격 2px로 겹치고 타일이 `z6`까지라 확대해도 풀리지 않으나, 좌측 패널 행의 「지도에서 보기」가 **선박 id로** 카드를 열어 겹친 배도 전부 열린다) · 닫는 길에 **닫기 버튼 · 지도 확대** · v2.28이 적은 **[실시간 CII] 버튼 제거**(그 경로가 항차 id를 요구한다) |
| 2026-09-26 | `#1939` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.30**으로 — `§7.2` 좁은 창에서 **사이드바를 축소(64) 상태로**(기준 `1100px`) · **상단바 유틸리티는 본문 열보다 넓어지지 않는다**(`#1885`). 「접힘 동작 자체는 MVP 범위 밖」으로 둔 동안 400px에서 `scrollWidth 547` · 본문 열 **88px**였다. ⚠️ 축소 상태에서 **라벨은 지우지 말고 시각적으로만 감춘다**(아이콘이 `aria-hidden`이라 항목에 이름이 남지 않는다) |
| 2026-09-26 | `#1948` | 문서 구조 표의 `API_SPEC.md` 행을 **v1.47**(§3.12 공적 기록으로 채우기) · `DB_SCHEMA.md` 행을 **v1.38**(실제 시각 출처 열 넷 064) · `TEST_PLAN.md` 행을 **v1.32**(§3.29)로 (#1923) |
