...
> 종전 `layer:backlog`(「현 범위 밖」)를 폐지했다 (#777). 일곱 값 중 여섯은 계층을, 그 하나만 범위를 가리켜 **한 축이 두 질문에 답하고 있었다.** 그 결과 그 라벨만 달린 이슈 18건이 **어느 계층 일인지를 아예 갖고 있지 않았다** — `scripts/issue_matrix.py`의 `layer_of()`가 목록 순서로 첫 라벨을 쓰는데 `layer:backlog`가 맨 뒤였기 때문이다.
