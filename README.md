...
| 2026-09-25 | `#___` | **로컬 DB 경고를 새 마운트에 맞춤** — `cubrid-data`가 이미지의 데이터 경로에 붙어(#1867) `down`(`-v` 없이)은 더 이상 데이터를 지우지 않는다. 금지 대상은 `down -v`이고, 그 전에 만든 로컬 스택은 처음 한 번 옮기거나(`OPERATIONS §9.2.1`) 다시 만든다 (#1867) |
