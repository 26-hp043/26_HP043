...
| 2026-09-26 | `#___` | 문서 구조 표의 `DESIGN_SYSTEM.md` 행을 **v2.30**으로 — `§7.2` 좁은 창에서 **사이드바를 축소(64) 상태로**(기준 `1100px`) · **상단바 유틸리티는 본문 열보다 넓어지지 않는다**(`#1885`). 「접힘 동작 자체는 MVP 범위 밖」으로 둔 동안 400px에서 `scrollWidth 547` · 본문 열 **88px**였다. ⚠️ 축소 상태에서 **라벨은 지우지 말고 시각적으로만 감춘다**(아이콘이 `aria-hidden`이라 항목에 이름이 남지 않는다) |
