...
| [`API_SPEC.md`](./API_SPEC.md) | REST API 명세서 (v1.8, Oracle Review + 외부 리뷰 반영 + 인증(#272) + PATCH null 의미론·전환 policy 규칙 #310·#312 + 위치 갱신 #369 + `as_of` 공통 계약 #368 + 연도별 CII 이력 #355 + not under way CRUD #370) — 36개 엔드포인트, 수치 직렬화 정책, field_label 오류 체계, CSV escape 보안 | ✅ 완료 |
...
| [`TEST_PLAN.md`](./TEST_PLAN.md) | 테스트 계획서 (v1.7, Oracle Review + 외부 리뷰 반영 + Layer 1 픽스처 정본값 규칙 #166 + §1.3 케이스 스키마 #46 + §4.7 인증 API #279 + §4.8 not under way CRUD #370) — 63개 파일, Fixture 1~4, 정본값 생성기 계약(§1.7), 이중 capacity 검증, Layer 변환/감사 로그/소프트 삭제/CSV injection 테스트, 유효숫자 기반 Monte Carlo 비교 | ✅ 완료 |
