...
| 2026-09-19 | `#1301` | 문서 구조 표의 `PRD.md`(v4.16) · `API_SPEC.md`(v1.38) · `DB_SCHEMA.md`(v1.31) · `UIFLOW.md`(v2.14) 행을 갱신 — 관리자(`ADMIN`)를 더해 역할 3종으로, 계정 관리 2경로를 사무직에서 관리자로 뗐다. 배포 절의 최초 권한 변수를 `INITIAL_OFFICE_EMAILS` → **`INITIAL_ADMIN_EMAILS`**로 고쳤다(옛 이름이 남아 있으면 기동 거부) — 과거 변경 이력 행은 기록이라 그대로 둔다 (#1301) |
