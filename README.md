...
| 2026-09-20 | `#1340` | **「지도 자산」 절 정정 — 사실 정정이며 배포 방침 변경이 아니다.** `#985`(2026-09-18)가 항만 43곳의 깊은 층(z7–z10)을 걷어내고 전 세계 z0–z5(약 26 MB)로 좁혀 **저장소에 커밋했는데**, 이 절이 여전히 「`.gitignore`가 막아 두었다 · 약 92 MB · z0–z6+z7–z10」이라는 종전 구성을 안내하고 있었다. `git ls-files`로 커밋 여부를 다시 확인하고, 절을 「받는다」에서 「평소엔 손댈 일 없다」로 고쳤다 — 정확한 수치는 `scripts/fetch_basemap.sh` 머리말을, 확대 상한은 `basemap.ts`의 `MAX_ZOOM`을 가리키게 해 값이 다시 벌어지지 않게 했다. `AGENTS §4.3` 「오기·값 정정」이라 버전은 올리지 않는다 (#1340) |
