...
## 왜 `§5.1`만 보는가 — `§4.1`·`§6.1`로 넓히지 않았다 (#753)

- **`§4.1`은 이미 본다** — `test_voyage_cii_service.py`가 같은 입력으로 계산해 응답
  문자열을 계약 예시와 대조한다. 여기로 옮기면 같은 대조가 두 곳에 생긴다
- **`§6.1`은 값이 문서 밖의 데이터에 달려 있다** — Monte Carlo 입력이 실행 시점
  **선박의 항차 전체를 뜬 스냅샷**이라, 예시 값을 맞추려면 문서가 데모 시드의 항차
  구성까지 고정해야 한다. 시드가 바뀔 때마다 정본을 고치게 되고, 그것은 `#559`가
  유지비를 이유로 기각한 값 대조의 성질 그대로다. `§6.1`의 **필드 집합**은
  `test_response_contract_db.py`가 보고, 값의 **표기**(4 유효숫자 문자열)는
  `#757`의 검사가 본다
"""
