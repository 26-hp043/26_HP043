...
#: ``| 2026-09-19 | `#1284` | …`` — 변경 이력 한 행. 날짜로 시작하는 표 행만 본다.
#:
#: 산문에 적힌 ``#___``(`AGENTS §7`의 PR 본문 서식 예시)는 대상이 아니다 —
#: 규칙을 적으려면 빈칸 모양을 그대로 보여 줘야 한다. `AGENTS §4.7` 금지 예시를
#: 코드펜스로 비켜 간 것과 같은 판단이다.
_CHANGELOG_ROW = re.compile(r"^\|\s*[0-9]{4}-[0-9]{2}-[0-9]{2}\s*\|")
...
def test_변경_이력에_PR_번호가_비어_있지_않다() -> None:
    ...
    for name in TARGETS.values():
        ...
        for index, line in enumerate(_text(path).splitlines(), start=1):
            if _CHANGELOG_ROW.match(line) and "#___" in line:
