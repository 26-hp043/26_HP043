...
1. **새 계정은 현장직**이고, `INITIAL_OFFICE_EMAILS`에 든 이메일만 가입·로그인에서 사무직이 된다
   — 새 DB에서 사무직 0명이 되지 않게(`auth/role_bootstrap.py`)
...
async def _demote_other_office_users(keep: list[str]) -> list[UUID]:
    """``keep`` 밖의 사무직을 전부 현장직으로 내리고 **되돌릴 id**를 돌려준다.

    044가 기존 계정을 전부 사무직으로 채웠으므로(데모 계정·dev 스텁 포함) 「마지막 사무직」
    상황은 만들어서 본다. 끝나면 :func:`_restore_office` 로 되돌린다.
    """
    from cii_platform.db.session import get_sessionmaker
...
def test_parse_initial_office_emails_normalises_like_signup():
    """쉼표 목록 · 공백 · 대소문자 — 가입이 소문자로 정규화하므로 목록도 같은 규칙이다."""
    parsed = role_bootstrap.parse_initial_office_emails(" Office@BlueLog.kr, b@x.io ,, ")
    assert parsed == frozenset({"office@bluelog.kr", "b@x.io"})
    assert role_bootstrap.parse_initial_office_emails(None) == frozenset()
    assert role_bootstrap.is_initial_office(
        "OFFICE@bluelog.kr", {"INITIAL_OFFICE_EMAILS": "office@bluelog.kr"}
    )
    assert not role_bootstrap.is_initial_office(
        "x@bluelog.kr", {"INITIAL_OFFICE_EMAILS": "office@bluelog.kr"}
    )


def test_production_refuses_to_start_without_an_initial_office(monkeypatch: pytest.MonkeyPatch):
    """조용히 사무직 0명으로 뜨면 첫 사용자가 리포트를 열 때에야 드러난다 — 기동 시점에 끊는다."""
    from cii_platform import config

    monkeypatch.setattr(config, "_ENV", "production")
    with pytest.raises(RuntimeError, match="INITIAL_OFFICE_EMAILS"):
        role_bootstrap.validate_initial_office({})
    role_bootstrap.validate_initial_office({"INITIAL_OFFICE_EMAILS": "office@bluelog.kr"})

    monkeypatch.setattr(config, "_ENV", "development")
    role_bootstrap.validate_initial_office({})


def test_real_app_lifespan_runs_the_initial_office_check(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("INITIAL_OFFICE_EMAILS", raising=False)
    with pytest.raises(RuntimeError, match="INITIAL_OFFICE_EMAILS"), TestClient(app):
        pass  # pragma: no cover - 진입 자체가 실패한다


async def test_new_signup_is_field_and_initial_office_email_is_office(client, monkeypatch):
    """새 계정은 현장직, 목록에 든 이메일만 사무직 — 응답과 DB 둘 다 본다."""
    field_email = "role-new@example.com"
    office_email = "role-initial@example.com"
    try:
        assert _signup(client, field_email)["role"] == "FIELD"
        assert await _role_in_db(field_email) == "FIELD"

        monkeypatch.setenv("INITIAL_OFFICE_EMAILS", " Role-Initial@example.com ")
        assert _signup(client, office_email)["role"] == "OFFICE"
        assert await _role_in_db(office_email) == "OFFICE"
    finally:
        await _cleanup([field_email, office_email])


async def test_login_promotes_an_initial_office_email_and_audits_once(client, monkeypatch):
    """044 이전 가입자·화면에서 강등된 계정도 목록에 있으면 로그인에서 사무직이 된다."""
    email = "role-promote@example.com"
    try:
        user = _signup(client, email)
        assert user["role"] == "FIELD"

        monkeypatch.setenv("INITIAL_OFFICE_EMAILS", email)
        assert _login(client, email)["role"] == "OFFICE"
        assert await _role_in_db(email) == "OFFICE"
        ...
        # 이미 사무직이면 다시 쓰지 않는다 — 「OFFICE → OFFICE」가 쌓이면 실제 변경을 못 찾는다
        _login(client, email)
...
def _office_only_in_source() -> set[str]:
    """``routes/*.py``를 **소스에서** 읽어 ``require_office``가 걸린 라우트를 모은다.
    ...
    for path in sorted(routes_dir.glob("*.py")):
        ...
        for node in ast.walk(tree):
            if not isinstance(node, ast.FunctionDef | ast.AsyncFunctionDef):
                continue
            if "require_office" not in ast.unparse(node.args):
                continue
...
async def test_user_list_and_role_update_share_the_user_contract(client, monkeypatch):
    """사무직 — 목록·역할 변경 응답은 `/auth/me`와 같은 사용자 계약이다 (`#753` 계약 표)."""
    ...
    try:
        ...
        monkeypatch.delenv("INITIAL_OFFICE_EMAILS")
        ...
        # 모르는 역할은 422, 없는 계정은 404 — DB 제약에 닿아 500이 되지 않는다
        bad = client.patch(
            f"{API_V1_PREFIX}/auth/users/{target['id']}/role",
            json={"role": "ADMIN"},
            headers=_csrf(client),
...
async def test_last_office_cannot_be_demoted_or_deleted(client, monkeypatch):
    """사무직이 하나면 그 하나는 탈퇴도 강등도 못 한다. 둘이면 서로 바꿀 수 있다."""
    solo = "role-last@example.com"
    second = "role-second@example.com"
    demoted: list[UUID] = []
    try:
        monkeypatch.setenv("INITIAL_OFFICE_EMAILS", f"{solo},{second}")
        me = _signup(client, solo)
        demoted = await _demote_other_office_users(keep=[solo])
        ...
        with TestClient(app, base_url=_BASE) as other:
            ...
            assert changes[-1]["details_json"] == {"role_before": "OFFICE", "role_after": "FIELD"}
            assert changes[-1]["user_id"] == other_me["id"]

            # 이제 solo는 현장직 — 사무직 전용 경로가 막힌다(세션 캐시가 아니라 DB 역할이다)
