...
NOT-COVERED: IT-AUDIT-002 — 기능이 없으므로 케이스도 `#444`로 옮겼다 (`TEST_PLAN §14.5`).

종전에는 이 자리에 **ID를 적지 못했다.** 인용만으로도 커버리지 게이트가 「덮였다」로
읽어 CI가 실패했고, 우회가 「그 ID를 아예 적지 않는 것」이었다 — 그러면 왜 그 케이스를
다루지 않는지 설명할 자리가 사라진다. `#498`이 `NOT-COVERED:` 표시를 두어 열었다.
