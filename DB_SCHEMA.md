...
| `voyage_id` | UUID | NULL, FK → voyage(id) **ON DELETE RESTRICT** [C-3, C-5 정정] | 관련 항차 (있으면). 계산 이력 보존을 위해 항차 물리 삭제를 차단 |
...
| `voyage(id)` | `calculation_run.voyage_id` | **RESTRICT** [C-5 정정] | 계산 이력 보존. calculation_run은 immutable(§7.3)이라 SET NULL(자식 UPDATE)이 트리거로 차단됨 → RESTRICT |
