...
> **[#1626] 「항차당 채택 행 하나」는 서비스 불변식이다 — DB 제약이 아니다.** `is_adopted = 1`인 행은 한 `voyage_id`에 하나만 남는다. 지키는 것은 `services/scenario_adopt`가 **항차 행을 `SELECT … FOR UPDATE`로 잠근 안에서** 이전 채택을 내리고 자기 행을 올리는 순서다(`TECH_SPEC §16.3`). 잠금 없이 두 채택이 교차하면 해제 UPDATE가 둘 다 0건이라 **두 행이 남는다** — `#1796` ⑺ 실측(잠금 없이 2건 3/3 · 항차 행 잠금 뒤 1건 3/3). 검사는 `tests/test_voyage_row_lock_db.py`.
>
> DB 제약으로 세우지 않는 이유 — CUBRID에는 부분 유니크 인덱스가 없어(`#1058`) `061`(`#1631`)의 **활성 키 열** 형태(채택이면 `voyage_id`의 사본, 아니면 NULL인 열 + 유니크 인덱스 + 채움 트리거)로 옮겨야 하고, `F-8` 결정(2026-09-22)이 새 DB 객체 없이 **잠금으로** 닫는다고 정했다. 잠금이 있으면 그 제약은 보강이지 대체가 아니다(`#1796` 코멘트). 필요해지면 같은 형태로 옮길 수 있다.
...
| `planned_*` / `actual_*` | **두 벌을 모두 남긴다.** 계산은 `ACTUAL` 행에서 「실적이 있으면 실적, 없으면 계획」(`PRD §8.3`)으로 고르고 `PLAN` 행은 **계획값만** 쓰는데(`TECH_SPEC §11.4` · `#1337`), 사본에 한 벌만 두면 **어느 쪽이 쓰였는지**를 나중에 알 수 없다. 진행 중(`IN_PROGRESS`) 항차의 `actual_*`는 그래서 사본에는 남되 계산에는 들어가지 않는다 |
...
> 형태**(`status_at_snapshot` · 실제 쓰인 값 한 벌)다. 투영 규칙은 계산과 **행 종류별로**
> 같다 — `ACTUAL` 행은 **실적이 있으면 실적, 없으면 계획**, `PLAN` 행은 **계획값만**
> (`API_SPEC §6.3` `[#1337]` 각주 — 종전 「실적이 있으면 실적」 한 규칙은 진행 항차의
> 실적 일부를 계산에 쓰지 않았는데도 응답에 실었다). 응답 쪽 수치는 JSON **숫자**이고
> 저장 쪽은 **문자열**이라는 점도 다르다.
...
> **[#1629] 「같은 선박의 구간은 겹치지 않는다」는 서비스 불변식이다 — DB 제약이 아니다.** 지키는 것은 `services/not_underway._assert_no_overlap`이 겹침 조회 **전에** 선박 행을 `SELECT … FOR UPDATE`로 잡는 것이다(`db/repositories/vessel.lock_row` · `TECH_SPEC §16.3`). 생성·수정·CSV 가져오기가 전부 이 함수를 지나므로 잠금은 한 곳이다. 두 번째 요청은 첫 요청의 커밋까지 기다렸다가 **커밋된 구간을 보고** 기존 409로 떨어진다. 잠금 없이 두 연결을 교차시키면 겹친 구간이 2건 남는 것을 `#1796`이 실측했다(3/3). 항차 쪽 같은 형태의 각주는 `§2.4`(`#1626`)에 있다.
...
> ⚠️ **[#1346] `SPEED` 행은 Monte Carlo가 표본추출하지 않는다 — 정의와 기록으로만 쓰인다.** `PRD §12.4.1` 각주가 근거다: `CII = M / (W · Dt)`에 속도가 없어 독립으로 뽑아도 결과가 바뀌지 않고, cubic model로 이으면 **독립 표본추출된 연료와 같은 변동을 두 번 센다**. 그래도 **행을 지우지 않는다** — `parameters_used.simulation_profile`·`parameter_hash`(`TECH_SPEC §5.2.1.1`)에 계속 실어, 속도를 표본추출하게 되는 날 **옛 실행과 해시가 겹쳐 「재현됐다」가 거짓**이 되는 것을 막는다. `bound_type`·`floor_value`가 속도 때문에 존재한다는 위 문단도 그대로 유효하다. ⚠️ **이 행은 `PRD §12.6` 민감도의 ±1kn이 아니다** — 그쪽은 정본이 직접 못박았고 응답 키(`speed_minus_1kn`)도 고정이다.
...
9. **`IS`의 오른쪽에 `0`/`1`이 올 수 없다** (`#1316`). `IS`는 `NULL`·`TRUE`·`FALSE`만 받는다.
   ...
   호출을 막는다.
10. **같은 이름의 트리거를 두 번 만드는 것을 막지 않는다 — 그리고 중복이 생기면 이름으로는
    어느 쪽도 지울 수 없다** (`#1373`). 있는 이름으로 `CREATE TRIGGER`를 다시 치면 성공하고
    `db_trigger`가 한 줄 는다(160 → 161). 그 뒤 `DROP TRIGGER`는 「없다」로 답한다.
    ...
    `dba.` 접두를 붙이거나 따옴표로 감싸도 같다. 한 번 중복이 생기면 그 DB는 **이름
    기반으로는 영구히 회복되지 않는다** — 다시 만드는 수밖에 없다(`README` 「테스트 DB
    복구」). 마이그레이션이 `CREATE TRIGGER`를 무조건 치면 `upgrade`가 두 번 도는 순간(끊긴
    왕복의 재시도 · 부분 실패 뒤 재실행) 중복이 생기고, 그 뒤의 `downgrade`가 `-503`에서
    그 리비전에 갇힌다 — 2026-09-20 CI에서 PR 10건 중 3건이 이 경로로 빨갰다.

    🔒 **트리거를 만들고 지우는 마이그레이션은 전부 `db/trigger_ddl.py`를 지난다** —
    `create_trigger`는 이미 있으면 만들지 않고, `drop_trigger`는 없으면 지우지 않는다
    (카탈로그에는 있는데 `-503`이 오는 중복 상태는 경고를 남기고 넘어간다 — 그 자리에서
    멈추면 롤백 전체가 갇힌다). 그 관용이 중복을 「없음」으로 읽을 수 있으므로
    `tests/test_zz_roundtrip.py`가 왕복을 돌리기 **전에** 둘을 센다 — 같은 이름이 둘 이상인
    트리거가 0건인가, head DB의 트리거 이름 집합이 마이그레이션이 만든다고 적은 집합과
    같은가. `048`이 `#1386`에서 응급으로 넣었던 같은 판단을 공용으로 올린 것이다(결정요청
    v6 `D-20` · 2026-09-21).

    **관용의 방향은 하나다 — `downgrade`는 넘어가고 `upgrade`의 교체는 멈춘다.** `050`·`051`·
    `057`은 조건을 바꾸려고 지우고 다시 만드는데, 중복 상태에서 지우지 못한 것을 넘어가면
    **옛 조건이 남은 채 리비전만 올라간다**(`051`이면 NULL 구멍이, `057`이면 `ADMIN` 거부가
    그대로다). 그래서 `upgrade`는 `replace_trigger`로 지운 뒤 카탈로그를 다시 물어 이름이
    남아 있으면 `README` 「테스트 DB 복구」를 가리키며 멈춘다. `downgrade`는 같은 자리에서
    멈추면 롤백 전체가 그 리비전에 갇히므로 `drop_trigger`의 관용을 그대로 둔다 — 옛 조건이
    남는 것보다 갇히는 것이 나쁘다.
...
| ✅ 선행 (충족) | **`#672`·`#1301` 어드민 계정·권한 재판정** — 테넌시는 권한 체계 **위에** 선다. `#672`(역할 2종 · 09-15)·`#1301`(관리자 분리 · 09-19)로 닫혔다 |
| 실측 (2026-09-23) | 회사 1곳 · `app_user`는 **사무직·현장직·관리자 3종**(`#672`·`#1301` · §2.15 `role` · `#1663` 정정 — 2026-09-15 실측 당시에는 2종이었다). 회사 소속 컬럼은 없다 — 역할은 행위 권한이지 소속·소유가 아니다(`PRD §7.10`) |
...
>
> **권한이 먼저인 이유** — 「누가 어느 회사에 속하는가」는 계정 모델의 문제다. 역할 구분은 `#672`·`#1301`로 생겼으나(§2.15 `role` 3종), 그것은 **행위 권한이지 소속이 아니다**(`PRD §7.10`) — **회사를 붙일 자리는 여전히 없다**(`#1663` 정정 — 종전에는 「역할 구분 자체가 없어」로 적었고, `#808`은 가입 게이트 결정이지 역할 부재 확정이 아니다).
...
| 2026-09-23 | `#1714` | **`result_json` 예시의 `ratio_to_required` `0.98758` → `0.98757`** (`#1600`). 비율도 전송 자릿수로 절사한다(`TECH_SPEC §1.2.1`). 이미 저장된 옛 결과 문자열은 고치지 않는다(`TECH_SPEC §5.4`). `AGENTS §4.3`상 값 정정이라 버전은 올리지 않는다 (#1600) |
| 2026-09-23 | `#1737` | §2.16 테넌시 착수 조건 표·각주의 역할 서술 3곳 정정 — ⑴ 실측 행의 **「사무직·현장직 2종」 → 「사무직·현장직·관리자 3종」**(2026-09-15 실측 당시에는 2종이었고 `#1301`이 09-19에 관리자를 더했다. 같은 문서 `§2.15`는 이미 3종이었다) ⑵ 각주의 **「지금은 역할 구분 자체가 없어(`#808` 확정)」**를 정정 — 역할 구분은 `#672`·`#1301`로 있고, `#808`은 가입 게이트 결정이지 역할 부재 확정이 아니다. **결론(회사를 붙일 자리가 없다)은 그대로다** — 역할은 행위 권한이지 소속이 아니기 때문이다(`PRD §7.10`) ⑶ 선행 조건 행을 **충족(`#672`·`#1301`로 닫힘)**으로 표시. `AGENTS §4.3`상 값 정정이라 버전은 올리지 않는다 (#1663) |
...
| 2026-09-25 | `#___` | §2.17 `not_underway_period`에 **「같은 선박의 구간은 겹치지 않는다」 서비스 불변식 각주** (`#1629` · PR #1852) — 겹침 조회 전에 선박 행을 `FOR UPDATE`로 잡는다(`TECH_SPEC §16.3`). 항차 쪽 `§2.4`(`#1626`)에만 있던 비대칭을 맞춘다. `§4.3`상 각주 보강이라 버전은 올리지 않는다 (#1862) |
