...
| 2026-09-24 | `#___` | §2.4 `voyage_scenario` 표 아래에 **「항차당 채택 행 하나」 불변식 각주** (`#1626` · `F-8`). `is_adopted`의 유일성이 어디에도 적혀 있지 않았다 — 서비스 불변식이며 항차 행 잠금 안에서 해제→채택 순서로 지킨다는 것, 잠금 없이는 두 행이 남는다는 `#1796` ⑺ 실측, DB 제약으로 세우지 않는 이유(부분 유니크 인덱스 없음 · `061` 활성 키 열 형태가 필요 · `F-8`이 새 DB 객체 없이 닫기로 함)와 필요 시 옮길 형태를 적었다. `AGENTS §4.3`상 각주 보강이라 버전은 올리지 않는다 (#1626) |
