...
| `imo_number` | VARCHAR(7) | NOT NULL, UNIQUE | IMO 번호 (7자리 숫자) |
...
| `cii_value` | NUMERIC(15,8) | NOT NULL | 항차 CII 추정값. **[M-8]** 시나리오 추정값은 Layer 2 수준 정밀도로 저장. Layer 1 확정값은 `calculation_run.result_json`에 문자열로 보존 |
...
| LNG_CARRIER | DWT < 65000 | fixed 65000 | **14479E10** ~~14779E10~~ | 2.673000 |
...
> **[C-2 수정]** LNG_CARRIER DWT < 65000 행의 `a_raw`를 `14779E10`에서 `14479E10`으로 정정. MEPC.364(79)에 따라 fixed-capacity 변형은 variable-capacity 대역(65000 ≤ DWT < 100000)과 동일한 `a` 값을 사용한다. `14779`는 `14479`의 전치 오류(4↔7)였다.
...
| `a_decimal` | NUMERIC(30,6) | `14479E10` = 144,790,000,000,000 (15자리). float64 한계 근접 |
...
| `voyage_scenario.cii_value` | NUMERIC(15,8) **[M-8]** | 시나리오 추정값은 Layer 2 수준 정밀도. Layer 1 확정값은 `calculation_run.result_json`에 문자열로 보존되므로 정밀도 불일치 문제 없음 |
...
| C-2 | `14779E10` → `14479E10` 오타 정정 (LNG_CARRIER seed) | seed 데이터 수정 | §3.3 |
