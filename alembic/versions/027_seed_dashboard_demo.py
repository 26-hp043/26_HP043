...
#:
#: ⚠️ **이 전제는 정박 연료에는 성립하지 않았다** (`#797`). 이 파일은 `#451` 이후
#: 무동작이라 지금 적재에는 영향이 없지만, 「그날 넣었던 값의 기록」으로 읽는 사람이
#: **같은 판단을 반복할 수 있어** 적어 둔다 — 정박 연료 8건 중 6건은
#: `DIESEL_GAS_OIL`이고 정본 CF는 `3.206000`이다. 시드는 이제 적재 시점에
#: `fuel_type` 표에서 읽으며, 기존 환경의 6행은 `038`이 정정했다.
